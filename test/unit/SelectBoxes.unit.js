...
import { comp1, comp3, comp4, comp5, comp6, comp7, comp8 } from './fixtures/selectboxes/index';
import { comp12 } from './fixtures/radio/index';
...
describe('SelectBoxes Component', function () {
      ...
      .then((form) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              assert.equal(
                _.isEqual(form.submission.metadata.selectData.selectBoxes, [
                  { name: 'Alaska' },
                  { name: 'American Samoa' },
                ]),
                true,
  ...
  describe('error messages', function () {
    it('Should have a minSelectedCount validation message', function () {
      ...
      const element = document.createElement('div');
      return Formio.createForm(element, formJson).then(async (form) => {
        form.submission = {
          data: {
            options: { 1: true },
          },
        ...
        setTimeout(() => {
          ...
          assert.equal(messageContainer.textContent.trim(), 'You must select at least 2 items');
        }, 300);
      });
    });
    ...
    it('Should use the minSelectedCountMessage if provided', function () {
      ...
      return Formio.createForm(element, formJson).then(async (form) => {
        ...
        setTimeout(() => {
          const { messageContainer } = comp.refs;
          assert.equal(messageContainer.textContent.trim(), 'Please select at least 2 items.');
        }, 300);
      });
    });
    ...
    it('Should have a maxSelectedCount validation message', function () {
      ...
      return Formio.createForm(element, formJson).then(async (form) => {
        ...
        setTimeout(() => {
          const { messageContainer } = comp.refs;
          assert.equal(messageContainer.textContent.trim(), 'You may only select up to 2 items');
        }, 300);
      });
    });
    ...
    it('Should use the maxSelectedCountMessage if provided', function () {
      ...
      return Formio.createForm(element, formJson).then(async (form) => {
        ...
        setTimeout(() => {
          const { messageContainer } = comp.refs;
          assert.equal(messageContainer.textContent.trim(), 'Please select 2 items at most.');
        }, 300);
      });
    });
  });
  ...
  it('Should interpolate config data when data source type is set to URL', function () {
    const originalMakeRequest = Formio.makeRequest;
    function fakeMakeRequest(formio, type, url) {
      if (url !== 'test123') {
  ...
  it('Should have correct submission data when setting the value property', function (done) {
    ...
    Formio.createForm(document.createElement('div'), comp12, {}).then((form) => {
      setTimeout(() => {
        ...
        setTimeout(() => {
          assert.deepEqual(form.getValue().data, {
            selectBoxes: { Bob: true, Tom: false, Joe: false },
          });
          done();
        }, 200);
        Formio.makeRequest = originalMakeRequest;
  ...
  it('Should show validation errors when the value property is set', function () {
    const originalMakeRequest = Formio.makeRequest;
    Formio.makeRequest = async () => {
      return [
        {
  ...
  it('Should perform OnlyAvailableItems check properly', function (done) {
    Harness.testCreate(SelectBoxesComponent, comp7)
      .then((component) => {
        assert.equal(
          component.validateValueAvailability(true, { a: true }),
          true,
          'Should be valid',
        );
        assert.equal(
          component.validateValueAvailability(true, { a: false, b: false, c: false }),
          true,
          'Should be valid',
        );
        assert.equal(
          component.validateValueAvailability(true, { a: false, newKey: false }),
          false,
          'Should not be valid',
        );
        assert.equal(
          component.validateValueAvailability(true, { newKey: false }),
          false,
          'Should not be valid',
        );
  ...
  it('Should wait for selectboxes url options to load before submit', function (done) {
    const element = document.createElement('div');
    const originalMakeRequest = Formio.makeRequest;
    const urlResponse = [
      {
    ...
    Formio.createForm(element, fastCloneDeep(comp8))
      .then((instance) => {
        ...
        assert.equal(
          !!selectBoxes.element.querySelector('.loader'),
          true,
  ...
  it('Should render options from metadata in readOnly when selectBoxes value is empty in submission', function (done) {
    const element = document.createElement('div');
    const originalMakeRequest = Formio.makeRequest;
    let optionsCalls = 0;
    Formio.makeRequest = function () {
  ...
  it('Should render options from metadata in readOnly when selectBoxes has a value in submission', function (done) {
    const element = document.createElement('div');
    const originalMakeRequest = Formio.makeRequest;
    let optionsCalls = 0;
    Formio.makeRequest = function () {
