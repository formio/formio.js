...
describe('Address Component', function () {
  ...
  it('Test manual mode', function (done) {
    ...
    Formio.createForm(element, form)
      .then((form) => {
        ...
        assert.equal(
          address.refs.modeSwitcher.nextElementSibling.textContent,
          'Check it to switch to manual mode',
          'Should set custom label for manual mode checkbox',
        );
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            assert.deepEqual(
              address.dataValue,
              { address: manualModeValue, mode: 'manual' },
  ...
  it('Should build an address component with multiple values and set value', function (done) {
    const element = document.createElement('div');

    Formio.createForm(element, _.cloneDeep(comp5))
      .then((form) => {
        const address = form.getComponent('address');
        assert.equal(!!address.provider, true);
