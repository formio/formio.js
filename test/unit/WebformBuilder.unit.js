...
describe('WebformBuildert tests', function () {
  describe('WebformBuilder basic tests', function () {
    this.retries(3);

    before(function (done) {
      // eslint-disable-next-line mocha/no-return-and-callback
      return Harness.builderBefore(done);
    });
    ...
    after(function (done) {
      // eslint-disable-next-line mocha/no-return-and-callback
      return Harness.builderAfter(done);
    });
    ...
    it('Should open only one default opened group at a time', async function () {
    ...
    it('Should uniquify the key for the component inside layout component that inside container', function (done) {
      const builder = Harness.getBuilder();
      builder.webform
        .setForm(uniqueApiKeysTranslation)
        .then(() => {
    ...
    it('should be able to render form with unknown component without crashing', function () {
    ...
    it('should return false if hasEditTabs is passed a type that does not exist', function () {
      const webformBuilder = new WebformBuilder({});
      assert.equal(webformBuilder.hasEditTabs('abc123'), false);
    });

    it('Should not duplicate nested components of the DataGrid for new Data components', function (done) {
      const builder = Harness.getBuilder();
  ...
  describe('WebformBuilder with nested forms', function () {
    ...
    it('Should not validate a nested form inside of dataGrid in edit mode', async function () {
