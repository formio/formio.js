...
describe('Panel Component', function () {
  ...
  it('Should keep validation errors after expanding collapsed panel', function (done) {
    ...
    Formio.createForm(element, comp2)
      .then((form) => {
        ...
        assert.equal(
          numberComp.element.classList.contains('has-error'),
          false,
          'Should not contain error classes.',
        );
        assert.equal(
          textComp.element.classList.contains('has-error'),
          false,
          'Should not contain error classes.',
        );
  ...
  it('Should show its label in DataGrid when "Hide Label" is set to true and "Show Label in DataGrid" is set to true', function () {
    return Formio.createForm(document.createElement('div'), comp3, {}).then((form) => {
      assert(
        form.getComponent('dataGrid[0]').element.querySelector('.card-title'),
        'Panel should display its label',
      );
    });
  });
