...
describe('Tabs Component', function () {
  it('Test setting error classes when set to modalEdit', function (done) {
    ...
    Formio.createForm(formElement, { display: 'form', type: 'form', components: [comp1] })
      .then((form) => {
        ...
        setTimeout(() => {
          ...
          assert(
            openModalWrapper.className.includes('formio-error-wrapper'),
            'Should have error class',
          );
          assert(
            openModalWrapper.className.includes('has-message'),
            'Should have class indicating that the component has a message',
          );

          const openModalButton = comp.element.querySelector('[ref="openModal"]');

          assert(
            !openModalButton.className.includes('tab-error'),
            'Open modal element should not have a tab-error class',
          );
          ...
          setTimeout(() => {
            ...
            assert(
              !openModalWrapper.className.includes('has-message'),
              'Should not have class indicating that the component has a message',
            );
