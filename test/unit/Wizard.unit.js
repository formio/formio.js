...
import wizardWithHighPages from '../forms/wizardWithHighPages.json';
...
describe('Wizard tests', function () {
  ...
  describe('Wizard Form with Nested Form validation', function () {
    ...
    it('Should validate wizard with nested forms with lazy load on without POST request (on client side)', async function () {
      const formElement = document.createElement('div');
      const wizard = await Formio.createForm(
        formElement,
        'http://localhost:3000/idwqwhclwioyqbw/parentForm',
      );
  ...
  it('Should fire validation on next page form deeply nested wizards and not submit invalid form', function (done) {
    const formElement = document.createElement('div');

    const forms = _.cloneDeep(wizardWithDeeplyNestedWizards);
    const originalMakeRequest = Formio.makeRequest;
    let attemptsToSubmit = 0;
    Formio.setUser({
      _id: '123',
    });
    ...
    Formio.makeRequest = (formio, type, url, method, data) => {
      ...
      if (type === 'form' && method === 'get') {
        if (url.endsWith('testwizard')) {
          return Promise.resolve(forms.mainForm);
        } else if (url.includes('69d78f1f2f4e367b7a474ba5')) {
          return Promise.resolve(forms['69d78f1f2f4e367b7a474ba5']);
        } else if (url.includes('69d78f1f2f4e367b7a474b9e')) {
          return Promise.resolve(forms['69d78f1f2f4e367b7a474b9e']);
        } else if (url.includes('69d78f1f2f4e367b7a474b82')) {
          return Promise.resolve(forms['69d78f1f2f4e367b7a474b82']);
        } else {
          return Promise.resolve();
        }
      }
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              assert.equal(wizard.errors[0].message, 'Country is required');
              const countryEl = wizard.element.querySelector('.formio-component-country');
              assert.equal(countryEl.classList.contains('has-message'), true);
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              assert.equal(
                textField.refs.messageContainer.textContent.includes(
                  'Text Field must have at least 4 characters',
                ),
                true,
              );
              done();
      ...
      .then(() => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              const editGridFirstRowInput = editGrid.element.querySelector(
              ...
              const dataGridFirstRowInput = dataGrid.element.querySelector(
  ...
  it('Should show conditional page inside nested wizard', async function () {
    const formElement = document.createElement('div');
    const wizard = new Wizard(formElement);
    const nestedWizard = _.cloneDeep(wizardTestForm.form);
  ...
  it("Should validate the entire wizard's components when the form has been submitted", async function () {
    ...
    assert.equal(
      form.alert.querySelectorAll('span[ref=errorRef]').length,
      2,
      'Should have two errors total',
    );
  });
          ...
          wizard.allPages[wizard.page].everyComponent((comp) => {
            ...
            if (!isParent) {
              ...
              const value = isInEditGrid
                ? comp.parent.refs['editgrid-editGrid-row'][comp.rowIndex].textContent.trim()
                : comp.element.querySelector("[ref='value']").textContent;
              const expectedValue = _.get(wizardTestForm.htmlModeValues, comp.path, 'no data');

              assert.equal(
                value,
                expectedValue === 'true' ? 'True' : expectedValue,
    ...
    customizedWizard
      ...
      .then(() => {
        ...
        const checkBtns = (page) => {
          ...
          assert.equal(
            !!customizedWizard.refs[`${customizedWizard.wizardKey}-next`],
            false,
            'Should not render wizard next btn',
          );
          assert.equal(
            !!customizedWizard.refs[`${customizedWizard.wizardKey}-cancel`],
            false,
            'Should not render wizard cancel btn',
          );
          assert.equal(
            !!customizedWizard.refs[`${customizedWizard.wizardKey}-previous`],
            false,
        ...
        const navigatePage = (btnKey) => {
          const customBtn =
            customizedWizard.components[customizedWizard.page].getComponent(btnKey).refs.button;
          const clickEvent = new Event('click');
      ...
      .then(() => {
        ...
        formio.makeRequest = (type, url, method, data) => {
          ...
          assert.equal(
            ...
            `Submission data should ${expectedValues[number].id ? '' : 'not'} contain id of editted submission`,
          );
          assert.equal(
    ...
    wizard
      ...
      .then(() => {
        ...
        const checkInvalidComp = (compKey, highLight) => {
          ...
          assert.deepEqual(
            comp.element.classList.contains(`${highLight ? 'formio-error-wrapper' : 'has-error'}`),
            true,
                ...
                setTimeout(() => {
                  ...
                  setTimeout(() => {
                    ...
                    assert.equal(
                      wizard.visibleErrors.length,
                      0,
                      'Should not have visible validation error',
                    );
                    clickWizardBtn(wizard, 'link[0]');
    ...
    wizardWithPageTooltip
      ...
      .then(() => {
        ...
        const pageTooltipIcon =
          wizardWithPageTooltip.refs[`${wizardWithPageTooltip.wizardKey}-tooltip`][0];
        ...
        setTimeout(() => {
          const tooltipText =
            wizardWithPageTooltip.element.querySelector('.tippy-content').textContent;
  ...
  it('Should not clear wizard data when navigating between wizard pages with hidden panel', function (done) {
    ...
    formWithHiddenPage
      ...
      .then(() => {
        ...
        setTimeout(() => {
          assert.equal(
            formWithHiddenPage.element.querySelector('[name="data[number]"]').value,
            555,
          );
      ...
      .then(() => {
        ...
        setTimeout(() => {
          const signatureImage =
            formWithSignatureHTMLMode.element.querySelector('[ref="signatureImage"]');
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                const numberComponent =
                  wizardWithCustomConditionalPage.element.querySelector('[name="data[number]"]');
    ...
    wizardWithConditionalPage
      ...
      .then(() => {
        setTimeout(() => {
          ...
          const inputEvent = new Event('input');
          const numberComponent =
            wizardWithConditionalPage.element.querySelector('[name="data[number]"]');
  ...
  it('Should show first conditional wizard page', function (done) {
    ...
    wizard
      ...
      .then(() => {
        ...
        assert.equal(
          wizard.refs[`wizard-${wizard.id}-link`][0].parentElement.classList.contains('active'),
          true,
        );
        wizard.setValue({
          data: { b: 'true' },
        ...
        setTimeout(() => {
          ...
          assert.equal(
            wizard.refs[`wizard-${wizard.id}-link`][1].parentElement.classList.contains('active'),
            true,
          );
          done();
        }, 300);
  ...
  it('Should display select submission data when lazy load is checked', async function () {
    const formElement = document.createElement('div');
    const wizardForm = await Formio.createForm(formElement, wizardWithLazyLoadSelect, {
      readOnly: true,
    });
    wizardForm.setSubmission({
      metadata: {
        selectData: {
    ...
    setTimeout(() => {
      ...
      assert.equal(
        select1.element.querySelectorAll('[aria-selected="true"] span')[0].innerHTML,
        'two',
      );
      ...
      setTimeout(() => {
        ...
        assert.equal(
          select2.element.querySelectorAll('[aria-selected="true"] span')[0].innerHTML,
          'Three',
        );
      }, 500);
    }, 500);
  ...
  it('Should set correct page index after hiding a conditionally hidden page in sibling nested wizard.', async function () {
    const formElement = document.createElement('div');
    wizardForm = new Wizard(formElement);
    ...
    assert.equal(
      wizardForm.page,
      3,
      'A2 is removed from the pages array and the page number/index must be decremented to maintain B2 as the current page',
    );
    assert.equal(wizardForm.currentPanel.title, 'B2');
  });
  ...
  describe('Conditional pages', function () {
    ...
    it('Nested conditional wizard test', function (done) {
      const formElement = document.createElement('div');
      Formio.createForm(formElement, nestedConditionalWizard)
        .then((form) => {
          ...
          setTimeout(() => {
            const secondQuestionToOpenNestedFormRadio = form.getComponent([
  ...
  it('Should retain previous checkbox checked property when navigating to another page (checked)', function (done) {
  ...
  it('Should retain previous checkbox checked property when navigating to another page (unchecked)', function (done) {
    const wizard = new Wizard(document.createElement('div'));
    wizard.setForm(WizardWithCheckboxes).then(() => {
      wizard.element.querySelector('input').click();
      wizard.element.querySelector('input').click();
