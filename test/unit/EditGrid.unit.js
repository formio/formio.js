...
describe('EditGrid Component', function () {
    ...
    Harness.testCreate(EditGridComponent, comp4).then((component) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const saveBtnsQty = component.element.querySelectorAll(
            '[ref="editgrid-editGrid-saveRow"]',
          ).length;
    ...
    Harness.testCreate(EditGridComponent, comp3).then((component) => {
      ...
      setTimeout(() => {
        const firstValue = component.element
          .querySelectorAll('[ref="editgrid-editGrid-row"]')[0]
          .querySelector('.col-sm-2')
          .textContent.trim();
        const secondValue = component.element
          .querySelectorAll('[ref="editgrid-editGrid-row"]')[1]
          .querySelector('.col-sm-2')
          .textContent.trim();
  ...
  it('Should show error messages for existing data in rows', function () {
    return Harness.testCreate(EditGridComponent, comp1).then((component) => {
      ...
      Harness.testInnerHtml(
        component,
        'li.list-group-item:nth-child(2) div.has-error div.editgrid-row-error',
        'Must be good',
      );
      Harness.testInnerHtml(
        component,
        'li.list-group-item:nth-child(4) div.has-error div.editgrid-row-error',
        'Must be good',
      );
    });
  });
  ...
  describe('Display As Modal', function () {
    it('Should show add error classes to invalid components', function (done) {
      ...
      form
        .setForm(displayAsModalEditGrid)
        .then(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              const areRequiredComponentsHaveErrorWrapper = components.every((comp) => {
                ...
                return (
                  (className.includes('required') && className.includes('formio-error-wrapper')) ||
                  true
                );
              });
      ...
      form
        ...
        .then(() => {
          ...
          setTimeout(() => {
            ...
            assert(
              !confirmationDialog,
              "Shouldn't open a confirmation dialog when no values were changed",
            );
        ...
        .then(() => {
          ...
          setTimeout(() => {
            ...
            Harness.dispatchEvent(
              'input',
              dialog,
              '[name="data[editGrid][0][firstName]"]',
              (el) => (el.value = 'Michael'),
            );
            Harness.dispatchEvent('click', dialog, '[ref="dialogClose"]');
            const confirmationDialog = document.querySelector('[ref="confirmationDialog"]');
  ...
  describe('Draft Rows', function () {
    it('Check saving rows as draft', function (done) {
      Harness.testCreate(EditGridComponent, comp5)
        .then((component) => {
          ...
          assert(
            component.editRows[0].state === 'draft',
            'Row should be saved as draft if it has errors',
          );
          done();
        })
        .catch(done);
                  ...
                  setTimeout(() => {
                    ...
                    setTimeout(() => {
                      editGrid.editRow(0).then(() => {
                        ...
                        setTimeout(() => {
                          const errorAlert = editGrid.editRows[0].dialog.querySelector(
                            `#error-list-${editGrid.id}`,
                          );
      ...
      form
        ...
        .then(() => {
          ...
          assert.equal(
            form.errors.length,
            1,
            'Should have an error saying that EditGrid is required',
          );
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              assert(
                !alert,
                'Should not show an error alert when drafts are enabled and form is not submitted',
              );
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  ...
                  setTimeout(() => {
                    ...
                    assert(
                      textFieldComponent.className.includes('has-error'),
                      'Should add error class to component even when drafts enabled if the component is not pristine',
                    );
                    document.innerHTML = '';
                    done();
                  }, 300);
    ...
    it('Should keep fields valid inside NestedForms if drafts are enabled part 2', function (done) {
      const formElement = document.createElement('div');
      const form = new Webform(formElement);
      ModalEditGrid.components[0].rowDrafts = true;
            ...
            setTimeout(() => {
              ...
              form.submit().finally(() => {
                assert.equal(
                  form.errors.length,
                  2,
                  'Should be validated after an attempt to submit',
                );
                assert.equal(
                  editGrid.editRows[0].errors.length,
                  2,
                  'Should add errors to the row after an attempt to submit',
                );
                ...
                setTimeout(() => {
                  ...
                  assert.equal(
                    editRow.errors.length,
                    2,
                    'Should add errors to components inside draft row after it was submitted',
                  );
                  const textField = editRow.components[0].getComponent('textField');
                  ...
                  assert(
                    alert,
                    'Should show an error alert when drafts are enabled and form is submitted',
                  );
                  assert(
                    textField.element.className.includes('error'),
                    'Should add error class to component even when drafts enabled if the form was submitted',
                  );
                  ...
                  setTimeout(() => {
                    ...
                    assert.equal(textField.dataValue, 'new value');
                    assert(
                      !textFieldEl.className.includes('error'),
                      'Should remove an error class from component when it was fixed',
                    );
                    const editRow = editGrid.editRows[0];
                    ...
                    setTimeout(() => {
                      ...
                      assert(
                        !textField2.element.className.includes('has-error'),
                        'Should remove an error class from component when it was fixed',
                      );
                      ...
                      setTimeout(() => {
                        assert(
                          !form.alert,
                          'Should remove an error alert after all the rows were fixed',
                        );

                        const rows = editGrid.element.querySelectorAll(
  ...
  it("Test simple conditions based on the EditGrid's child's value and default values when adding rows", function (done) {
    ...
    const form = new Webform(formElement);
    form
      .setForm({ display: 'form', components: [comp7], type: 'form' })
      .then(() => {
  ...
  it('Test refreshing inside EditGrid', function (done) {
    ...
    const form = new Webform(formElement);
    form
      .setForm({ display: 'form', components: [comp8], type: 'form' })
      .then(() => {
    ...
    form
      ...
      .then(() => {
        ...
        const checkRows = (columnsNumber, rowsNumber) => {
          ...
          assert.equal(
            rowWithColumns.children.length,
            columnsNumber,
            'Row should contain values only for visible components',
          );
    ...
    Formio.createForm(element, form)
      .then((form) => {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            assert.deepEqual(
              editGrid.editRows[0].data,
              { number: 11111, textField: '11111' },
              'Should set row data',
            );
            click('saveRow', 0);
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  ...
                  setTimeout(() => {
                    assert.equal(
                      editGrid.refs['editgrid-editGrid-row'].length,
                      1,
                      'Should have 1 row',
                    );
                    assert.equal(editGrid.editRows.length, 1, 'Should have 1 row');
  ...
  it('Should disable adding/removing rows', function (done) {
    ...
    const value = [
      { number: 1, textField: 'test' },
      { number: 2, textField: 'test2' },
    ];
    ...
    Formio.createForm(element, form)
      .then((form) => {
        ...
        setTimeout(() => {
          ...
          assert.equal(
            editGrid.refs['editgrid-editGrid-addRow'].length,
            0,
            'Should not have add row btn',
          );
          assert.equal(
            editGrid.element.querySelectorAll('.removeRow').length,
            0,
            'Should not have remove row btn',
          );
  ...
  it('Should use custom text for save/cancel/add btns', function (done) {
    ...
    Formio.createForm(element, form)
      .then((form) => {
        ...
        setTimeout(() => {
          assert.equal(
            editGrid.refs['editgrid-editGrid-saveRow'][0].textContent.trim(),
            'save custom',
          );
          assert.equal(
            editGrid.refs['editgrid-editGrid-cancelRow'][0].textContent.trim(),
            'cancel custom',
          );
  ...
  it('Should render headers when openWhenEmpry is enabled', function (done) {
    ...
    Formio.createForm(element, form)
      .then((form) => {
        ...
        const headerEls =
          editGrid.element.querySelector('.list-group-header').firstElementChild.children;
        assert.equal(headerEls.length, rowComponents.length);
        for (let index = 0; index < headerEls.length; index++) {
          const el = headerEls[index];
          assert.equal(
            el.textContent.trim(),
            rowComponents[index].label,
            `Should render ${rowComponents[index].key} component label in header`,
          );
        }
        done();
      })
  ...
  it('Should keep value for conditional editGrid on setValue when server option is provided', function (done) {
    ...
    Formio.createForm(element, formsWithEditGridAndConditions.form1, { server: true })
      .then((form) => {
        const formData = {
          checkbox: true,
          radio: 'yes',
  ...
  it('Should set value for conditional editGrid inside editGrid on event when form is not pristine ', function (done) {
    ...
    Formio.createForm(element, formsWithEditGridAndConditions.form2)
      .then((form) => {
  ...
  it('Should keep value for conditional editGrid in tabs on setValue when server option is provided', function (done) {
    ...
    Formio.createForm(element, formsWithEditGridAndConditions.form3, { server: true })
      .then((form) => {
        const formData = {
          affectedRiskTypes: {
            creditRisk: false,
        ...
        setTimeout(() => {
          ...
          assert.deepEqual(
            editGrid.dataValue,
            formData.mr.quantitativeInformation.impactsPerEntity,
          );
  ...
  it('Should keep value for conditional editGrid deeply nested in panels and containers on setValue when server option is provided', function (done) {
    ...
    Formio.createForm(element, formsWithEditGridAndConditions.form5, { server: true })
      .then((form) => {
        const formData = {
          generalInformation: {
        ...
        setTimeout(() => {
          ...
          assert.deepEqual(
            editGrid.dataValue,
            formData.generalInformation.deSpecific.suboutsourcers,
          );
  ...
  it('Should calculate editGrid value when condition is met in advanced logic', function (done) {
    ...
    Formio.createForm(element, formsWithEditGridAndConditions.form6)
      .then((form) => {
        ...
        setTimeout(() => {
          ...
          assert.deepEqual(editGrid.dataValue, [
            { number: 1, textArea: 'test' },
            { number: 2, textArea: 'test2' },
          ]);
          assert.deepEqual(editGrid.editRows.length, 2);
  ...
  it('Should not show validation in new row with required conditional fields before attempt to save', function (done) {
    const form = _.cloneDeep(comp12);
    const element = document.createElement('div');
  ...
  it('Should trigger validation onChange before attempt to save', function (done) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            assert.equal(
              editGrid.editRows[0].errors.length,
              1,
              'Should include custom validation error',
            );
            ...
            setTimeout(() => {
              assert.equal(
                editGrid.editRows[0].errors.length,
                0,
                'Should not include custom validation error',
              );
              document.innerHTML = '';
              done();
            }, 300);
  ...
  it('Should not allow to save invalid row when there are required components inside columns in the editGrod row', function (done) {
    const formElement = document.createElement('div');
    const form = new Webform(formElement);
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                assert.equal(
                  textField.dataValue,
                  '',
                  'Value should be cleared after cancelling the row',
                );
                ...
                setTimeout(() => {
                  ...
                  setTimeout(() => {
                    assert.equal(
                      editGrid.editRows.length,
                      1,
                      'Should add the first row when delete the last one',
                    );
                    assert.equal(
                      editGrid.editRows[0].state === 'new',
                      1,
                      'Should add the new row when the last one was deleted',
                    );
        ...
        .then((form) => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                assert.equal(
                  editGrid.editRows[2].state,
                  'editing',
                  'Should keep the row in the editing state',
                );
                assert.equal(
                  editGrid.editRows[3].state,
                  'saved',
                  'Should keep the next row in the saved state',
                );
                done();
              }, 200);
            }, 200);
        ...
        .then(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              assert(
                textField.element.className.includes('formio-error-wrapper'),
                'Should add error class to component',
              );
              done();
            }, 450);
          }, 100);
