...
describe('TextField Builder', function () {
  ...
  it('Should create a new textfield component', function () {
    ...
    return builder.editForm.formReady.then(() => {
      ...
      assert(
        preview.indexOf(
          'formio-component formio-component-textfield formio-component-textField',
        ) !== -1,
        'Must have correct classes',
      );
      assert(
        preview.indexOf('<label class="control-label" style="">Text Field</label>') !== -1,
        'Must have a label',
      );
      assert(
        preview.indexOf('<input name="data[textField]" type="text" class="form-control"') !== -1,
        'Must have an input',
      );
    });
  });
  ...
  it('Should allow you to change the label', function (done) {
    Harness.setComponentProperty('label', 'Text Field', 'First Name', (preview) => {
      ...
      assert(
        preview.indexOf('<label class="control-label" style="">First Name</label>') !== -1,
        'Must have a label',
      );
      done();
    });
  ...
  it('Should set the prefix of the input', function (done) {
    Harness.testBuilderProperty(
      ...
      '',
      '$',
      /div class="input-group">.*<div class="input-group-addon">\$<\/div>.*input/,
      done,
    );
  });
  ...
  it('Should set the suffix of the input', function (done) {
    Harness.testBuilderProperty(
      ...
      '',
      'USD',
      /div class="input-group">.*input.*<div class="input-group-addon">USD<\/div>/,
      done,
    );
  });
  ...
  it('Should set the custom css class of the input', function (done) {
    Harness.testBuilderProperty('customClass', '', 'custom-text-field', null, () => {
      assert(
        builder.preview.hasClass(builder.preview.element, 'custom-text-field'),
