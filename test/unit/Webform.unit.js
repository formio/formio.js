...
import calculateValueWithManualOverrideLableValueDataGrid from '../forms/calculateValueWithManualOverrideLableValueDataGrid.js';
...
describe('Webform tests', function () {
  this.retries(3);
  let formElement, checkForErrors;
  ...
  it('Should set calculated value correctly when uploading files', function (done) {
    formElement.innerHTML = '';
    const form = new Webform(formElement);
    form
  ...
  describe('Initially Collapsed Panel', function () {
    let formElement, form, formPromise;

    before(function () {
      formElement = document.createElement('div');
      form = new Webform(formElement, { language: 'en' });
      formPromise = form.setForm(initiallyCollapsedPanel);
    });
    ...
    it('Should open when an Error occured', async function () {
      await formPromise;
      try {
        await form.executeSubmit();
      } catch (ignoreErr) {
        const panelBody = form.element.querySelector('[ref=nested-panel]');
  ...
  xit('Test Truncate Multiple Spaces', async function (done) {
    try {
      const formElement = document.createElement('div');
      const form = new Webform(formElement);
  ...
  describe('SaveDraft functionality', function () {
    let originalMakeRequest, saveDraftCalls, restoreDraftCalls, state, scenario, restoredDraftData;
