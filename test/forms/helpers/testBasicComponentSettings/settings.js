...
export default {
  ...
  customDefaultValue: {
    ...
    tree: {
      ...
      expectedValue: { children: [], data: { textFieldTree: 'base value default' } },
    },
    file: {
      js: `
      value = [{
  ...
  calculateValue: {
    ...
    email: {
      js: 'value = `${data.basis.split(" ").join("")}@example.com`',
      expectedValue: (basis) => `${basis.split(' ').join('')}@example.com`,
    },
    url: {
      js: 'value = `https://${data.basis.split(" ").join("")}.com`',
      expectedValue: (basis) => `https://${basis.split(' ').join('')}.com`,
    },
    phoneNumber: {
      js: 'value = `(${data.basis ? "222":"333"}) 555-5555`',
    ...
    day: {
      js: 'value = data.basis.length > 5 ? "05/05/2015" : "03/03/2003"',
      expectedValue: (basis) => (basis.length > 5 ? '05/05/2015' : '03/03/2003'),
    },
    time: {
      js: 'value = data.basis.length > 5 ? "04:45:00" : "04:05:00"',
      expectedValue: (basis) => (basis.length > 5 ? '04:45:00' : '04:05:00'),
    },
    ...
    container: {
      ...
      expectedValue: (basis) => ({ textFieldContainer: basis + ' calculated' }),
    },
    dataMap: {
    ...
    tree: {
      ...
      expectedValue: (basis) => ({ children: [], data: { textFieldTree: basis + ' calculated' } }),
    },
    file: {
  ...
  'validate.custom': _.reduce(
    ...
    (obj, componentKey) => {
      ...
      obj[componentKey] =
        `valid = !_.isEqual(instance.dataValue, ${_.isNumber(value) ? value : JSON.stringify(value)}) ? true : 'Custom validation message: component is invalid.'`;
      return obj;
    },
    {},
  ),
