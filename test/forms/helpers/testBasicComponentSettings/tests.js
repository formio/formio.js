    ...
    'Should show placeholder'(form, done) {
      form.components.forEach((comp) => {
        ...
        if (compType === 'day') {
          ...
          renderedPlaceholder =
            compType === 'select' ? compInput.attributes.placeholder.value : compInput.placeholder;
      ...
      FormioUtils.eachComponent(
        ...
        (component) => {
          ...
          if (component.type === 'datagrid') {
            componentInstance.component.components.forEach((comp) =>
              _.set(comp, 'validate.required', true),
            );
          }
          if (
      ...
      const testVisibility = (shouldBeVisible) => {
        testComponents.forEach((comp) => {
          ...
          assert.equal(
            ...
            shouldBeVisible,
            `${compKey} (component ${compType}): should ${shouldBeVisible ? 'not' : ''} be conditionally visible`,
          );
          ...
          if (compType !== 'well') {
            assert.equal(
              comp.element.classList.contains('formio-hidden'),
              !shouldBeVisible,
              `Should ${shouldBeVisible ? 'not' : ''} set formio-hidden class for ${compKey} (component ${compType})`,
            );
          }
        });
      };
