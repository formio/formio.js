...
export default {
			...
			form.components.forEach((comp) => {
				...
				const checkDisabled = (component, child) => {
					...
					if (componentType === 'day') {
						compInputs = Object.keys(component.component.fields).map(
							(fieldName) => {
		...
		'Should redraw on checkbox value change'(form, done) {
			...
			setTimeout(() => {
				...
				form.components.forEach((comp) => {
					...
					assert.deepEqual(
						comp.name.trim().endsWith(changedCheckboxValue.toString()),
						true,
		...
		'Should set multiple values'(form, done) {
			...
			setTimeout(() => {
				form.components.forEach((comp) => {
					...
					assert.deepEqual(
						comp.type === 'datetime'
							? comp.getValue().every((val, ind) => val.startsWith(value[ind]))
							: comp.getValue(),
	...
	modalEdit: {
		'Should open and close modal window'(form, done) {
			...
			const testComponents = form.components.filter(
				(comp) =>
					![...componentsWithBug, 'button'].includes(comp.component.type),
			);
			testComponents.forEach((comp, index) => {
				const isLastComp = index === testComponents.length - 1;
				const compKey = comp.component.key;
		...
		'Should delete component changes when closing modal window and clicking "delete it" in confirmation dialog'(
			...
			const testComponents = form.components.filter(
				(comp) =>
					!['htmlelement', 'content', 'button'].includes(comp.component.type),
			);
					...
					setTimeout(() => {
						if (layoutComponents.includes(compType)) {
							_.each(comp.components, (child) => {
								...
								assert.deepEqual(
									childType === 'datetime'
										? childDataValue.startsWith(childExpectedValue)
										: childDataValue,
						...
						setTimeout(() => {
							...
							const clearChangesBtn = confirmationDialog.querySelector(
								'[ref="dialogYesButton"]',
							);
							clearChangesBtn.dispatchEvent(clickEvent);

							setTimeout(() => {
								const confirmationDialogAfter = document.querySelector(
									'.formio-dialog-content[ref="dialogContents"]',
								);
								assert.deepEqual(
				...
				setTimeout(() => {
					testComponents
						...
						.forEach((comp) => {
							...
							assert.deepEqual(
								comp.subForm ? !!comp.subForm.errors.length : !!comp.error,
								true,
			...
			const checkCalculatedValue = () => {
				form.components.forEach((comp) => {
					...
					const getExpectedCalculatedValue = (basis) =>
						settings.calculateValue[`${compKey}`].expectedValue(basis);
					...
					assert.deepEqual(
						compType === 'datetime'
							? inputValue.startsWith(getExpectedCalculatedValue(basis))
							: inputValue,
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					...
					testComponents.forEach((comp) => {
						...
						assert.deepEqual(
							!!comp.refs.messageContainer.querySelector('.error'),
							false,
		...
		'Should show validation errors for nested components'(form, done, test) {
			...
			form.everyComponent((comp) => {
				...
				if (
					...
					![...layoutComponents, 'datagrid'].includes(component.type) &&
					(!treeComponent || !treeComponent.getComponents().includes(comp))
				) {
	...
	logic: {
		...
		'Should execute value action if js logic condition is met'(
			...
			form
				.getComponent('basis')
				.setValue('some text value with length over twenty');
			setTimeout(() => {
				checkSetValue(
					testComponents,
