    ...
    if (thisScript) {
        ...
        const resolveLibs = (cdn) => {
            ...
            if (cdn instanceof CDN) {
                ...
                libs.uswds.js = `${url}/uswds/${cdn.getVersion(
                    'uswds',
                )}/uswds.min.js`;
                libs.uswds.css = `${url}/uswds/${cdn.getVersion(
                    'uswds',
                )}/uswds.min.css`;
                libs.fontawesome.css = `${url}/font-awesome/${cdn.getVersion(
                    'font-awesome',
                )}/css/font-awesome.min.css`;
                libs.bootstrap.css = `${url}/bootstrap/${cdn.getVersion(
                    'bootstrap',
        ...
        const createElement = (tag, attrs, children) => {
            ...
            (children || []).forEach((child) => {
                element.appendChild(
                    createElement(child.tag, child.attrs, child.children),
                );
            });
            return element;
        };
        ...
        addScript(config.script, 'Formio', (Formio) => {
            const renderForm = () => {
                ...
                const isReady =
                    ...
                    Formio.Promise.resolve();
                const form = config.form || config.src;
                debug('Creating form', form, config.config);
                isReady.then(() => {
                    Formio.license = true;
                    Formio.createForm(formElement, form, config.config).then(
                        (instance) => {
                            const submitDone = (submission) => {
                                ...
                                if (
                                    ...
                                    debug(
                                        'Return url found in form configuration',
                                    );
                                    returnUrl =
                                        instance._form.settings.returnUrl ||
                                        instance._form.settings.redirect;
                                ...
                                if (returnUrl) {
                                    const formSrc = instance.formio
                                        ? instance.formio.formUrl
                                        : '';
                                    const hasQuery = !!returnUrl.match(/\?/);
                                    const isOrigin =
                                        returnUrl.indexOf(location.origin) ===
                                        0;
                                    returnUrl += hasQuery ? '&' : '?';
                                    returnUrl += `sub=${submission._id}`;
                                    if (!isOrigin && formSrc) {
                                        returnUrl += `&form=${encodeURIComponent(
                                            formSrc,
                                        )}`;
                                    }
                            ...
                            instance.on('submit', (submission) => {
                                ...
                                if (config.submit) {
                                    debug(
                                        `Sending submission to ${config.submit}`,
                                    );
                                    const headers = {
            ...
            if (config.template) {
                ...
                let templateSrc;
                if (cdn instanceof CDN) {
                    templateSrc = `${cdn[config.template]}/${
                        config.template
                    }.min`;
                } else {
