...
export class Formio {
  ...
  static async addLibrary(libWrapper, lib, name) {
    ...
    if (lib.js) {
      const module = await Formio.addScript(
        lib.global ? document.body : libWrapper,
        lib.js,
        lib.use ? name : false,
      );
      if (lib.use) {
  ...
  static async init(element, options = {}, builder = false) {
    ...
    Formio.FormioClass.setProjectUrl(
      options.projectUrl || Formio.projectUrl || Formio.config.project,
    );
    ...
    await Formio.addStyles(
      libWrapper,
      Formio.formioScript(Formio.config.style || `${Formio.cdn.js}/${renderer}.css`, builder),
    );
