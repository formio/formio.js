...
describe('WizardBuilder tests', function () {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                assert.equal(
                    builder.instance.webform.components[0].key,
                    'page2',
                    'Should switch to the next page when' +
    ...
    it('Test page remove with confirmation when remove from component settings window', function (done) {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                assert(
                    builder.instance.editForm,
                    'Should create the settings form on component edit',
                );
                const removeButton =
                    builder.instance.componentEdit.querySelector(
                        '[ref="removeButton"]',
                    );
    ...
    it('Test pages reorder', function (done) {
        ...
        setTimeout(() => {
            ...
            const addPageBtn =
                builder.instance.element.querySelector('.wizard-add-page');
            assert.equal(
                drake.canMove(addPageBtn),
                false,
                'Should not be able to move Add Page button',
            );
            const pagesElements =
                builder.instance.element.querySelectorAll('.wizard-pages li');
            assert.equal(
            ...
            assert.equal(
                drake.canMove(firstPage),
                true,
                'Should be able to move page button',
            );
            done();
        }, 500);
    });
