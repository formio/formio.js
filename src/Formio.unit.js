            ...
            if (typeof testFn === 'function') {
                ...
                for (const param in testFn) {
                    assert.equal(
                        formio[param],
                        testFn[param],
                        `${param} is not equal. ${formio[param]} == ${testFn[param]}\n`,
                    );
                }
            }
            done();
...
describe('Formio.js Tests', function () {
    describe('Formio Constructor Tests', function () {
        runTests((tests) => {
            ...
            tests[
                `http://form.io/project/${projectId}/form/${formId}/action/${actionId}`
            ] = {
                projectUrl: `http://form.io/project/${projectId}`,
                projectsUrl: 'http://form.io/project',
                projectId: projectId,
            ...
            tests[
                `http://form.io/project/${projectId}/form/${formId}/submission/${submissionId}`
            ] = {
                projectUrl: `http://form.io/project/${projectId}`,
                projectsUrl: 'http://form.io/project',
                projectId: projectId,
            ...
            tests[
                `http://form.io/project/${projectId}/form/${formId}?test=hello&test2=there`
            ] = {
                projectUrl: `http://form.io/project/${projectId}`,
                projectsUrl: 'http://form.io/project',
                projectId: projectId,
            ...
            tests[
                `http://project.form.io/user/login/submission/${submissionId}`
            ] = {
                projectUrl: 'http://project.form.io',
                projectsUrl: `${baseUrl}/project`,
                projectId: 'project',
            ...
            tests[
                `http://project.form.io/user/login/action/${actionId}?test=test2`
            ] = {
                projectUrl: 'http://project.form.io',
                projectsUrl: `${baseUrl}/project`,
                projectId: 'project',
            ...
            tests[
                `http://project.form.io/user/loginform/action/${actionId}?test=test2`
            ] = {
                projectUrl: 'http://project.form.io',
                projectsUrl: `${baseUrl}/project`,
                projectId: 'project',
            ...
            tests[
                `http://project.form.io/user/actionform/submission/${submissionId}`
            ] = {
                projectUrl: 'http://project.form.io',
                projectsUrl: `${baseUrl}/project`,
                projectId: 'project',
                ...
                actionId: '',
                submissionsUrl:
                    'http://project.form.io/user/actionform/submission',
    ...
    describe('Plugins', function () {
        ...
        beforeEach(function () {
            ...
            assert.ok(
                plugin.init.calledOnce,
                'plugin.init must be called exactly once',
            );
            assert.ok(
                plugin.init.calledOn(plugin),
                'plugin.init must be called on plugin as `this`',
            );
            assert.ok(
        ...
        afterEach(function () {
            ...
            assert.ok(
                plugin.deregister.calledOnce,
                'plugin.deregister must be called exactly once',
            );
            assert.ok(
        ...
        const testRequest = function testRequest(url, method, type) {
            ...
            switch (method) {
                case 'GET':
                    fnName = `load${_.capitalize(type)}`;
                    break;
                case 'POST':
                case 'PUT':
                    fnName = `save${_.capitalize(type)}`;
                    break;
                case 'DELETE':
                    fnName = `delete${_.capitalize(type)}`;
                    break;
            }
            ...
            it(`Plugin ${method} ${fnName}`, function (done) {
                ...
                plugin.preRequest = function (requestArgs) {
                    assert.equal(
                        ++step,
                        1,
                        'preRequest hook should be called first',
                    );
                    assert.deepEqual(
                        requestArgs,
                        expectedArgs,
                    ...
                    return Promise.resolve().then(() => {
                        assert.equal(
                            ++step,
                            3,
                            'preRequest promise should resolve third',
                        );
                        // TODO
                    });
                };
                plugin.request = function (requestArgs) {
                    assert.equal(
                        ++step,
                        4,
                        'request hook should be called fourth',
                    );
                    assert.deepEqual(
                        requestArgs,
                        expectedArgs,
                    ...
                    return Promise.resolve().then(() => {
                        assert.equal(
                            ++step,
                            5,
                            'request promise should resolve fifth',
                        );
                        return testResult;
                    });
                };
                plugin.wrapRequestPromise = function (promise, requestArgs) {
                    assert.equal(
                        ++step,
                        2,
                        'wrapRequestPromise hook should be called second',
                    );
                    assert.deepEqual(
                        requestArgs,
                        expectedArgs,
                    ...
                    return promise.then((result) => {
                        assert.equal(
                            ++step,
                            6,
                            'wrapRequestPromise post-result promise should resolve sixth',
                        );
                        assert.deepEqual(
                            result,
                            testResult,
                            'Result should match result from request hook',
                        );
                        return result;
                    });
                };
                ...
                promise.then((result) => {
                    assert.equal(
                        ++step,
                        7,
                        'post request promise should resolve last',
                    );
                    assert.deepEqual(
                        result,
                        testResult,
        ...
        const testStaticRequest = function testStaticRequest(
            fnName,
            url,
            method,
        ) {
            it(`Plugin ${fnName}`, function (done) {
                ...
                plugin.wrapStaticRequestPromise = function (
                    promise,
                    requestArgs,
                ) {
                    assert.equal(
        ...
        const testFileRequest = function testFileRequest(
            fnName,
            formUrl,
            args,
        ) {
            it(`Plugin ${fnName}`, function (done) {
                ...
                plugin.wrapFileRequestPromise = function (
                    promise,
                    requestArgs,
                ) {
                    assert.equal(
                        ++step,
                        2,
                        'wrapFileRequestPromise hook should be called second',
                    );
                    assert.deepEqual(
                        requestArgs,
                        expectedArgs,
                    ...
                    return promise.then((result) => {
                        assert.equal(
                            ++step,
                            6,
                            'wrapFileRequestPromise post-result promise should resolve sixth',
                        );
                        assert.deepEqual(
                            result,
                            testResult,
                ...
                formio[fnName].apply(null, args).then((result) => {
                    assert.equal(
                        ++step,
                        7,
                        'post request promise should resolve last',
            ...
            it(test.name, function (done) {
                ...
                if (test.mock) {
                    ...
                    if (mock instanceof Array) {
                        _.each(mock, (_mock) => {
                            fetchMock.mock(_mock.url, _mock.response, {
                                method: _mock.method,
                            });
                        });
                    } else {
                        fetchMock.mock(mock.url, mock.response, {
                            method: mock.method,
                        });
                    }
                }
        ...
        const tests = [
            {
                ...
                test() {
                    ...
                    const formio = new Formio(
                        `${Formio.getBaseUrl()}/user/register`,
                    );
                    return formio.saveSubmission(req).then((response) => {
                        assert.deepEqual(
                            response,
                            user,
                            'saveSubmission response should match test user',
                        );
                        assert.equal(
                            Formio.getToken(),
                            userToken,
                            'Formio should save the user token',
                        );
                    });
                },
                mock() {
                    ...
                    return Formio.currentUser()
                        .then((response) => {
                            assert.deepEqual(
                                response,
                                user,
                                'currentUser response should match test user',
                            );
                            return Formio.currentUser();
                        })
                        .then((response) => {
                ...
                mock() {
                    ...
                    return {
                        ...
                        response() {
                            assert.ok(
                                !called,
                                'User should be requested only once.',
                            );
                            called = true;
                            return {
                                headers: {
                ...
                test() {
                    ...
                    return formio.saveProject(req).then((response) => {
                        assert.deepEqual(
                            response,
                            project,
                            'saveProject response should match test user',
                        );
                    });
                },
                mock() {
            ...
            },
            {
                ...
                test() {
                    const formio = new Formio(
                        `${Formio.getBaseUrl()}/project/${project._id}`,
                    );
            ...
            },
            {
                ...
                test() {
                    const formio = new Formio(
                        `/project/${project._id}/form/${form._id}`,
                    );
            ...
            },
            {
                ...
                test() {
                    const formio = new Formio(
                        `/project/${project._id}/form/${form._id}/submission`,
                    );
                    const req = {
                        data: {
                ...
                test() {
                    ...
                    return formio.loadSubmissions().then((submissions) => {
                        assert.equal(
                            submissions.length,
                            1,
                            'Should return only one submission.',
                        );
            ...
            },
            {
                ...
                mock() {
                    return {
                        url: `${Formio.getBaseUrl()}/project/${
                            project._id
                        }/form/${form._id}/submission/${submission._id}`,
                        method: 'GET',
                        response() {
                            return {
                                headers: {
                ...
                test() {
                    return Formio.loadProjects().then((projects) => {
                        ...
                        assert.equal(
                            projects.skip,
                            undefined,
                            'skip should be undefined.',
                        );
                        assert.equal(
                            projects.limit,
                            undefined,
                            'limit should be undefined.',
                        );
                        assert.equal(
                            projects.serverCount,
                            0,
                            'serverCount should be 0.',
                        );
                    });
                },
                mock() {
            ...
            },
            {
                ...
                test() {
                    ...
                    return formio
                        .userPermissions(user, undefined, submission)
                        .then((permissions) => {
                            assert.equal(permissions.create, true);
                            assert.equal(permissions.edit, false);
                            assert.equal(permissions.read, false);
            ...
            },
            {
                ...
                test() {
                    ...
                    return Promise.all([
                        formio
                            .userPermissions(user1, undefined, submission)
                            .then((permissions) => {
                                assert.equal(permissions.create, false);
                                assert.equal(permissions.read, true);
                                assert.equal(permissions.edit, false);
                        ...
                        formio
                            .userPermissions(user2, undefined, submission)
                            .then((permissions) => {
                                assert.equal(permissions.create, false);
                                assert.equal(permissions.read, true);
                                assert.equal(permissions.edit, false);
    ...
    describe('Formio.currentUser', function () {
        ...
        it('Initial currentUser() should make static request', function (done) {
            ...
            Formio.currentUser().then(() => {
                assert.ok(
                    plugin.staticRequest.calledOnce,
                    'staticRequest should be called once',
                );
                done();
            });
            assert.ok(
        ...
        it('Next currentUser() should return cached value', function (done) {
            ...
            Formio.currentUser().then(() => {
                assert.ok(
                    !plugin.staticRequest.called,
                    'staticRequest should not be called',
                );
                done();
            });
            assert.ok(
        ...
        it("Should render after form submission if renderMode = 'html'", function (done) {
            ...
            Formio.createForm(element, formJson, { renderMode: 'html' })
                .then((form) => {
                    ...
                    assert.equal(
                        textField.element.querySelector('[ref=value]')
                            .innerHTML,
                        '-',
                    );
                    assert.equal(
                        phoneNumber.element.querySelector('[ref=value]')
                            .innerHTML,
                        '-',
                    );
                    form.submission = {
                ...
                .then((form) => {
                    assert.equal(
                        form
                            ...
                            .element.querySelector('[ref=value]').innerHTML,
                        '-',
                    );
                    assert.equal(
                        form
