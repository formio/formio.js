...
export default class Webform extends NestedDataComponent {
      ...
      if (err) {
        const createListItem = (message, index) => {
          const messageFromIndex =
            !_.isUndefined(index) && err.messages && err.messages[index];
          const keyOrPath =
            (messageFromIndex && messageFromIndex.formattedKeyOrPath) ||
  ...
  onChange(flags, changed, modified, changes) {
    ...
    if (changed && changed.component) {
      this.customErrors = this.customErrors.filter(
        (err) => err.component && err.component !== changed.component.key,
      );
    }
  ...
  setMetadata(submission) {
    ...
    _.defaults(submission.metadata, {
      ...
      offset: parseInt(
        _.get(this, '_submission.metadata.offset', moment().utcOffset()),
        10,
      ),
