...
function getOptions(options) {
    options = _.defaults(options, {
        submitOnEnter: false,
        iconset: getIconSet(
            options && options.icons ? options.icons : Formio.icons,
        ),
        i18next: null,
        saveDraft: false,
        alwaysDirty: false,
...
export default class Webform extends NestedDataComponent {
    ...
    constructor() {
            ...
            if (options.i18n.resources) {
                ...
                _.each(options.i18n, (lang, code) => {
                    if (code === 'options') {
                        ...
                        i18n.resources[code] = {
                            translation: _.assign(
                                fastCloneDeep(
                                    i18nDefaults.resources.en.translation,
                                ),
                                lang,
                            ),
                        };
        ...
        if (this.options.saveDraftThrottle) {
            this.triggerSaveDraft = _.throttle(
                this.saveDraft.bind(this),
                this.options.saveDraftThrottle,
            );
        } else {
    ...
    addLanguage(code, lang, active = false) {
        if (this.i18next) {
            var translations = _.assign(
                fastCloneDeep(i18nDefaults.resources.en.translation),
                lang,
            );
                ...
                cleanupThis.i18next.init(
                    ...
                    (err) => {
                        if (!cleanupThis) {
                            reject(
                                new Error(
                                    'Lost reference to `this` while initializing i18next.',
                                ),
                            );
                        }
                        // Get language but remove any ;q=1 that might exist on it.
                        cleanupThis.options.language =
                            cleanupThis.i18next.language.split(';')[0];
                        if (err) {
                            cleanupThis = null;
                            return reject(err);
                        }
    ...
    setForm(form, flags) {
        ...
        try {
            ...
            if (
                isFormAlreadySet &&
                JSON.stringify(this._form) === JSON.stringify(form)
            ) {
                return NativePromise.resolve();
            }
    ...
    setSubmission(submission, flags = {}) {
        flags = {
            ...flags,
            fromSubmission: _.has(flags, 'fromSubmission')
    ...
    init() {
        ...
        this.on(
            'checkValidity',
            (data) => this.checkValidity(data, true, data),
            true,
        );
        this.on(
            'requestUrl',
            (args) => this.submitUrl(args.url, args.headers),
            true,
        );
    ...
    showErrors(error, triggerEvent, onChange) {
        ...
        errors.forEach((err) => {
            ...
            components.forEach((path) => {
                const originalPath =
                    this._parentPath + getStringFromComponentPath(path);
        ...
        errors.forEach((err) => {
            if (err) {
                const createListItem = (message, index) => {
                    ...
                    let formattedKeyOrPath = keyOrPath
                        ? getStringFromComponentPath(keyOrPath)
                        : '';
                ...
                if (err.messages && err.messages.length) {
                    ...
                    err.messages.forEach(
                        ({ message, context, fromServer }, index) => {
                            const text =
                                context?.hasLabel || fromServer
                                    ? this.t('alertMessage', {
    ...
    onChange(flags, changed, modified, changes) {
        ...
        if (changed && changed.component) {
            this.customErrors = this.customErrors.filter(
                (err) =>
                    err.component && err.component !== changed.component.key,
            );
        }
    ...
    setMetadata(submission) {
        ...
        _.defaults(submission.metadata, {
            timezone: _.get(
                this,
                '_submission.metadata.timezone',
                currentTimezone(),
            ),
            offset: parseInt(
                _.get(
                    this,
                    '_submission.metadata.offset',
                    moment().utcOffset(),
                ),
                10,
            ),
    ...
    submitForm(options = {}) {
        ...
        return new NativePromise((resolve, reject) => {
            ...
            this.hook(
                'beforeSubmit',
                { ...submission, component: options.component },
                (err, data) => {
                    if (err) {
                        return reject(err);
                    }
    ...
    submitUrl(URL, headers) {
        ...
        if (headers && headers.length > 0) {
            headers.map((e) => {
                if (e.header !== '' && e.value !== '') {
                    settings.headers[e.header] = this.interpolate(
                        e.value,
                        submission,
                    );
                }
    ...
    triggerRecaptcha() {
        ...
        if (recaptchaComponent.length > 0) {
            recaptchaComponent[0].verify(
                `${this.form.name ? this.form.name : 'form'}Load`,
            );
        }
    }
