...
export default class PasswordStrengthAddon extends FormioAddon {
        ...
        return {
            length: {
                check: (value, options) => {
                    const minLength =
                        options.minLength ||
                        this.component.component.validate.minLength ||
                        6;
    ...
    set entropy(value) {
        ...
        const updateOnEntropyChange =
            this.settings.updateOn === 'entropyChange' &&
            this._entropy !== value;
        this._entropy = value;
        this.level = this.getLevel();
        const updateOnLevelChange =
            this.settings.updateOn === 'levelChange' &&
            oldLevel.name !== this.level.name;
    ...
    calculatePasswordEntropyWords(wordsCount) {
        return !this.dictionarySize
            ? 0
            : this.log2(this.dictionarySize) * wordsCount;
    }
    ...
    checkBlackList(value) {
        ...
        if (
            ...
            typeof customBlacklistedWords === 'string'
        ) {
        ...
        if (blacklistedWords.length) {
            ...
            const { charactersPoolSize } = restValue.length
                ? this.performChecks(restValue)
                : 0;
            ...
            const dictionaryCheckEntropy = this.calculatePasswordEntropyWords(
                blacklistedWords.length,
            );
            const entropy =
                dictionaryCheckEntropy + entropyOfNonblacklistedValue;
    ...
    performChecks(value) {
        ...
        this.rulesSettings.forEach((settings) => {
            if (this.rules[settings.name]) {
                ...
                charactersPoolSize += this.handleRuleCheckResult(
                    valid,
                    rule,
                    message,
                    errors,
        ...
        this.customRules.forEach((rule) => {
            if (rule.check && typeof rule.check === 'string') {
                ...
                const message =
                    typeof valid === 'string'
                        ? valid
                        : `Password does not meet ${rule.name} validation`;
                charactersPoolSize += this.handleRuleCheckResult(
                    valid,
                    rule,
                    message,
    ...
    checkValidity(value) {
        ...
        const blackListCheck =
            this.settings.blackList?.length ||
            this.settings.customBlacklistedWords
    ...
    handleBlackListCheckResult(result) {
        ...
        const message = `Password ${
            isRequired ? 'must' : 'should'
        } not include common words: ${blacklistedWords.join(', ')}`;
        const validation = {
            name: 'blacklist',
            required: isRequired,
        };
    ...
    attach(element) {
        ...
        const container = this.component.ce('div', {
