...
export default class Form extends Element {
    ...
    createElement(tag, attrs, children) {
        ...
        (children || []).forEach((child) => {
            element.appendChild(
                this.createElement(child.tag, child.attrs, child.children),
            );
        });
        return element;
    }
    ...
    create(display) {
        if (
            this.options &&
            (this.options.flatten || this.options.renderMode === 'flat')
        ) {
            display = 'form';
        }
            ...
            components: [
                {
                    ...
                    tag: 'div',
                    className:
                        'error error-message alert alert-danger ui red message',
                    attrs: [
                        {
                            attr: 'role',
                            value: 'alert',
    ...
    static embed(embed) {
        return new Promise((resolve) => {
            ...
            const id =
                this.id || `formio-${Math.random().toString(36).substring(7)}`;
            const className = embed.class || 'formio-form-wrapper';
            let code = embed.styles
                ? `<link rel="stylesheet" href="${embed.styles}">`
                : '';
