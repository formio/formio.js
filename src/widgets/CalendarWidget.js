  ...
  attach(input) {
    ...
    this.settings.onChange = () => {
      if (this.settings.allowInput && this.settings.enableTime) {
        this.calendar._input.value = this.settings.isManuallyOverriddenValue
          ? this.settings.manualInputValue
          : this.calendar.altInput.value;
      }
      this.emit('update');
    };
    ...
    return superAttach
      ...
      .then((ShortcutButtonsPlugin) => {
        return Formio.requireLibrary(
          'flatpickr',
          'flatpickr',
          `${Formio.cdn['flatpickr']}/flatpickr.min.js`,
          true,
        ).then((Flatpickr) => {
  ...
  initFlatpickr(Flatpickr) {
    ...
    if (isIEBrowser) {
      ...
      const originalBlurListener = this.calendar._handlers.find(
        ({ event, element }) => event === 'blur' && element === this.calendar._input,
      );
