...
export default class PDFBuilder extends WebformBuilder {
    ...
    attach(element) {
        ...
        if (!this.hasPDF) {
            ...
            if (!this.projectUrl) {
                this.setUploadError(
                    'Form options.projectUrl not set. Please set the "projectUrl" property of the options for this form or use Formio.setProjectUrl(). This setting is necessary to upload a pdf background.',
                );
            } else {
                this.setUploadError();
            }
            ...
            if (this.refs.fileBrowse && this.refs.hiddenFileInputElement) {
                this.addEventListener(
                    ...
                    (event) => {
                        ...
                        // a click event on it.
                        if (
                            typeof this.refs.hiddenFileInputElement.trigger ===
                            'function'
                        ) {
                ...
                this.addEventListener(
                    this.refs.hiddenFileInputElement,
                    'change',
                    () => {
    ...
    upload(file) {
        ...
        formio
            ...
            .then((result) => {
                ...
                if (
                    result.data.formfields?.components &&
                    result.data.formfields.components.length
                ) {
                    const formInitState =
                        this.webform.form.components[0]?.key === 'submit';
                    const wizardInitState =
                        this.webform.form.components[0]?.key === 'page1' &&
                        this.webform.form.components[0]?.components.length ===
                            0;
                    const emptyFormState =
                        this.webform.form.components.length === 0;
                ...
                _.set(this.webform.form, 'settings.pdf', {
                    ...
                    src: result.data.filesServer
                        ...
                        : `${new URL(this.projectUrl).origin}/pdf-proxy${
                              result.data.path
                          }`,
                    nonFillableConversionUsed:
                        autoConversionComponentsAssigned &&
                        result.data.formfields.nonFillableConversionUsed,
                });
        ...
        this.webform.on('iframe-elementUpdate', (schema) => {
            ...
            if (component && component.component) {
                ...
                if (!this.options.noNewEdit && !component.component.noNewEdit) {
                    this.editComponent(
                        component.component,
                        this.getParentContainer(component),
                        isNew,
                    );
                }
    ...
    onDragEnd(e) {
        ...
        const iframeRect =
            this.webform.refs.iframeContainer.getBoundingClientRect();
        ...
        if (!this.dropEvent) {
            ...
            if (
                !this.dropEmitted &&
                (getBrowserInfo().chrome || getBrowserInfo().edge) &&
        ...
        schema.overlay = {
            ...
            left: layerX
                ? layerX - this.itemOffsetX
                : e.clientX - iframeRect.left - this.itemOffsetX * 2,
            width: WIDTH,
            height: HEIGHT,
        };
        ...
        this.emit(
            ...
            this.webform,
            schema.key,
            this.webform.component.components.length,
            !this.options.noNewEdit && !schema.noNewEdit,
        );
