...
export default class WizardBuilder extends WebformBuilder {
    constructor() {
        ...
        for (const group in this.groups) {
            if (this.groups[group] && this.groups[group].components) {
                this.groups[group].componentOrder = Object.keys(
        ...
        const originalRenderComponentsHook =
            this.options.hooks.renderComponents;
        ...
        const originalAttachComponentsHook =
        ...
        this.options.hooks.attachComponents = (
            ...
            return originalAttachComponentsHook(
                element,
                components,
                container,
                component,
        ...
        this.on(
            ...
            (component, originalComponent) => {
                const webformComponents = this.webform.components.map(
                    ({ component }) => component,
                );
                if (this._form.components.includes(originalComponent)) {
                    this._form.components[
                        this._form.components.indexOf(originalComponent)
                    ] = component;
                    this.rebuild();
    ...
    removeComponent(component, parent, original) {
        ...
        if (remove && component.type === 'panel') {
            const pageIndex = this.pages.findIndex(
                (page) => page.key === component.key,
            );
            const componentIndex = this._form.components.findIndex(
    ...
    allowDrop(element) {
        return this.webform &&
            this.webform.refs &&
            this.webform.refs.webform === element
            ? false
            : true;
    }
    ...
    addPage(page) {
        const newPage =
            page && page.schema
                ? fastCloneDeep(page.schema)
                : this.getPageConfig(this.pages.length + 1);
    ...
    onReorder(element, _target, _source, sibling) {
        const isSiblingAnAddPageButton =
            sibling?.classList.contains('wizard-add-page');
        // We still can paste before Add Page button
        if (
            !element.dragInfo ||
            (sibling && !sibling.dragInfo && !isSiblingAnAddPageButton)
        ) {
            console.warn(
    ...
    pasteComponent(component) {
        ...
        if (
            this._form.components.find((comp) =>
                _.isEqual(component.component, comp),
            )
        ) {
