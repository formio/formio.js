...
export default class WebformBuilder extends Component {
    ...
    constructor() {
        ...
        if (this.options && this.options.resourceTag) {
            ...
            !this.options ||
            !this.options.hasOwnProperty('resourceTag')
        ) {
    ...
    searchFields(searchString = '') {
        ...
        const filterGroupBy = (group, searchValue = '') => {
            ...
            for (const key in components) {
                const isMatchedToTitle = components[key].title
                    .toLowerCase()
                    .match(searchValue);
    ...
    getComponentInfo(key, group) {
        ...
        if (this.schemas.hasOwnProperty(key)) {
            ...
            if (
                resourceGroup &&
                resourceGroup.components.hasOwnProperty(`component-${key}`)
            ) {
                info = fastCloneDeep(
    ...
    removeComponent(component, parent, original) {
        ...
        if (remove && index !== -1) {
            ...
            const rebuild =
                parent.formioComponent.rebuild() || NativePromise.resolve();
            rebuild.then(() => {
                this.emit(
                    'removeComponent',
                    component,
    ...
    saveComponent(component, parent, isNew, original) {
        ...
        if (index !== -1) {
            ...
            const rebuild =
                parentComponent.rebuild() || NativePromise.resolve();
            return rebuild.then(() => {
