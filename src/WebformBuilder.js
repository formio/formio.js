...
export default class WebformBuilder extends Component {
    ...
    constructor() {
        ...
        this.sideBarScrollOffset = _.get(
            this.options,
            'sideBarScrollOffset',
            0,
        );
        ...
        for (const group in this.builder) {
            if (this.builder[group]) {
                ...
                this.groups[group].components =
                    this.groups[group].components || {};
                this.groups[group].componentOrder =
                    this.groups[group].componentOrder || [];
                this.groups[group].subgroups = Object.keys(
                    this.groups[group].groups || {},
                ).map((groupKey) => {
                    this.groups[group].groups[groupKey].componentOrder =
                        Object.keys(
        ...
        this.options.hooks.renderComponent = (html, { component, self }) => {
            ...
            if (
                (this.options.disabled &&
                    this.options.disabled.includes(self.key)) ||
        ...
        this.options.hooks.attachComponents = (
            element,
            components,
            container,
            component,
            ...
            const containerElement =
                element.querySelector(
                    `[ref="${component.component.key}-container"]`,
                ) || element;
        ...
        this.options.hooks.attachDatagrid = (element, component) => {
            ...
            const dataGridContainer =
                component.refs[`${component.key}-container`];
        ...
        if (this.options && this.options.resourceTag) {
            ...
            !this.options ||
            !Object.prototype.hasOwnProperty.call(this.options, 'resourceTag')
        ) {
            query.params.tags = ['builder'];
        }
        const formio = new Formio(Formio.projectUrl);
        const isResourcesDisabled =
            this.options.builder && this.options.builder.resource === false;
        ...
        formio
            ...
            .catch((err) => {
                console.warn(
                    `Could not load project settings: ${err.message || err}`,
                );
            });

        if (!formio.noProject && !isResourcesDisabled && formio.formsUrl) {
            const resourceOptions =
                this.options.builder && this.options.builder.resource;
            ...
            sidebar: this.renderTemplate('builderSidebar', {
                ...
                groups: this.groupOrder.map((groupKey) =>
                    this.renderTemplate('builderSidebarGroup', {
                        ...
                        groupKey,
                        groupId: `builder-sidebar-${this.id}`,
                        subgroups: this.groups[groupKey].subgroups.map(
                            (group) =>
                                this.renderTemplate('builderSidebarGroup', {
                                    group,
                                    groupKey: group.key,
            ...
            if (!bootstrapVersion(this.options)) {
                ...
                this.refs['sidebar-anchor'].forEach((anchor, index) => {
                    this.addEventListener(
                        ...
                        'click',
                        () => {
                            const clickedParentId = getAttribute(
                                anchor,
                                'parent',
                            ).slice('#builder-sidebar-'.length);
                            const clickedId = getAttribute(
                                anchor,
                                'target',
                            ).slice('#group-'.length);
                            this.refs['sidebar-group'].forEach(
                                (group, groupIndex) => {
                                    const openByDefault =
                                        getAttribute(group, 'default') ===
                                        'true';
                                    const groupId = group
                                        .getAttribute('id')
                                        .slice('group-'.length);
                                    const groupParent = getAttribute(
                                        group,
                                        'parent',
                                    ).slice('#builder-sidebar-'.length);
                                    hideShow(
    ...
    searchFields(searchString = '') {
        ...
        const filterGroupBy = (group, searchValue = '') => {
            ...
            for (const key in components) {
                const isMatchedToTitle = this.t(components[key].title)
                    .toLowerCase()
                    .match(searchValue);
                const isMatchedToKey = components[key].key
                    .toLowerCase()
                    .match(searchValue);
        ...
        const filterGroupOrder = (groupOrder, searchValue) => {
            ...
            return result.filter((key) =>
                filterGroupBy(this.groups[key], searchValue),
            );
        };
        ...
        const toTemplate = (groupKey) => {
            return {
                ...
                subgroups: filterSubgroups(
                    this.groups[groupKey].subgroups,
                    searchValue,
                ).map((group) =>
                    this.renderTemplate('builderSidebarGroup', {
                        group,
            ...
            groupInfo.componentOrder = Object.keys(components)
                ...
                .filter(
                    (component) =>
                        component &&
                        !component.ignore &&
                        !component.ignoreForForm,
                )
                .sort((a, b) => a.weight - b.weight)
                .map((component) =>
                    isResource ? `component-${component.key}` : component.key,
                );
        }
    }
    ...
    initDragula() {
        ...
        this.dragula = dragula(containersArray, {
            ...
            accepts(el, target) {
                return (
                    !el.contains(target) &&
                    !target.classList.contains('no-drop')
                );
            },
        ...
        if (group && group.slice(0, group.indexOf('-')) === 'resource') {
            ...
            if (
                ...
                Object.prototype.hasOwnProperty.call(
                    resourceGroup.components,
                    `component-${key}`,
                )
            ) {
                info = fastCloneDeep(
                    resourceGroup.components[`component-${key}`].schema,
                );
            }
        }
                ...
                tableRowIndex = _.findIndex(parent.rows, (row) =>
                    row.some((column) =>
                        column.components.some(
                            (comp) => comp.key === component.key,
                        ),
                    ),
                );
                tableColumnIndex = _.findIndex(
    ...
    onDrop(element, target, source, sibling) {
        ...
        if (key && group) {
            ...
            index = _.findIndex(source.formioContainer, {
            ...
            if (index !== -1) {
                ...
                info = source.formioContainer.splice(
                    _.findIndex(source.formioContainer, {
                        key: element.formioComponent.component.key,
                    }),
                    1,
                );
        ...
        const draggableComponent =
            this.groups[group]?.components[compKey] || {};
    ...
    setForm(form) {
        ...
        this.keyboardActionsEnabled =
            _.get(this.options, 'keyboardBuilder', false) ||
            this.options.properties?.keyboardBuilder;

        const isShowSubmitButton =
            !this.options.noDefaultSubmitButton &&
            (!form.components.length ||
                !form.components.find((comp) => comp.key === 'submit'));
        ...
        if (this.webform) {
            ...
            return this.webform
                .setForm(form, { keepAsReference: true })
                .then(() => {
                    if (this.refs.form) {
    ...
    removeComponent(component, parent, original, componentInstance) {
        ...
        if (remove && index !== -1) {
            ...
            const rebuild =
                parent.formioComponent.rebuild() || Promise.resolve();
            rebuild.then(() => {
                this.emit(
                    'removeComponent',
                    component,
    ...
    updateComponent(component, changed) {
        ...
        const defaultValueComponent = getComponent(
        ...
        if (defaultValueComponent && component.type !== 'hidden') {
            const defaultChanged =
                changed &&
                ((changed.component &&
                    changed.component.key === 'defaultValue') ||
                    (changed.instance &&
    ...
    highlightInvalidComponents() {
        ...
        this.webform.everyComponent((comp) => {
            ...
            if (repeatablePaths.includes(path)) {
                ...
                comp.error?.message?.startsWith('API Key is not unique')
            ) {
        ...
        this.editForm.on('change', (event) => {
            if (event.changed) {
                    ...
                    if (isNew) {
                        if (!event.data.keyModified) {
                            this.editForm.everyComponent((component) => {
                                if (
                                    ...
                                    component.setValue(
                                        this.updateComponentKey(event.data),
                                    );
                                    return false;
                                }
                            });
                        ...
                        if (this.form) {
                            ...
                            formComponents = formComponents.filter(
                                (comp) =>
                                    editFormOptions.editComponent.id !==
                                    comp.id,
                            );
                ...
                this.updateComponent(
                    event.data.componentJson || editFormData || event.data,
                    event.changed,
                );
            }
        });
    ...
    updateComponentKey(data) {
        return _.camelCase(
            data.title || data.label || data.placeholder || data.type,
        ).replace(/^[0-9]*/, '');
    }
    ...
    moveComponent(component) {
        if (this.selectedComponent) {
            ...
            prevSelected.element?.classList.remove(
    ...
    updateComponentPlacement(direction) {
        ...
        if (component) {
            ...
            if (index === -1) {
                ...
                index = _.findIndex(source.formioComponent.components, {
                    key: key,
                });
                const firstElement =
                    source.formioComponent.components[index].element;
        ...
        if (window.sessionStorage) {
            ...
            if (data) {
                ...
                if (parent) {
                    ...
                    const isParentSaveChildMethod =
                        this.isParentSaveChildMethod(parent.formioComponent);

                    if (parent.formioContainer && !isParentSaveChildMethod) {
                        index = parent.formioContainer.indexOf(
                            component.component,
                        );
                        ...
                        parent.formioComponent.saveChildComponent(
                            schema,
                            false,
                        );
                    }
