...
import calculateValueWithManualOverrideLableValueDataGrid from '../test/forms/calculateValueWithManualOverrideLableValueDataGrid';
...
describe('Webform tests', function () {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                ...
                                errorClasses.forEach((cl) =>
                                    assert.equal(
                                        textArea.element.classList.contains(cl),
                                        false,
                                    ),
                                );
                                ...
                                setTimeout(() => {
                                    ...
                                    errorClasses.forEach((cl) =>
                                        assert.equal(
                                            number.element.classList.contains(
                                                cl,
                                            ),
                                            false,
                                        ),
                                    ...
                                    errorClasses.forEach((cl) =>
                                        assert.equal(
                                            textArea.element.classList.contains(
                                                cl,
                                            ),
                                            false,
                                        ),
    ...
    it('Should fire error and submitError events with args on attempt to submit invalid form', function (done) {
        ...
        form.setForm(formWithValidation)
            .then(() => {
                let errorEvents = 0;
                ...
                form.on('error', (arg) => {
                    assert.equal(
                        !!arg,
                        true,
                        'Error event should have argument',
                    );
                ...
                form.on('submitError', (arg) => {
                    assert.equal(
                        !!arg,
                        true,
                        'submitError event should have argument',
                    );
                    submitErrorEvents = submitErrorEvents + 1;
                });

                const clickEvent = new Event('click');
                const submitBtn = form.element.querySelector(
                    '[name="data[submit]"]',
                );
    ...
    it('Should keep non-component server errors visible after changes in the form', function (done) {
        ...
        Formio.createForm(element, form)
            .then((instance) => {
                ...
                assert.equal(
                    !!(instance.serverErrors && instance.serverErrors.length),
                    false,
                );
                assert.equal(
                    !!(instance.refs.errorRef && instance.refs.errorRef.length),
                    false,
                );

                const clickEvent = new Event('click');
                const submitBtn = instance.element.querySelector(
                    '[name="data[submit]"]',
                );
                submitBtn.dispatchEvent(clickEvent);
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        instance.refs.errorRef[0].textContent.trim(),
                        errorText,
                    );

                    const inputEvent = new Event('input');
                    const textField = instance.element.querySelector(
    ...
    it('Should set radio components value inside data grid correctly', function (done) {
        Formio.createForm(formWithRadioInsideDataGrid)
            .then((form) => {
                ...
                form.setValue({
                    data: { dataGrid: fastCloneDeep(dataGridData) },
                });
                setTimeout(() => {
    ...
    it('Should not fall into setValue calls loop when doing value calculation on server', function (done) {
        ...
        const spy = sinon.spy(
            Formio.Components.components.editgrid.prototype,
            'setValue',
        );
    ...
    it('Should fire blur and focus events for address and select components', function (done) {
        ...
        form.setForm(checkBlurFocusEventForm)
            .then(() => {
                let blurEvents = 0;
                let focusEvents = 0;
                form.on('blur', () => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        selectHtml.refs.selectContainer.dispatchEvent(
                            blurEvent,
                        );
                        ...
                        setTimeout(() => {
                            address.refs.searchInput[0].dispatchEvent(
                                blurEvent,
                            );
        ...
        Formio.createForm(formWithCheckboxRadioType)
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    form.setValue({
                        data: { radio: 'value2', checkbox: false },
                    });
    ...
    it('Should recalculate value when submission is being set in edit mode', function (done) {
        ...
        form.setForm(formWithCalcValue)
            .then(() => {
    ...
    it('Should not activate checkbox when clicking tooltip icon', function (done) {
        ...
        form.setForm(tooltipActivateCheckbox)
            .then(() => {
                const checkboxValue = form.element.querySelector(
                    '[name="data[checkbox]"]',
                ).value;
                Harness.clickElement(
                    form,
                    form.element.querySelector('[ref="tooltip"]'),
                );

                setTimeout(() => {
                    assert.equal(
                        form.element.querySelector('[name="data[checkbox]"]')
                            .value,
                        checkboxValue,
                    );
                    done();
    ...
    it('Should show submission if passed as option', function (done) {
        ...
        const form = new Webform(formElement, {
            ...
            readOnly: true,
            submission: {
                data: { survey: { question1: 'a3', question2: 'a1' } },
            },
        });

        form.setForm(formWithSurvey)
            .then(() => {
    ...
    it('Should show select boxes values in html render mode', function (done) {
        ...
        form.setForm(formWithSelectBoxes)
            .then(() => {
                form.setSubmission({
                    data: { selectBoxes: { a: true, b: true, c: false } },
                }).then(() => {
                    const selectBoxes = form.getComponent('selectBoxes');
                    const values = selectBoxes.element
                        .querySelector('[ref="value"]')
                        .textContent.trim();
    ...
    it('Should allow to input value and add rows in deeply nested conditional dataGrid', function (done) {
        ...
        form.setForm(deeplyNestedDataGridAndContainer)
            .then(() => {
                ...
                setTimeout(() => {
                    assert.equal(parentDataGrid.rows.length, 1);
                    assert.equal(
                        parentDataGrid.rows[0].dataGrid5.visible,
                        true,
                    );

                    const numberInput =
                        parentDataGrid.rows[0].dataGrid5.rows[0].number.refs
                            .input[0];
                    ...
                    setTimeout(() => {
                        const conditionalDataGrid =
                            form.getComponent('dataGrid6').rows[0].dataGrid5;
                        ...
                        const addRowBtn =
                            conditionalDataGrid.refs[
                                `${'datagrid-dataGrid5-addRow'}`
                            ][0];
                        const clickEvent = new Event('click');
    ...
    it('Should adjust columns when conditional fields appear/disappear', function (done) {
        ...
        form.setForm(columnWithConditionalComponents)
            .then(() => {
                const selectBoxes = form.getComponent('selectBoxes');
                ...
                setTimeout(() => {
                    ...
                    const visibleTextField1 =
                        columns.columns[2][0].refs.input[0];
                    const visibleTextField2 =
                        columns.columns[5][0].refs.input[0];
                        ...
                        setTimeout(() => {
                            columns.columns.forEach((column, index) => {
                                if ([6].includes(index + 1)) {
                                    ...
                                    assert.equal(
                                        columns.component.columns[index]
                                            .currentWidth,
                                        2,
                                        `Column ${index + 1}  width should be 2`,
                                    );
    ...
    it('Should not translate en value if _userInput option is provided and value presents in reserved translation names', function (done) {
        ...
        form.setForm(translationTestForm)
            .then(() => {
                setTimeout(() => {
                    const selectComp = form.getComponent('select');
                    const options =
                        selectComp.element.querySelector(
                            '[role="listbox"]',
                        ).children;
                    const option1 = options[0].textContent.trim();
                    const option2 = options[1].textContent.trim();
                    const label = selectComp.element
                        .querySelector('label')
                        .textContent.trim();
                    assert.equal(
                        option1,
                        translationTestForm.components[0].data.values[0].label,
                    );
                    assert.equal(
                        option2,
                        translationTestForm.components[0].data.values[1].label,
                    );
                    assert.equal(
                        label,
                        translationTestForm.components[0].label,
                    );
            ...
            .then(() => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            formWithDG.components[0].components.length,
                            1,
                        );
        ...
        formWithDate
            ...
            .then(() => {
                setTimeout(() => {
                    ...
                    const dateAltFormat =
                        dateCompInputWidget.calendar.config.altFormat;
                    dateCompInputWidget.calendar.setDate(
                        '30-05-2020',
                        true,
                        dateAltFormat,
                    );
                    ...
                    const dateCompInputWidget1 = formWithDate.element
                        ...
                        .querySelector('.flatpickr-input').widget;
                    const dateAltFormat1 =
                        dateCompInputWidget1.calendar.config.altFormat;
                    dateCompInputWidget1.calendar.setDate(
                        '30-05-2020',
                        true,
                        dateAltFormat1,
                    );
                    ...
                    const dateCompInputWidget2 = formWithDate.element
                        ...
                        .querySelector('.flatpickr-input').widget;
                    const dateAltFormat2 =
                        dateCompInputWidget2.calendar.config.altFormat;
                    dateCompInputWidget2.calendar.setDate(
                        '30-05-2020',
                        true,
                        dateAltFormat2,
                    );
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            dateCompAltInput1.value,
                            '2020-05-30T00:00:00',
                        );
                        assert.equal(dateComp1.value, '30-05-2020');

                        assert.equal(
                            dateCompAltInput2.value,
                            '2020-05-30T00:00:00',
                        );
                        assert.equal(dateComp2.value, '30-05-2020');

                        const addNewRowBtn = formWithDate.element.querySelector(
                        ...
                        setTimeout(() => {
                            const dataGridRows =
                                formWithDate.element.querySelectorAll(
                                    '[ref="datagrid-dataGrid-row"]',
                                );
                            assert.equal(dataGridRows.length, 2);

                            const dateCompAltInputAfterAddingRow =
                                formWithDate.element
                                    .querySelectorAll(
                            ...
                            const dateCompAltInputAfterAddingRow1 =
                                formWithDate.element
                                    .querySelectorAll(
                            ...
                            const dateCompAltInputAfterAddingRow2 =
                                formWithDate.element
                                    .querySelectorAll(
                                        '.formio-component-textField2',
                                    )[0]
                                    .querySelector('.flatpickr-input');
                            ...
                            assert.equal(
                                dateCompAltInputAfterAddingRow.value,
                                '30-05-2020',
                            );
                            assert.equal(
                                dateCompAfterAddingRow.value,
                                '30-05-2020',
                            );

                            assert.equal(
                                dateCompAltInputAfterAddingRow1.value,
                                '2020-05-30T00:00:00',
                            );
                            assert.equal(
                                dateCompAfterAddingRow1.value,
                                '30-05-2020',
                            );

                            assert.equal(
                                dateCompAltInputAfterAddingRow2.value,
                                '2020-05-30T00:00:00',
                            );
                            assert.equal(
                                dateCompAfterAddingRow2.value,
                                '30-05-2020',
                            );
    ...
    it('Should open collapsed panel with invalid components inside container that is inside the panel on submit', function (done) {
        ...
        formWithPanel
            ...
            .then(() => {
                ...
                const submitBtn = formWithPanel.element.querySelector(
                    '[name="data[submit]"]',
                );
                submitBtn.dispatchEvent(clickEvent);
        ...
        formWithDate
            ...
            .then(() => {
                ...
                const dateTimeCompInputWidget = formWithDate.element
                    ...
                    .querySelector('.flatpickr-input').widget;
                const dateTimeAltFormat =
                    dateTimeCompInputWidget.calendar.config.altFormat;
                dateTimeCompInputWidget.calendar.setDate(
                    '05-05-2020T00:00:00',
                    true,
                    dateTimeAltFormat,
                );
                ...
                const textFieldDateCompWidget = formWithDate.element
                    ...
                    .querySelector('.flatpickr-input').widget;
                const textFieldDateAltFormat =
                    textFieldDateCompWidget.calendar.config.altFormat;
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        dateTimeCompAltInput.value,
                        '2020-05-05T00:00:00',
                    );
                    assert.equal(
                        textFieldDateCompAltInput.value,
                        '2020-04-04T00:00:00',
                    );
                    ...
                    const panelCollapseBtn = formWithDate.element.querySelector(
                    ...
                    setTimeout(() => {
                        const panelBody =
                            formWithDate.element.querySelector('.panel-body');
                        assert.equal(!!panelBody, false);

                        formWithDate.element
                            .querySelector('.formio-collapse-icon')
                            .dispatchEvent(clickEvent);

                        setTimeout(() => {
                            const dateTimeCompAfterOpenningPanel =
                                formWithDate.element
                                    .querySelector(
                                        '.formio-component-dateTime1',
                                    )
                                    .querySelector('[type="text"]');
                            const textFieldDateCompAfterOpenningPanel =
                                formWithDate.element
                                    .querySelector(
                                        '.formio-component-textField1',
                                    )
                                    .querySelector('[type="text"]');

                            const dateTimeCompAltInputAfterOpenningPanel =
                                formWithDate.element
                                    .querySelector(
                                        '.formio-component-dateTime1',
                                    )
                                    .querySelector('.flatpickr-input');
                            const textFieldDateCompAltInputAfterOpenningPanel =
                                formWithDate.element
                                    .querySelector(
                                        '.formio-component-textField1',
                                    )
                            ...
                            assert.equal(
                                dateTimeCompAltInputAfterOpenningPanel.value,
                                '2020-05-05T00:00:00',
                            );
                            assert.equal(
                            ...
                            assert.equal(
                                dateTimeCompAfterOpenningPanel.value,
                                '05-05-2020',
                            );
                            assert.equal(
                                textFieldDateCompAfterOpenningPanel.value,
                                '04-04-2020',
                            );
                            done();
                        }, 250);
  ...
  editGrid modal draft row`, function (done) {
        ...
        formWithNestedDraftModals
            ...
            .then(() => {
                const editGrid =
                    formWithNestedDraftModals.getComponent('editGrid');
                const clickEvent = new Event('click');
                const inputEvent = new Event('input');

                const addRowBtn =
                    formWithNestedDraftModals.element.querySelector(
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        const saveModalBtn =
                            rowModal.querySelector('.btn-primary');
                        //clicking save button to save row draft
                        saveModalBtn.dispatchEvent(clickEvent);

                        setTimeout(() => {
                            const editGridRows =
                                formWithNestedDraftModals.element.querySelectorAll(
                            ...
                            const editRowBtn =
                                editGridRows[0].querySelector('.editRow');
                            //click the edit btn to open the row again
                            editRowBtn.dispatchEvent(clickEvent);

                            setTimeout(() => {
                                const rowModalForEditing =
                                    document.querySelector(
                                        `.editgrid-row-modal-${editGrid.id}`,
                                    );
                                const textFieldInputForEditing =
                                    rowModalForEditing.querySelector(
                                        '[name="data[textField]"]',
                                    );
                                textFieldInputForEditing.value =
                                    'changed value';
                                //changing textfield value
                                textFieldInputForEditing.dispatchEvent(
                                    inputEvent,
                                );
                                ...
                                setTimeout(() => {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        setTimeout(() => {
                                            ...
                                            setTimeout(() => {
                                                const confirmationDialogsAfterClickingOverlay =
                                                    document.querySelectorAll(
                                                        `.editgrid-row-modal-confirmation-${editGrid.id}`,
                                                    );
                                                assert.equal(
                                                    confirmationDialogsAfterClickingOverlay.length,
                                                    1,
                                                );
        ...
        formWithDraftModals
            ...
            .then(() => {
                ...
                setTimeout(() => {
                    ...
                    const textFieldInput = rowModal.querySelector(
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            const editGridRows =
                                formWithDraftModals.element.querySelectorAll(
                                    '[ref="editgrid-editGrid-row"]',
                                );
                            //checking if the editGrid row was created
                            assert.equal(editGridRows.length, 1);
        ...
        formWithDataGridInitEmptyOption
            ...
            .then(() => {
                formWithDataGridInitEmptyOption.setSubmission(
                    formWithDataGridInitEmpty.submission2,
                );

                setTimeout(() => {
                    const dataGridRows =
                        formWithDataGridInitEmptyOption.element.querySelectorAll(
                            '[ref = "datagrid-dataGrid-row"]',
                        );
                    const dataGrid1Rows =
                        formWithDataGridInitEmptyOption.element.querySelectorAll(
                    ...
                    formWithDataGridInitEmptyOption.setSubmission(
                        formWithDataGridInitEmpty.submission3,
                    );
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            dataGridSecondRowComponentValue.value,
                            'test2',
                        );
                        assert.equal(
                            dataGrid1FirstRowComponentValue.textContent,
                            'test3',
                        );
                        assert.equal(
                            dataGrid1SecondRowComponentValue.value,
                            222,
                        );
    ...
    it('Should not show dataGrid rows when empty submission is set for dataGrid with initEmpty', function (done) {
        ...
        formWithDataGridInitEmptyOption
            ...
            .then(() => {
                formWithDataGridInitEmptyOption.setSubmission(
                    formWithDataGridInitEmpty.submission1,
                );
        ...
        formWithAddress
            ...
            .then(() => {
                formWithAddress.setSubmission({
                    data: formWithAddressComponent.submission,
                });
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        addressInput.value,
                        formWithAddressComponent.submission.dataGrid[0].address[
                            'formatted_address'
                        ],
                    );
        ...
        formWithBlurValidation
            ...
            .then(() => {
                ...
                'test'.split('').forEach((character) => {
                    fieldWithBlurValidation.value =
                        fieldWithBlurValidation.value + character;
                    fieldWithBlurValidation.dispatchEvent(inputEvent);
                });

                setTimeout(() => {
                    const validationErrorBeforeBlur =
                        formWithBlurValidation.element.querySelector('.error');
                    ...
                    setTimeout(() => {
                        const validationErrorAfterBlur =
                            formWithBlurValidation.element.querySelector(
                                '.error',
                            );
    ...
    it('Should submit form with empty time field when time field is not required', function (done) {
        ...
        formWithTime
            ...
            .then(() => {
                const clickEvent = new Event('click');
                const submitBtn = formWithTime.element.querySelector(
                    '[name="data[submit]"]',
                );
  ...
  in draft modal mode after pushing submit btn`, function (done) {
        ...
        formWithDraftModals
            ...
            .then(() => {
                ...
                setTimeout(() => {
                    const editGrid =
                        formWithDraftModals.getComponent('editGrid');
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            const submitBtn =
                                formWithDraftModals.element.querySelector(
                                    '[name="data[submit]"]',
                                );
                            ...
                            setTimeout(() => {
                                ...
                                assert.equal(
                                    formWithDraftModals.errors.length,
                                    2,
                                );
                                ...
                                assert.equal(
                                    rowError,
                                    'Invalid row. Please correct it or delete.',
                                );
                                assert.equal(
                                    editGridError,
                                    'Please correct invalid rows before proceeding.',
                                );
                                ...
                                setTimeout(() => {
                                    const rowModalAfterValidation =
                                        editGrid.editRows[0].dialog;

                                    const alertWithErrorText =
                                        rowModalAfterValidation.querySelector(
                                            '.alert-danger',
                                        );
                                    ...
                                    const alertErrorMessages =
                                        rowModalAfterValidation.querySelectorAll(
                                            '[ref="messageRef"]',
                                        );
                                    assert.equal(alertErrorMessages.length, 1);

                                    const numberComponentError =
                                        rowModalAfterValidation
                                            .querySelector(
                                    ...
                                    setTimeout(() => {
                                        const rowModalWithValidFields =
                                            document.querySelector(
                                                `.editgrid-row-modal-${editGrid.id}`,
                                            );
                                        const alertErrorMessagesAfterInputtingValidValues =
                                            rowModalWithValidFields.querySelectorAll(
                                                '[ref="messageRef"]',
                                            );
                                        assert.equal(
                                            alertErrorMessagesAfterInputtingValidValues.length,
                                            0,
                                        );

                                        //input values to make all row fields invalid
                                        const validNumberInput =
                                            rowModalWithValidFields.querySelector(
                                                '[name="data[editGrid][0][number]"]',
                                            );
                                        validNumberInput.value = null;
                                        validNumberInput.dispatchEvent(
                                            inputEvent,
                                        );
                                        ...
                                        setTimeout(() => {
                                            ...
                                            assert.equal(
                                                alertErrorMessagesAfterInputtingInvalidValues.length,
                                                2,
                                            );
                                            document.body.innerHTML = '';
    ...
    it('Should not override calculated value', function (done) {
        ...
        formWithCalculatedAmount
            ...
            .then(() => {
                ...
                const amountInput1 =
                    formWithCalculatedAmount.element.querySelector(
                        '[name="data[amount1]"]',
                    );
                const amountInput2 =
                    formWithCalculatedAmount.element.querySelector(
    ...
    it('Should modify calculated value only if it was not manually modified when allowCalculateOverride is true', function (done) {
        ...
        form.setForm(formWithAllowCalculateOverride)
            .then(() => {
  ...
  components with the same key`, function (done) {
        ...
        formWithCondition
            ...
            .then(() => {
                ...
                setTimeout(() => {
                    const conditionalFieldInContainer1 =
                        formWithCondition.element.querySelector(
                            '[name="data[container1][textField]"]',
                        );
                    const conditionalFieldInContainer2 =
                        formWithCondition.element.querySelector(
    ...
    it('Should show only "required field" error when submitting empty required field with pattern validation', function (done) {
        ...
        formWithPattern
            ...
            .then(() => {
                Harness.clickElement(
                    formWithPattern,
                    formWithPattern.element.querySelector(
                        '[name="data[submit]"]',
                    ),
                );
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        formWithPattern.errors[0].messages[0].message,
                        'Text Field is required',
                    );
                    assert.equal(
                        formWithPattern.element
                            .querySelector('[ref="errorRef"]')
                            .textContent.trim(),
                        'Text Field is required',
                    );
                    done();
    ...
    it('Should only scroll to alerts dialog when submitting an invalid form', function (done) {
        ...
        form.setForm(formJson)
            .then(() => {
                Harness.clickElement(
                    form,
                    form.element.querySelector('[name="data[submit]"]'),
                );
        ...
        formWithCalculatedValue
            ...
            .then(() => {
                ...
                setTimeout(() => {
                    const inputEvent = new Event('input');
                    const input1 =
                        formWithCalculatedValue.components[0].refs.input[0];

                    input1.value = 55;
                    input1.dispatchEvent(inputEvent);

                    setTimeout(() => {
                        const input2 = formElement.querySelector(
                            'input[name="data[number2]"]',
                        );
    ...
    it('Should calculate the value when editing set values with possibility of manual override', function (done) {
        ...
        formWithCalculatedValue.setForm(manualOverride).then(() => {
            formWithCalculatedValue
                ...
                .then(() => {
                    setTimeout(() => {
                        const input1 = formElement.querySelector(
                            'input[name="data[number1]"]',
                        );
                        const input2 = formElement.querySelector(
                            'input[name="data[number2]"]',
                        );
    ...
    it('Should keep components valid if they are pristine', function (done) {
        ...
        form.setForm(settingErrors)
            .then(() => {
                const inputEvent = new Event('input', {
                    bubbles: true,
                    cancelable: true,
                });
                const input = form.element.querySelector(
    ...
    it('Should delete value of hidden component if clearOnHide is turned on', function (done) {
        ...
        form.setForm(clearOnHide).then(() => {
            ...
            const textField = form.element.querySelector(
                'input[name="data[clearOnHideField]"]',
            );
    ...
    const checkForErrors = function (
        ...
        form.setSubmission(submission, flags)
            .then(() => {
                setTimeout(() => {
                    const errors = formElement.querySelectorAll(
    ...
    it('Should set calculated value correctly', function (done) {
        formElement.innerHTML = '';
        const form = new Webform(formElement);
        form.setForm(calculateZeroValue)
            .then(() => {
    ...
    it('Should render Nested Modal Wizard Form correctly', function (done) {
        formElement.innerHTML = '';
        const form = new Webform(formElement);
        form.setForm(nestedModalWizard)
            .then(() => {
                const openModalRef =
                    form.element.querySelector('[ref="openModal"]');
                ...
                setTimeout(() => {
                    ...
                    assert(
                        openModalRef,
                        'Should render Open Modal button after the page was changed',
                    );
                    done();
                }, 250);
            })
    ...
    it('Should set calculated value correctly, given a file component', function (done) {
        formElement.innerHTML = '';
        const form = new Webform(formElement);
        form.setForm(disableSubmitButton)
            .then(() => {
                const textField = form.getComponent(['textField']);
                ...
                assert.equal(
                    ...
                    false,
                    'Button should be enabled at the beginning',
                );
                ...
                const simulateFileUploading = (comp, debounce = 250) => {
                    ...
                    filePromise.then(() =>
                        comp.emit('fileUploadingEnd', filePromise),
                    );
    ...
    describe('Validate onBlur', function () {
        ...
        it('Should keep components inside DataGrid valid onChange', function (done) {
            formElement.innerHTML = '';
            const form = new Webform(formElement, { language: 'en' });
            form.setForm(dataGridOnBlurValidation)
                .then(() => {
                    ...
                    setTimeout(() => {
                        const textField =
                            component.iteratableRows[0].components.textField;
                        assert.equal(
                            !!textField.error,
                            false,
                            'Should stay valid on input',
                        );
                        ...
                        setTimeout(() => {
                            assert(
                                textField.error,
                                'Should be validated after blur',
                            );
                            done();
                        }, 250);
                    }, 250);
    ...
    describe('New Simple Conditions', function () {
        it('Should show field if all conditions are met', function (done) {
            ...
            form.setForm(formsWithNewSimpleConditions.form1)
                .then(() => {
                    const conditionalComponent =
                        form.getComponent('conditionalField');
                    assert.equal(
        ...
        it('Should show field if any condition is met', function (done) {
            ...
            form.setForm(formCopy)
                .then(() => {
                    ...
                    form.setValue({
                        data: { number: 1100, email: 'test@form.io' },
                    });
                    ...
                    setTimeout(() => {
                        ...
                        form.setValue({
                            data: { number: 10, email: 'test@form1.io' },
                        });
        ...
        it('Should show field if all conditions are met (test with different component types + multiple components)', function (done) {
            ...
            form.setForm(formsWithNewSimpleConditions.form2)
                .then(() => {
                    const conditionalComponent =
                        form.getComponent('conditionalField');
                    assert.equal(
        ...
        it('Should show/hide field inside datagrid rows', function (done) {
            ...
            form.setForm(formsWithNewSimpleConditions.form4)
                .then(() => {
                    const dataGrid = form.getComponent('dataGrid');
                    ...
                    setTimeout(() => {
                        ...
                        _.each(dataGrid.rows, (row, index) => {
                            assert.equal(
                                ...
                                `Component should be conditionally ${
                                    expectedValues[`${index}`]
                                        ? 'visible'
                                        : 'hidden'
                                } in row ${index}`,
        ...
        it('Should set component value through logic triggered by simple condition', function (done) {
            ...
            form.setForm(formsWithNewSimpleConditions.form3)
                .then(() => {
                    const componentWithLogic =
                        form.getComponent('fieldWithLogic');
                    assert.equal(
                        componentWithLogic.isEmpty(),
                        true,
        ...
        it('Should show field if all conditions are met (test all operators)', function (done) {
            ...
            form.setForm(formsWithNewSimpleConditions.form5)
                .then(() => {
                    const conditionalComponent =
                        form.getComponent('conditionalField');
                    assert.equal(
        ...
        it('Should show the field on select boxes value', function (done) {
            ...
            form.setForm(formsWithNewSimpleConditions.form6)
                .then(() => {
        ...
        it('Should show field when condition is based on the values of select resource with object value', function (done) {
            ...
            Formio.createForm(element, formWithObjectValueSelect)
                .then((form) => {
                    ...
                    const selectNoValuePropertyMult = form.getComponent(
                        'selectNoValueProperty',
                    );
                    selectNoValuePropertyMult.setValue([
                        fastCloneDeep(values[2]),
                    ]);
                    const selectEntireObject =
                        form.getComponent('selectEntireObject');
                    selectEntireObject.setValue(fastCloneDeep(values[1].data));
                    const selectEntireObjectMult = form.getComponent(
                        'selectEntireObjectMult',
                    );
    ...
    describe('Calculate Value with allowed manual override', function () {
        ...
        it('Should reset all values correctly.', function (done) {
            const formElement = document.createElement('div');
            const form = new Webform(formElement, { language: 'en' });
            form.setForm(calculateValueWithManualOverride)
                .then(() => {
                    const dataGrid = form.getComponent('dataGrid');
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            setTimeout(() => {
                                expect(form.submission).to.deep.equal(
                                    submissionWithOverridenValues2,
                                );
                                form.setSubmission(
                                    submissionWithOverridenValues,
                                ).then(() => {
        ...
        it('Should apply submission metadata value in calculation.', function (done) {
            const formElement = document.createElement('div');
            const form = new Webform(formElement, { language: 'en' });
            form.setForm(calculateValueWithSubmissionMetadata)
                .then(() => {
                    const textField = form.getComponent('textField');
                    ...
                    setTimeout(() => {
                        ...
                        expect(form.submission.metadata.timezone).to.be.not
                            .empty;
                        ...
                        expect(form.submission.data.textArea).to.equal(
                            form.submission.data.textField +
                                form.submission.metadata.timezone,
                        );
        ...
        it('Should allow to change value.', function (done) {
            const formElement = document.createElement('div');
            const form = new Webform(formElement, { language: 'en' });
            form.setForm(calculatedSelectboxes)
                .then(() => {
            ...
            Formio.createForm(formElement, formsWithAllowOverride.withResetBtn)
                .then((form) => {
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            textField.dataValue,
                            calculatedValues.textField,
                        );
                        assert.equal(
                            textArea.dataValue,
                            calculatedValues.textArea,
                        );
                        ...
                        setTimeout(() => {
                            ...
                            assert.equal(
                                number.getValue(),
                                overridenValues.number,
                            );
                            assert.equal(
                                textField.dataValue,
                                overridenValues.textField,
                            );
                            assert.equal(
                                textArea.dataValue,
                                overridenValues.textArea,
                            );
                            assert.equal(
                                radio.dataValue,
                                overridenValues.radio,
                            );
        ...
        it('Should recalculate values for conditional components with "allow override" and "clear on hide" enabled when components become visible again', function (done) {
            ...
            Formio.createForm(
                formElement,
                formsWithAllowOverride.withClearOnHide,
            )
                .then((form) => {
                    const calculatedValues = {
                        number: 111,
    ...
    describe('Modal Edit', function () {
        ...
        it('Test rendering previews after the submission is set', function (done) {
            const formElement = document.createElement('div');
            const form = new Webform(formElement, { language: 'en' });
            form.setForm(modalEditComponents)
                .then(() => {
                    return form
                        .setSubmission(submission, { fromSubmission: true })
                        .then(() => {
                            componentsKeys.forEach((key) => {
                                const comp = form.getComponent([key]);
                                assert(comp);
                                const preview =
                                    comp.componentModal.refs.openModal;
                                assert(preview);
                                assert.equal(
                                    preview.textContent
                                        .replace(/\n|\t/g, '')
                    ...
                    return form
                        ...
                        .then(() => {
                            ...
                            setTimeout(() => {
                                ...
                                comp.componentModal.closeModalHandler(
                                    fakeEvent,
                                );
                                const preview =
                                    comp.componentModal.refs.openModal;
    ...
    describe('Initially Collapsed Panel', function () {
        const formElement = document.createElement('div');
        const form = new Webform(formElement, { language: 'en' });
        form.setForm(initiallyCollapsedPanel)
            .then(() => {
                it('Should be collapsed', function (done) {
                    try {
                        const panelBody =
                            form.element.querySelector('[ref=nested-panel]');
                        assert.equal(
                            panelBody,
                            null,
                ...
                it('Should open when an Error occured', function (done) {
                    form.executeSubmit().catch(() => {
                        try {
                            ...
                            assert(
                                panelBody,
                                'Should open the panel when an error occured',
                            );
                            done();
                        } catch (err) {
                            done(err);
    ...
    describe('Calculate Value', function () {
        it('Should calculate value when set submission if the component is not persistent', function (done) {
            ...
            form.setForm(calculatedNotPersistentValue)
                .then(() => {
                    form.setSubmission({
                        data: {
                            a: 'testValue',
                    ...
                    setTimeout(() => {
                        ...
                        const notPersistentFieldInput =
                            form.element.querySelector(
                                'input[name="data[textField]"]',
                            );
                        assert.equal(
                            notPersistentFieldInput.value,
                            'testValue',
        ...
        it('Should calculate value by datasouce component when editing mode is on', function (done) {
            ...
            form.setForm(calculateValueInEditingMode)
                .then(() => {
                    form.editing = true;
                    form.setSubmission({
                        data: {
                        ...
                        setTimeout(() => {
                            const dataSourceDisplay =
                                form.getComponent('dataSourceDisplay');
                            assert.equal(
        ...
        it('Should not override value which was set from submission', function (done) {
            ...
            form.setForm(calculateValueWithManualOverrideLableValueDataGrid)
                .then(() => {
                    form.editing = true;
                    form.setSubmission({
                        state: 'submitted',
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            value1.dataValue,
                            '1a',
                            'Should have a value set from submission',
                        );
    ...
    it('Should set different ids for components inside different Table rows', function (done) {
        ...
        form.setForm(conditionalDataGridWithTableAndRadio)
            .then(() => {
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        radioInspection0.dataValue,
                        'reject',
                        'Should set value',
                    );
                    assert.equal(
                        repairDataGrid0.visible,
                        true,
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            assert.equal(
                                inspectionDataGrid.rows.length,
                                2,
                                'Should add a row',
                            );
                            ...
                            setTimeout(() => {
                                ...
                                assert.equal(
                                    radioInspection1.dataValue,
                                    'reject',
                                    'Should set value',
                                );
                                assert.equal(
                                    repairDataGrid1.visible,
                                    true,
                                    'Should become visible',
                                );
    ...
    it('Should render components properly', function (done) {
        const formElement = document.createElement('div');
        const form = new Webform(formElement, { language: 'en' });
        form.setForm(multipleTextareaInsideConditionalComponent)
            .then(() => {
                form.setSubmission({
                    data: {
                        textArea2: ['test'],
                ...
                setTimeout(() => {
                    ...
                    const inputRows =
                        textarea.element.querySelectorAll('[ref="input"]');
                    assert.equal(
                        inputRows.length,
                        1,
                        'Should render all the rows of the Textarea',
                    );
                    done();
                }, 750);
            })
    ...
    it('Should disable all the components inside Nested Form if it is disabled', function (done) {
        const formElement = document.createElement('div');
        const form = new Webform(formElement, { language: 'en' });
        form.setForm(disabledNestedForm)
            .then(() => {
                assert.equal(
    ...
    it('Should restore value correctly if NestedForm is saved as reference', function (done) {
        const formElement = document.createElement('div');
        const form = new Webform(formElement, { language: 'en' });
        form.setForm(nestedFormInsideDataGrid)
            .then(() => {
    ...
    it('Should add and clear input error classes correctly', function (done) {
        ...
        form.setForm(UpdateErrorClassesWidgets)
            .then(() => {
                const checkbox = form.getComponent('showDate');
                checkbox.setValue(true);
                setTimeout(() => {
                    const dateTimeComponent =
                        form.getComponent('condtionalDate');
                    const dateComponentElement = dateTimeComponent.element;
                    assert.equal(
                        !dateComponentElement.className.includes(
                            'formio-hidden',
                        ),
                        true,
                    ...
                    setTimeout(() => {
                        const dateVisibleInput =
                            dateComponentElement.querySelector('.input');
                        const flatpickerInput =
                            dateComponentElement.querySelector(
                                '.flatpickr-input',
                            );
                        ...
                        setTimeout(() => {
                            ...
                            assert(
                                !dateVisibleInput.className.includes(
                                    'is-invalid',
                                ),
                                'Invalid class should be removed',
                            );
                            assert(
                                !flatpickerInput.className.includes(
                                    'is-invalid',
                                ),
                            ...
                            setTimeout(() => {
                                const dateComponentElement =
                                    dateTimeComponent.element;
                                assert.equal(
                                    dateComponentElement.className.includes(
                                        'formio-hidden',
                                    ),
    ...
    it('Test Truncate Multiple Spaces', function (done) {
        ...
        form.setForm(truncateMultipleSpaces)
            .then(() => {
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    const customerSelectEl = form.element.querySelector(
                        '.formio-component-page2Customer',
                    );
                    const customerSelectValueEl =
                        customerSelectEl.querySelector('[ref="value"]');
                    const htmlSelectEl = form.element.querySelector(
                        '.formio-component-panelHtml5Select',
                    );
                    const htmlSelectValueEl =
                        htmlSelectEl.querySelector('[ref="value"]');
                    const checkboxEl = form.element.querySelector(
                        '.formio-component-page3Iagreetothefollowtherules',
                    );
                    const checkboxValueEl =
                        checkboxEl.querySelector('[ref="value"]');
    ...
    it('Should execute clearOnHide if visibility of the component inside an EditGrid has changed', function (done) {
        ...
        form.setForm(testClearOnHideInsideEditGrid)
            .then(() => {
                form.submission = {
                    state: 'submitted',
                    data: {
    ...
    it('Should show values in editGrid rows with nested dataGrid when viewing submission with initEmpty option', function (done) {
        ...
        formWithNestedDataGridInitEmptyOption
            ...
            .then(() => {
                ...
                setTimeout(() => {
                    const nestedDataGridFirstRowComponentValue =
                        formWithNestedDataGridInitEmptyOption.element
                            .querySelector('[ref="editgrid-editGrid-row"]')
                            .querySelectorAll('.col-sm-2');

                    assert.equal(
                        nestedDataGridFirstRowComponentValue[1].textContent.trim(),
                        'email',
                    );
                    assert.equal(
                        nestedDataGridFirstRowComponentValue[2].textContent.trim(),
                        'hhh@gmail.com',
                    );
        ...
        form.setForm(formJson)
            .then(() => {
                ...
                setTimeout(() => {
                    assert.equal(form.errors[0].messages.length, 1);
                    assert.equal(
                        form.errors[0].messages[0].message,
                        'will be showed once',
                    );
                    assert.equal(
                        form.element
    ...
    describe('Test sanitizeConfig', function () {
        it('Should sanitize components using default sanitizeConfig', function (done) {
            ...
            form.setForm(testForm)
                .then(() => {
                    const textFieldWithScript = form.getComponent(
                        'textFieldWithScript',
                    );
                    const textAreaWithIframe =
                        form.getComponent('textAreaWithIframe');

                    assert.equal(
                        textFieldWithScript.element?.getElementsByTagName(
                            'script',
                        ).length,
                        0,
                    ...
                    assert.equal(
                        textAreaWithIframe.element?.getElementsByTagName(
                            'iframe',
                        ).length,
                        0,
            ...
            Formio.createForm(
                ...
                .then((form) => {
                    setTimeout(() => {
                        ...
                        assert.equal(form.submission.state, 'draft');
                        const tfInput =
                            form.getComponent('textField').refs.input[0];
                ...
                for (const title in formTest.tests) {
                    ...
                    it(title, function (done) {
                        ...
                        let form = new Webform(
                            formElement,
                            _.cloneDeep(formTest.formOptions || {}),
                        );
