...
describe('Webform tests', function () {
	...
	it('Should not lose values of conditionally visible components on setValue when server option is passed', function (done) {
		...
		Formio.createForm(formElement, formWithDeeplyNestedConditionalComps, {
			...
			.then((form) => {
				...
				form.setValue(fastCloneDeep(submission), {
					sanitize: true,
				});
				setTimeout(() => {
	...
	it('Should set radio components value inside data grid correctly', function (done) {
		Formio.createForm(formWithRadioInsideDataGrid)
			.then((form) => {
				...
				form.setValue({
					data: {
						dataGrid: fastCloneDeep(dataGridData),
					},
				});
				setTimeout(() => {
	...
	it('Should not activate checkbox when clicking tooltip icon', function (done) {
		...
		form
			...
			.then(() => {
				...
				setTimeout(() => {
					assert.equal(
						form.element.querySelector('[name="data[checkbox]"]').value,
						checkboxValue,
					);
					done();
				}, 200);
		...
		form
			...
			.then(() => {
				...
				setTimeout(() => {
					...
					const numberInput =
						parentDataGrid.rows[0].dataGrid5.rows[0].number.refs.input[0];
					...
					setTimeout(() => {
						...
						const addRowBtn =
							conditionalDataGrid.refs[`${'datagrid-dataGrid5-addRow'}`][0];
						const clickEvent = new Event('click');
		...
		form
			...
			.then(() => {
				setTimeout(() => {
					...
					const options =
						selectComp.element.querySelector('[role="listbox"]').children;
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								...
								const textFieldInputForEditing =
									rowModalForEditing.querySelector('[name="data[textField]"]');
								...
								setTimeout(() => {
									...
									const XCloseBtn = rowModalForEditing.querySelector(
										'[ref="dialogClose"]',
									);
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						setTimeout(() => {
							...
							const submitBtn = formWithDraftModals.element.querySelector(
								'[name="data[submit]"]',
							);
							...
							setTimeout(() => {
								...
								setTimeout(() => {
									...
									const alertWithErrorText =
										rowModalAfterValidation.querySelector('.alert-danger');
	...
	it('Should not override calculated value', function (done) {
		...
		formWithCalculatedAmount
			...
			.then(() => {
				...
				const amountInput1 = formWithCalculatedAmount.element.querySelector(
					'[name="data[amount1]"]',
				);
				const amountInput2 = formWithCalculatedAmount.element.querySelector(
					'[name="data[amount2]"]',
				);
  ...
  components with the same key`, function (done) {
		...
		try {
			form
				...
				.then(() => {
					...
					expect(
						formElement.querySelector('.col-form-label').innerHTML.trim(),
						...
						_.each(dataGrid.rows, (row, index) => {
							assert.equal(
								...
								`Component should be conditionally ${
									expectedValues[`${index}`] ? 'visible' : 'hidden'
								} in row ${index}`,
							);
						});
						done();
	...
	describe('Initially Collapsed Panel', function () {
		...
		const form = new Webform(formElement, { language: 'en' });
		form
			.setForm(initiallyCollapsedPanel)
			.then(() => {
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						const flatpickerInput =
							dateComponentElement.querySelector('.flatpickr-input');
	...
	it('Test Truncate Multiple Spaces', function (done) {
		...
		form
			.setForm(truncateMultipleSpaces)
			.then(() => {
			...
			(testForm.settings.sanitizeConfig = {
				...
				form
					...
					.then(() => {
						...
						assert.equal(
							textFieldWithScript.element?.getElementsByTagName('script')
								.length,
							1,
