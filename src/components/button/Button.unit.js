...
describe('Button Component', function () {
    it('Should build a button component', function () {
        return Harness.testCreate(ButtonComponent, comp1).then((component) => {
            const buttons = Harness.testElements(
                component,
                'button[type="submit"]',
                1,
            );
            ...
            .then((form) => {
                form.on('change', () => {
                    ...
                    setTimeout(() => {
                        console.log(
                            'Text Content: ',
                            button.refs.buttonMessage.innerHTML,
                        );
                        assert.equal(
    ...
    it('Should not change color and show message if the error is silent', function (done) {
        ...
        Formio.createForm(element, formJson, {
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    assert(
                        !button.refs.button.className.includes(
                    ...
                    assert(
                        !button.refs.buttonMessageContainer.className.includes(
                            'has-success',
                        ),
                    );
                    assert(
    ...
    it("Should reset values of all the form's components and update properties dependent on values", function (done) {
        ...
        form.setForm(formWithResetValue)
            .then(() => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            textField.visible,
                            false,
                            'TextFiled should NOT be visible',
                        );
                        assert.equal(
                            textArea.visible,
                            false,
        ...
        Formio.createForm(element, form)
            .then((formObj) => {
                ...
                _.each(btns, (btn, index) => {
                    ...
                    const includeClass =
                        btn.refs.button.classList.contains(btnClass);
                    assert.equal(
                        includeClass,
                        true,
                        `Should set ${theme[index]} theme for button`,
                    );
                });
        ...
        Formio.createForm(element, form)
            .then((formObj) => {
                ...
                assert.equal(
                    includeClass,
                    true,
                    'Should set btn-block class for button',
                );
