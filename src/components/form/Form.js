...
export default class FormComponent extends Component {
    ...
    static get builderInfo() {
        return {
            ...
            group: 'premium',
            documentation:
                '/userguide/form-building/premium-components#nested-form',
            weight: 110,
    ...
    init() {
        ...
        if (
            this.builderMode &&
            Object.prototype.hasOwnProperty.call(this.component, 'formRevision')
        ) {
    ...
    getComponent(path, fn) {
        ...
        const originalPathStr = `${this.path}.data.${getStringFromComponentPath(
            path,
        )}`;
        if (this.subForm) {
    ...
    getValueAsString(value, options) {
      ...
      `;
            ...
            this.everyComponent(
                (component) => {
                    if (
                        component.isInputComponent &&
                        component.visible &&
                        !component.skipInEmail
                    ) {
                        result += `
            <tr>
              <th style="padding: 5px 10px;">${component.label}</th>
              <td style="width:100%;padding:5px 10px;">${component.getView(
                  component.dataValue,
                  options,
              )}</td>
            </tr>
        ...
        return super.attach(element).then(() => {
            ...
            return this.subFormReady.then(() => {
                ...
                if (this.subForm) {
                    ...
                    if (
                        !this.valueChanged &&
                        this.dataValue.state !== 'submitted'
                    ) {
                ...
                if (!this.builderMode && this.component.modalEdit) {
                    const modalShouldBeOpened = this.componentModal
                        ? this.componentModal.isOpened
                        : false;
    ...
    updateSubWizards(subForm) {
        if (
            this.isNestedWizard &&
            this.root?.subWizards &&
            subForm?._form?.display === 'wizard'
        ) {
    ...
    createSubForm(fromAttach) {
        this.subFormReady = this.loadSubForm(fromAttach)
            .then((form) => {
                ...
                // Render the form.
                return new Form(form, this.getSubOptions()).ready.then(
                    (instance) => {
                        ...
                        this.subForm.localRoot = this.isNestedWizard
                            ? this.localRoot
                            : this.subForm;
                        this.restoreValue();
                        this.valueChanged = this.hasSetValue;
    ...
    loadSubForm(fromAttach) {
        ...
        if (
            ...
            return new Formio(this.formSrc)
    ...
    checkComponentValidity(data, dirty, row, options) {
        ...
        if (this.subForm) {
            return this.subForm.checkValidity(
                this.subFormData,
                dirty,
                null,
                silentCheck,
    ...
    isSubFormLazyLoad() {
        return (
            this.root?._form?.display === 'wizard' && this.component.lazyLoad
        );
    }
    ...
    setSubFormValue(submission, flags) {
        ...
        if (shouldLoadSubmissionById) {
            const formId =
                submission.form || this.formObj.form || this.component.form;
            ...
            this.subForm.loadSubmission().catch((err) => {
                console.error(
                    `Unable to load subform submission ${submission._id}:`,
                    err,
                );
            });
        } else {
    ...
    get isNestedWizard() {
        return (
            this.subForm?._form?.display === 'wizard' &&
            this.parent?.parent?._form?.display === 'wizard'
        );
    }
