...
import { comp1, comp3, comp4, comp5, comp6 } from './fixtures';
...
describe('Form Component', function () {
        ...
        it('Should be able to set value to Nested Form Component and check result in the email template', function (done) {
            ...
            form.setForm(comp5)
                .then(() => {
                    ...
                    const toString = form.getValueAsString(textField.data, {
                        email: true,
                    });
                    assert.ok(
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            assert.equal(
                                model.dataValue,
                                '',
                                'Should refresh and clear value',
                            );
                            done();
                        }, 300);
                    }, 300);
    ...
    describe('Modal Edit', function () {
        it('Should render preview when modalEdit', function (done) {
            const formElement = document.createElement('div');
            const form = new Webform(formElement);
            form.setForm(formModalEdit)
                .then(() => {
                    const preview =
                        form.element.querySelector('[ref="openModal"]');
                    assert(
                        preview,
                        'Should contain element to open a modal window',
                    );
                    done();
                })
                .catch(done);
    ...
    describe('Conditional rendering', function () {
        it('Should render and set submission to conditional form component', function (done) {
            const formElement = document.createElement('div');
            const form = new Webform(formElement);
            form.setForm(formComponentWithConditionalRenderingForm)
                .then(() => {
                    form.setSubmission({
                        data: {
                            checkbox: true,
                        ...
                        setTimeout(() => {
                            const checkbox = formElement.querySelector(
                                'input[name="data[checkbox]"]',
                            );
                            const textField = formElement.querySelector(
                ...
                .then(() => {
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            nestedForm.disabled,
                            true,
                            'Nested Form should be disabled',
                        );
                        done();
                    }, 300);
                })
