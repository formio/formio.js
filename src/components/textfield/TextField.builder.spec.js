...
describe('TextField Builder', function () {
    ...
    it('Should create a new textfield component', function () {
        ...
        return builder.editForm.formReady.then(() => {
            ...
            assert(
                preview.indexOf(
                    '<label class="control-label" style="">Text Field</label>',
                ) !== -1,
                'Must have a label',
            );
            assert(
                preview.indexOf(
                    '<input name="data[textField]" type="text" class="form-control"',
                ) !== -1,
                'Must have an input',
            );
        });
    ...
    it('Should allow you to change the label', function (done) {
        Harness.setComponentProperty(
            ...
            (preview) => {
                ...
                assert(
                    preview.indexOf(
                        '<label class="control-label" style="">First Name</label>',
                    ) !== -1,
                    'Must have a label',
                );
                done();
    ...
    it('Should allow you to hide/show the label', function (done) {
        Harness.setComponentProperty('hideLabel', false, true, (preview) => {
            assert(
                preview.indexOf('<label class="control-label"') === -1,
    ...
    it('Should allow you to change the label position', function (done) {
        Harness.setComponentProperty(
            ...
            (preview) => {
                assert(
                    preview.match(/input.*label/),
                    'Label must be on bottom.',
                );
                Harness.setComponentProperty(
                    'labelPosition',
                    'bottom',
    ...
    it('Should allow you to set the input mask', function (done) {
        Harness.testBuilderProperty(
            ...
            () => {
                assert.equal(
                    builder.preview.inputs[0].placeholder,
                    '(___) ___-____',
                );
    ...
    it('Should set the tooltip of the input', function (done) {
        Harness.testBuilderProperty(
            ...
            () => {
                assert(
                    builder.preview.tooltip,
                    'There should be a tooltip instance',
                );
                builder.preview.tooltip.show();
                const toolTipText =
                    builder.preview.element.querySelector('.tooltip-inner');
                assert.equal(
