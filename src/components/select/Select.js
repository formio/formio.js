...
export default class SelectComponent extends ListComponent {
	...
	isEntireObjectDisplay() {
		return (
			this.component.dataSrc === 'resource' && this.valueProperty === 'data'
		);
	}
		...
		if (this.refs.selectContainer && this.component.widget === 'html5') {
			...
			div.innerHTML = this.sanitize(
				this.renderTemplate('selectOption', {
					...
					id,
					useId:
						(this.valueProperty === '' || this.isEntireObjectDisplay()) &&
						_.isObject(value) &&
						id,
				}),
				this.shouldSanitizeValue,
	...
	loadItems(url, search, headers, options, method, body) {
		...
		if (this.component.searchField && search) {
			...
			query[this.component.searchField] = this.component.searchField.endsWith(
				'__regex',
			)
	...
	getValue() {
		...
		if (this.choices) {
			...
			if (
				...
				value ===
					this.t(this.component.placeholder, {
						_userInput: true,
					})
			) {
	...
	setChoicesValue(value, hasPreviousValue, flags = {}) {
		...
		if (this.choices) {
			...
			if (hasValue) {
				...
				// Add the currently selected choices if they don't already exist.
				const currentChoices =
					Array.isArray(value) && this.component.multiple ? value : [value];
		...
		if (values) {
			if (_.isObject(value)) {
				const compareComplexValues = (optionValue) => {
					...
					try {
						return (
							JSON.stringify(normalizedOptionValue) === JSON.stringify(value)
						);
					} catch (err) {
			...
			return (
				values.findIndex(
					(optionValue) => this.normalizeSingleValue(optionValue) === value,
				) !== -1
			);
		}
		return false;
	...
	asString(value, options = {}) {
		...
		const convertToString = (data, valueProperty) => {
			if (valueProperty) {
				if (Array.isArray(data)) {
					data.forEach(
						(item) => (item[valueProperty] = item[valueProperty].toString()),
					);
				} else {
