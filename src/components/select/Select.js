...
export default class SelectComponent extends ListComponent {
    ...
    selectValueAndLabel(data) {
        const value = this.getOptionValue(
            this.isEntireObjectDisplay() && !this.itemValue(data)
                ? data
                : this.itemValue(data),
        );
        return {
            ...
            label: this.itemTemplate(
                this.isEntireObjectDisplay() && !_.isObject(data.data)
                    ? { data: data }
                    : data,
                value,
            ),
    ...
    itemTemplate(data, value) {
        ...
        if (data && !this.component.template) {
            ...
            const value =
                typeof itemLabel === 'string'
                    ? this.t(itemLabel, { _userInput: true })
                    : itemLabel;
        ...
        if (data.data) {
            ...
            const hasNestedFields = /item\.data\.\w*/g.test(
    ...
    addOption(value, label, attrs = {}, id = getRandomComponentId()) {
        ...
        const idPath = this.component.idPath
            ? this.component.idPath
                  .split('.')
                  .reduceRight((obj, key) => ({ [key]: obj }), id)
            : {};
        const option = {
        ...
        const skipOption = this.component.uniqueOptions
            ? !!this.selectOptions.find((selectOption) =>
                  _.isEqual(selectOption.value, option.value),
              )
            : false;
        ...
        if (this.refs.selectContainer && this.component.widget === 'html5') {
            ...
            div.innerHTML = this.sanitize(
                this.renderTemplate('selectOption', {
                    selected: _.isEqual(
                        this.getOptionValue(this.dataValue),
                        option.value,
                    ),
                    option,
                    attrs,
    ...
    addValueOptions(items) {
        ...
        if (!this.selectOptions.length) {
            ...
            const currentChoices =
                Array.isArray(data) && this.component.multiple ? data : [data];
    ...
    setItems(items, fromSearch) {
        ...
        if (this.itemsFromUrl) {
            areItemsEqual = this.isSelectURL
                ? _.isEqual(items, this.downloadedResources)
                : false;
    ...
    choicesOptions() {
        const useSearch = Object.prototype.hasOwnProperty.call(
            this.component,
            'searchEnabled',
        )
    ...
    attach(element) {
        ...
        if (Choices) {
            ...
            if (this.component.multiple) {
                ...
                this.choices.containerOuter.element.setAttribute(
                    'tabIndex',
                    '-1',
                );
            ...
            Input.prototype.addFocusBlurEvents.call(
                this,
            ...
            if (this.itemsFromUrl && !this.component.noRefreshOnScroll) {
                ...
                this.addEventListener(this.scrollList, 'scroll', () =>
                    this.onScroll(),
                );
            }
        ...
        if (this.component.searchField) {
            ...
            const debounceTimeout =
                this.component.searchField &&
                (this.isSelectResource || this.isSelectURL)
                    ? (this.component.searchDebounce === 0
                          ? 0
                          : this.component.searchDebounce ||
                            this.defaultSchema.searchDebounce) * 1000
                    : 0;
            ...
            this.addEventListener(input, 'stopSearch', () =>
                this.triggerUpdate(),
            );
        ...
        if (
            this.choices &&
            choicesOptions.placeholderValue &&
            this.choices._isSelectOneElement
        ) {
            ...
            this.addEventListener(this.refs.addResource, 'click', (event) => {
                ...
                new Form(formioForm, formUrl, {}).ready.then((form) => {
                    form.on('submit', (submission) => {
                        ...
                        let value = this.valueProperty
                            ? _.get(submission, this.valueProperty)
                            : submission;
    ...
    setDropdownPosition() {
        ...
        _.assign(dropdown.style, {
            top: `${
                isFlipped
                    ? containerPosition.top - dropdown.offsetHeight
        ...
        const added = values.reduce((defaultAdded, value) => {
            ...
            if (items && items.length) {
                _.each(items, (choice) => {
                    ...
                    const itemValue = keyValue
                        ? choice.value
                        : this.itemValue(choice, isSelectOptions);
    ...
    normalizeSingleValue(value, retainObject) {
                    ...
                    if (dataValue && _.isArray(dataValue) && dataValue.length) {
                        dataValue.forEach((dataValueItem) => {
                            ...
                            templateData[dataValueItemValue] =
                                this.templateData[dataValueItemValue];
                        });
                    }
        ...
        const normalize = {
            ...
            number() {
                ...
                const isEquivalent =
                    value.toString() === numberValue.toString();
    ...
    normalizeValue(value) {
        if (this.component.multiple && Array.isArray(value)) {
            return value.map((singleValue) =>
                this.normalizeSingleValue(singleValue, true),
            );
        }
    ...
    setValue(value, flags = {}) {
        ...
        if (
            ...
            (_.isEqual(value, previousValue) ||
                (_.isEqual(previousValue, {}) && _.isEqual(flags, {}))) &&
        ...
        if (this.isInitApiCallNeeded(hasValue)) {
            ...
            this.triggerUpdate(
                _.get(value.data || value, searchProperty, value),
                true,
            );
            return changed;
        }
    ...
    setChoicesValue(value, hasPreviousValue, flags = {}) {
        ...
        hasPreviousValue =
            hasPreviousValue === undefined ? true : hasPreviousValue;
        if (this.choices) {
            // Now set the value.
            if (hasValue) {
            ...
            if (hasValue) {
                ...
                if (
                    (!_.isEqual(this.dataValue, this.defaultValue) &&
                        this.selectOptions.length < 2) ||
                    (this.selectData && flags.fromSubmission)
                ) {
                    const { value, label } = this.selectValueAndLabel(
                        this.dataValue,
                    );
                ...
                _.each(this.selectOptions, (selectOption) => {
                    _.each(values, (val) => {
                        ...
                        if (
                            _.isEqual(val, selectOption.value) &&
                            selectOption.element
                        ) {
                            selectOption.element.selected = true;
                            selectOption.element.setAttribute(
                                'selected',
                                'selected',
                            );
                            return false;
        ...
        if (values) {
            if (_.isObject(value)) {
                const compareComplexValues = (optionValue) => {
                    const normalizedOptionValue = this.normalizeSingleValue(
                        optionValue,
                        true,
                    );
                    ...
                    try {
                        return (
                            JSON.stringify(normalizedOptionValue) ===
                ...
                return (
                    values.findIndex((optionValue) =>
                        compareComplexValues(optionValue),
                    ) !== -1
                );
            }
            ...
            return (
                values.findIndex(
                    (optionValue) =>
                        this.normalizeSingleValue(optionValue) === value,
                ) !== -1
            );
        }
        return false;
    ...
    getOptionValue(value) {
        return _.isObject(value) && this.isEntireObjectDisplay()
            ...
            : _.isObject(value) &&
                (this.valueProperty || this.component.key !== 'resource')
              ? value
              : _.isObject(value) && !this.valueProperty
                ? this.interpolate(this.component.template, {
                      item: value,
                  }).replace(/<\/?[^>]+(>|$)/g, '')
                : _.isNull(value)
                  ? this.emptyValue
    ...
    getOptionsValues() {
        ...
        return rawItems.map((item) =>
            this.getOptionValue(this.itemValue(item)),
        );
    }
    ...
    asString(value, options = {}) {
        ...
        const convertToString = (data, valueProperty) => {
            ...
            if (
                Array.isArray(data) &&
                data.some((item) => this.isBooleanOrNumber(item))
            ) {
                data = data.map((item) => {
