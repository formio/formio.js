	...
	it('Should render and set values in selects with different widget types', function (done) {
		...
		Formio.createForm(element, form)
			.then((form) => {
				...
				setTimeout(() => {
					assert.equal(
						selectChoices.element.querySelectorAll('.choices__item--choice')
							.length,
						3,
					);
					const value = 'b';
			...
			.then((form) => {
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						assert.equal(
							select.choices.containerInner.element.children[1].children[0]
	...
	it('Should set value in select url with lazy load option', function (done) {
		...
		Formio.createForm(element, form)
			.then((form) => {
				...
				setTimeout(() => {
					assert.deepEqual(select.getValue(), {
						name: 'Ivan',
					});
					assert.deepEqual(select.dataValue, {
						name: 'Ivan',
					});
					assert.equal(
						select.choices.containerInner.element.children[1].children[0]
	...
	it('Should render and set value in select json', function (done) {
		...
		Formio.createForm(element, formObj)
			.then((form) => {
				const select = form.getComponent('select');
				assert.equal(
					select.choices.containerInner.element.children[1].children[0].dataset
						.value,
					'',
				);
				select.choices.showDropdown();
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						values.forEach((value) => {
							assert.equal(
								_.find(select.component.data.values, {
									value,
								}).label,
