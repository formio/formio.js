...
describe('Select Component', function () {
    it('should not stringify select option value', function (done) {
        Harness.testCreate(SelectComponent, comp6).then((component) => {
            ...
            setTimeout(() => {
                assert.equal(
                    component.choices._currentState.items[0].value.value,
                    'a',
                );
                assert.equal(
                    typeof component.choices._currentState.items[0].value,
                    'object',
                );
    ...
    it('should set multiple selected values not repeating them', function (done) {
        Harness.testCreate(SelectComponent, multiSelect).then((component) => {
            ...
            component.setChoicesValue(
                ['Cheers', 'Cyberdyne Systems', 'Massive Dynamic'],
                2,
            );
            const choices = component.element.querySelector(
    ...
    it('should not show selected values in dropdown when searching', function (done) {
        Harness.testCreate(SelectComponent, multiSelect).then((component) => {
            ...
            const itemsInDropdown = component.element.querySelectorAll(
    ...
    it('Should preserve the tabindex', function () {
        return Harness.testCreate(SelectComponent, comp2).then((component) => {
            const element = component.element.getElementsByClassName(
    ...
    it('Should allow to override threshold option of fuzzy search', function () {
        try {
            const c1 = Object.assign(cloneDeep(comp1), {
                selectThreshold: 0.2,
            });
            const c2 = Object.assign(cloneDeep(comp1), {
                selectThreshold: 0.4,
            });
            const c3 = Object.assign(cloneDeep(comp1), {
                selectThreshold: 0.8,
            });
            const comps = [
    ...
    it('should remove selected item', function () {
        return Harness.testCreate(SelectComponent, comp1).then((component) => {
            ...
            const element =
                component.element.getElementsByClassName('choices__button')[0];
            component.choices._handleButtonAction(
                component.choices._store.activeItems,
                element,
            );
    ...
    it('Should render and set values in selects with different widget types', function (done) {
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    assert.equal(
                        selectChoices.element.querySelectorAll(
                            '.choices__item--choice',
                        ).length,
                        3,
                    );
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    assert.deepEqual(select.selectOptions[0].value, {
                        name: 'Ivan',
                    });
    ...
    it('Should be able to search if static search is enable', function (done) {
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                const searchField = select.element.querySelector(
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        const items =
                            select.choices.choiceList.element.children;
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        assert.equal(
                            select.choices.containerInner.element.children[1]
    ...
    it('Should render and set value in select json', function (done) {
        ...
        Formio.createForm(element, formObj)
            .then((form) => {
                const select = form.getComponent('select');
                assert.equal(
                    select.choices.containerInner.element.children[1]
        ...
        Formio.makeRequest = function (formio, type, url) {
            return new Promise((resolve) => {
                ...
                if (url.endsWith('Ivan')) {
                    ...
                    assert.equal(
                        url.endsWith(
                            '/form/60114dd32cab36ad94ac4f94/submission?limit=100&skip=0',
                        ),
                        true,
                    );
                }
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        assert.equal(
                            _.isEqual(
                                form.submission.metadata.selectData.select.data,
                                testItems[0],
                            ),
                            true,
                        );
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        const metadata =
                            form.submission.metadata.selectData.select;
                        assert.equal(_.keys(metadata).length, 2);
                        values.forEach((value) => {
                            assert.equal(
                                _.find(select.component.data.values, { value })
                                    .label,
    ...
    it('Should provide correct metadata.selectData for HTML5 Select', function (done) {
        ...
        Formio.createForm(element, comp21)
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            const metadata =
                                form.submission.metadata.selectData.animals2;
    ...
    it('OnBlur validation should work properly with Select component', function (done) {
        ...
        Formio.createForm(element, comp19)
            .then((form) => {
                ...
                setTimeout(() => {
                    assert(
                        !select.error,
                        'Select should be valid while changing',
                    );
                    ...
                    setTimeout(() => {
                        assert(
                            select.error,
                            'Should set error after Select component was blurred',
                        );
                        done();
                    }, 500);
                }, 200);
...
describe('Select Component with Entire Object Value Property', function () {
    ...
    it('Should get string representation of value for Resource DataSrc Type and Entire Object Value Property', function (done) {
        Harness.testCreate(SelectComponent, comp15.components[0]).then(
            (component) => {
                const entireObject = {
                    a: '1',
                    b: '2',
