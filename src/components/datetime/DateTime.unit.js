...
describe('DateTime Component', function () {
    it('Should build a date time component', function () {
        return Harness.testCreate(DateTimeComponent, comp1).then((dateTime) =>
            dateTime.destroy(),
        );
    });
    ...
    it('Should format value', function () {
        ...
        return Harness.testCreate(DateTimeComponent, comp2).then((dateTime) => {
            assert.equal(
                dateTime.getValueAsString('2020-09-18T12:12:00'),
                '2020-09-18 12:12 PM',
            );
            dateTime.destroy();
        });
    ...
    it('Should allow manual input', function (done) {
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    assert.equal(
                        dateTime.getValue().startsWith(expectedValueStart),
                        true,
                    );
                    assert.equal(
                        dateTime.dataValue.startsWith(expectedValueStart),
                        true,
                    );
    ...
    it('Should format date correctly', function (done) {
        ...
        const getAllFormats = function (formats) {
            ...
            const formatsWithDiffSeparators = separators.reduce(
                (result, separator) => {
    ...
    it('Should disable weekends', function (done) {
        ...
        Formio.createForm(element, form)
            .then((form) => {
                const dateTime = form.getComponent('dateTime');
                const calendar =
                    dateTime.element.querySelector('.flatpickr-input').widget
                        .calendar;
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        dateTime.dataValue.startsWith('2020-04-03T22:11:00'),
                        true,
                    );
    ...
    it('Should save hours and minutes values on first change', function (done) {
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    const calendar =
                        dateTime.element.querySelector('.flatpickr-input')
                            .widget.calendar;
                    ...
                    setTimeout(() => {
                        assert.equal(
                            dateTime.dataValue.includes(expectedValue),
                            true,
                        );
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        inputs[0].classList.contains('is-invalid'),
                        false,
                    );
                    assert.equal(
                        inputs[1].classList.contains('is-invalid'),
                        true,
                    );
                    ...
                    setTimeout(() => {
                        const input2 =
                            dateTime.element.querySelectorAll('.input')[1];
                        assert.equal(
                            input2.classList.contains('is-invalid'),
                            false,
                        );
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        const input2 =
                            textField.element.querySelector('.input');
