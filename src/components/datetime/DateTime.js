...
export default class DateTimeComponent extends Input {
    ...
    static get builderInfo() {
        return {
            ...
            icon: 'calendar',
            documentation:
                '/userguide/form-building/advanced-components#date-and-time',
            weight: 40,
    ...
    get defaultValue() {
        ...
        if (!defaultValue && this.component.defaultDate) {
            defaultValue = FormioUtils.getDateSetting(
                this.component.defaultDate,
            );
    ...
    isEqual(valueA, valueB = this.dataValue) {
        return (
            ...
            moment.utc(valueA).format(this.momentFormat) ===
                moment.utc(valueB).format(this.momentFormat)
        );
    }
    ...
    getValueAsString(value) {
        ...
        if (value && !this.attached && timezone) {
            ...
            return _.trim(
                FormioUtils.momentDate(value, format, timezone).format(format),
            );
        }

        if (Array.isArray(value) && this.component.multiple) {
            return value
                .map((item) => _.trim(moment(item).format(format)))
                .join(', ');
        }
