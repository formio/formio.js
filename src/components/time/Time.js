...
export default class TimeComponent extends TextFieldComponent {
	...
	getStringAsValue(view) {
		return view
			? moment(view, this.component.format).format(this.component.dataFormat)
			: view;
	}
	...
	getValueAsString(value) {
		if (Array.isArray(value) && this.component.multiple) {
			return value
				.map((item) =>
					moment(item, this.component.dataFormat).format(this.component.format),
				)
				.join(', ');
		}
		return (
			(value
				? moment(value, this.component.dataFormat).format(this.component.format)
				: value) || ''
		);
	}
