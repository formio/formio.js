...
describe('DataGrid Component', function () {
    ...
    it('Should build a data grid component', function () {
        return Harness.testCreate(DataGridComponent, comp1).then(
            (component) => {
    ...
    it('Should build a data grid component with formio-component-datagrid class property', function (done) {
        Harness.testCreate(DataGridComponent, comp6)
            .then((component) => {
                const element =
                    component.element.component.components[0].element;
                setTimeout(() => {
                    assert.deepEqual(
                        element.className.includes('formio-component-datagrid'),
                        true,
                    );
                    done();
                }, 200);
    ...
    it('Should not cause setValue loops when logic within hidden component is set', function (done) {
        Formio.createForm(document.createElement('div'), withLogic).then(
            (form) => {
    ...
    it('Should collapse group rows on group header click', function (done) {
        Formio.createForm(
            ...
            const datagrid = form.getComponent('dataGrid');
            assert.equal(
                datagrid.refs[groupHeadersRefName][0]?.classList?.contains(
                    'collapsed',
                ),
                false,
            );
            assert.equal(
                datagrid.refs.chunks[0][0].classList?.contains('hidden'),
                false,
            );
            assert.equal(
                datagrid.refs.chunks[0][1].classList?.contains('hidden'),
                false,
            );
            ...
            setTimeout(() => {
                ...
                assert.equal(
                    collapedGroupRows[0]?.classList?.contains('hidden'),
                    true,
                );
                assert.equal(
                    collapedGroupRows[1]?.classList?.contains('hidden'),
                    true,
                );
                done();
            }, 300);
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            const dataGridRowConditionalField =
                                form.getComponent([
                                    'dataGrid',
                                    0,
                                    'rowShowShowTextfieldWhenDataGridRadioHasShowValue',
...
describe('DataGrid Panels', function () {
    ...
    it('Should be able to set the values of one panel in the DataGrid.', function () {
        return Harness.testCreate(DataGridComponent, comp2).then(
            (component) => {
                Harness.testSetGet(component, [
                    {
                        firstName: 'Joe',
    ...
    it('Should have unique IDs inside data grid', function () {
        return Harness.testCreate(DataGridComponent, comp7).then(
            (component) => {
                component.addRow();
                const idArr = [];
...
describe('DataGrid disabling', function () {
    it('Child components should be disabled', function () {
        return Harness.testCreate(DataGridComponent, comp3).then(
            (component) => {
                assert.equal(
                    component.components.reduce(
                        (acc, child) => acc && child.parentDisabled,
                        true,
                    ),
                    true,
                );
