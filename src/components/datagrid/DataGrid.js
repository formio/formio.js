...
export default class DataGridComponent extends NestedArrayComponent {
	...
	hasTopSubmit() {
		return (
			this.hasAddButton() && ['top', 'both'].includes(this.addAnotherPosition)
		);
	}
	...
	getColumns() {
		return this.columns.filter((comp) => {
			return (
				!Object.prototype.hasOwnProperty.call(this.visibleColumns, comp.key) ||
	...
	checkValidity(data, dirty, row, silentCheck) {
		...
		if (
			!this.checkComponentValidity(data, dirty, row, {
				silentCheck,
			})
		) {
			return false;
		...
		this.rows.forEach((row, rowIndex) => {
			_.each(row, (col, key) => {
				if (col && typeof col.checkConditions === 'function') {
					...
					visibility[key] =
						!!visibility[key] ||
						(col.checkConditions(data, flags, dataValue[rowIndex]) &&
							col.type !== 'hidden');
	...
	restoreComponentsContext() {
		this.rows.forEach((row, index) =>
			_.forIn(row, (component) => (component.data = this.dataValue[index])),
		);
	}
