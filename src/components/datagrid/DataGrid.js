...
export default class DataGridComponent extends NestedArrayComponent {
    ...
    get initRows() {
        return (
            this.builderMode || this.path === 'defaultValue' || !this.initEmpty
        );
    }
    ...
    get defaultValue() {
        ...
        for (
            let dIndex = defaultValue.length;
            dIndex < this.minLength;
            dIndex++
        ) {
            defaultValue.push({});
        }
    ...
    get allowReorder() {
        return (
            !this.options.readOnly && _.get(this.component, 'reorder', false)
        );
    }
    ...
    hasRowGroups() {
        return (
            _.get(this, 'component.enableRowGroups', false) && !this.builderMode
        );
    }
    ...
    hasRemoveButtons() {
        return (
            ...
            this.dataValue.length >
                _.get(this.component, 'validate.minLength', 0)
        );
    }
    ...
    hasTopSubmit() {
        return (
            this.hasAddButton() &&
            ['top', 'both'].includes(this.addAnotherPosition)
        );
    }
    ...
    hasBottomSubmit() {
        return (
            this.hasAddButton() &&
            ['bottom', 'both'].includes(this.addAnotherPosition)
        );
    }
    ...
    loadRefs(element, refs) {
        ...
        if (refs['messageContainer'] === 'single') {
            const container = _.last(
                element.querySelectorAll('[ref=messageContainer]'),
            );
    ...
    attach(element) {
        ...
        if (this.allowReorder) {
            ...
            if (this.root.dragulaLib) {
                this.dragula = this.root
                    .dragulaLib([this.refs[`${this.datagridKey}-tbody`]], {
                        moves: (
                            _draggedElement,
                            _oldParent,
                            clickedElement,
                        ) => {
                            const clickedElementKey =
                                clickedElement.getAttribute('data-key');
                            const oldParentKey =
                                _oldParent.getAttribute('data-key');

                            //Check if the clicked button belongs to that container, if false, it belongs to the nested container
                            if (oldParentKey === clickedElementKey) {
                                return clickedElement.classList.contains(
                                    'formio-drag-button',
                                );
                            }
                        },
                ...
                this.dragula.on('cloned', (el, original) => {
                    if (el && el.children && original && original.children) {
                        _.each(original.children, (child, index) => {
                            ...
                            if (styles.cssText !== '') {
                                el.children[index].style.cssText =
                                    styles.cssText;
                            } else {
        ...
        this.refs[`${this.datagridKey}-removeRow`].forEach(
            (removeButton, index) => {
                this.addEventListener(
                    removeButton,
                    'click',
                    this.removeRow.bind(this, index),
                );
            },
        );

        if (this.hasRowGroups()) {
            this.refs.chunks = this.getRowChunks(
                this.getGroupSizes(),
                this.refs[`${this.datagridKey}-row`],
            );
            this.refs[`${this.datagridKey}-group-header`].forEach(
                (header, index) => {
        ...
        this.rows.forEach((row, rowIndex) => {
            ...
            columns.forEach((col) => {
                this.attachComponents(
                    this.refs[this.datagridKey][
                        rowIndex * rowLength + columnIndex
                    ],
    ...
    onReorder(element, _target, _source, sibling) {
        if (!element.dragInfo || (sibling && !sibling.dragInfo)) {
            console.warn(
                'There is no Drag Info available for either dragged or sibling element',
            );
            return;
        }
    ...
    checkColumns(data, flags = {}) {
            ...
            _.each(row, (col, key) => {
                if (col && typeof col.checkConditions === 'function') {
                    ...
                    if (col.component.logic && firstRowCheck) {
                        ...
                        const equalColumns = _.isEqualWith(
                            this.columns[compIndex],
                            col.component,
                            (col1, col2, key) => {
        ...
        const rebuild =
            !_.isEqual(visibility, this.visibleColumns) || logicRebuild;
    ...
    setValue(value, flags = {}) {
        ...
        const isSettingSubmission =
            flags.fromSubmission && !_.isEqual(value, this.emptyValue);
        ...
        if (
            this.initRows ||
            isSettingSubmission ||
            (Array.isArray(this.dataValue) &&
                this.dataValue.length !== this.rows.length)
        ) {
    ...
    restoreComponentsContext() {
        this.rows.forEach((row, index) =>
            _.forIn(
                row,
                (component) => (component.data = this.dataValue[index]),
            ),
        );
    }
