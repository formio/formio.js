...
export default class ReCaptchaComponent extends Component {
    ...
    static get builderInfo() {
        return {
            ...
            icon: 'refresh',
            documentation:
                '/userguide/form-building/premium-components#recaptcha',
            weight: 40,
    ...
    createInput() {
        if (this.builderMode) {
            ...
            if (siteKey) {
                ...
                console.warn(
                    'There is no Site Key specified in settings in form JSON',
                );
            }
        }
    }
    ...
    verify(actionName) {
        ...
        if (this.recaptchaApiReady) {
            this.recaptchaVerifiedPromise = new NativePromise(
                (resolve, reject) => {
                                ...
                                grecaptcha.ready(
                                    _debounce(() => {
                                        grecaptcha
                                            ...
                                            .then((token) => {
                                                return this.sendVerificationRequest(
                                                    token,
                                                ).then(
                                                    ({
                                                        verificationResult,
    ...
    beforeSubmit() {
        if (this.recaptchaVerifiedPromise) {
            return this.recaptchaVerifiedPromise.then(() =>
                super.beforeSubmit(),
            );
        }
