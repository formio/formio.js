...
import { comp1, comp3, comp4, comp5, comp6, comp7 } from './fixtures';
...
describe('SelectBoxes Component', function () {
    it('Should build a SelectBoxes component', function () {
        return Harness.testCreate(SelectBoxesComponent, comp1).then(
            (component) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            assert.equal(
                                form.submission.metadata.listData.selectBoxes
                                    .length,
                                3,
                            );
                            Formio.makeRequest = originalMakeRequest;
                ...
                components: [
                    {
                        ...
                        minSelectedCountMessage:
                            'Please select at least {{minCount}} items.',
                    },
                ],
            };
            const element = document.createElement('div');
                ...
                components: [
                    {
                        ...
                        maxSelectedCountMessage:
                            'Please select {{maxCount}} items at most.',
                    },
                ],
            };
            const element = document.createElement('div');
        ...
        it('Should provide validation for ValueProperty', function (done) {
            ...
            Formio.createForm(element, form)
                .then(async (form) => {
                    ...
                    setTimeout(() => {
                        const inputs =
                            selectBoxes.element.querySelectorAll('input');
            ...
            .then((form) => {
                ...
                const checkInputs = (values) => {
                    Object.entries(values).forEach(([key, value]) => {
                        const input = selectBoxes.element.querySelector(
                            `input[value="${key}"]`,
                        );
...
describe('SelectBoxes Component Part Two', function () {
    it('should have red asterisk left hand side to the options labels if component is required and label is hidden', function () {
        return Harness.testCreate(SelectBoxesComponent, comp3).then(
            (component) => {
                const options =
    ...
    it('Should perform OnlyAvailableItems check properly', function (done) {
        Harness.testCreate(SelectBoxesComponent, comp7)
            .then((component) => {
                ...
                assert.equal(
                    component.validateValueAvailability(true, {
                        a: false,
                        b: false,
                        c: false,
                    }),
                ...
                assert.equal(
                    component.validateValueAvailability(false, {}),
                    true,
                    'Should be valid',
                );
                done();
