...
export default class TreeComponent extends NestedDataComponent {
    ...
    createComponents(data, node) {
        const components = this.componentComponents.map((component) => {
            const componentInstance = Components.create(
                component,
                this.componentOptions,
                data,
            );
    ...
    attachNode(element, node) {
        ...
        return NativePromise.all([componentsPromise, childrenPromise]);
    }
    ...
    attachActions(node) {
        ...
        this.loadRefs.call(
            node,
            node.refs.content.children[0]?.children[1] || node.refs.content,
            {
                cancelNode: 'single',
                saveNode: 'single',
            },
    ...
    attachChildren(node) {
        ...
        return NativePromise.all(
            _.map(childElements, (childElement, index) =>
                this.attachNode(childElement, node.children[index]),
            ),
        );
    }
    ...
    setRoot() {
        ...
        this.treeRoot = new Node(null, value, {
            ...
            parentPath: this.isDefaultValueComponent
                ? this.path || this.component.key
                : null,
        });
    ...
    checkNode(data, node, flags, row) {
        return node.children.reduce(
            (result, child) =>
                this.checkNode(data, child, flags, row) && result,
    ...
    getComponents() {
        return this.treeRoot &&
            (this.isDefaultValueComponent ||
                (!this.isDefaultValueComponent && !this.builderMode))
