...
describe('Tree Component', () => {
    it('Should set and render values in readOnly mode', function (done) {
        Harness.testCreate(TreeComponent, comp1).then((component) => {
            ...
            assert.equal(
                component.element.querySelectorAll('.tree__node-content')
                    .length,
                3,
            );
            assert.equal(
                component.element.querySelectorAll('.editNode').length,
                3,
            );
            ...
            const valueContainers =
                component.element.querySelectorAll('.col-sm-2');
    ...
    it('Should render tree and allow to save node with basic, layout and data component inside', function (done) {
        Harness.testCreate(TreeComponent, comp2).then((component) => {
            ...
            const inputFields = {
                'data[tree][dataGrid][0][textField]':
                    value.data.dataGrid[0].textField,
    ...
    it('Should keep the node of parent tree opened when saving the node of a child tree', function (done) {
        Harness.testCreate(TreeComponent, comp3).then((component) => {
            ...
            const clickEvent = new Event('click');
            const childSaveNodeBtn = component.element
                .querySelector('.formio-component-tree1')
                .querySelector('[ref="saveNode"]');
            ...
            setTimeout(() => {
                ...
                const childTree = component.element.querySelector(
                    '.formio-component-tree1',
                );
    ...
    it('Should allow to open node of a child tree for editing after opening parent tree node', function (done) {
        Harness.testCreate(TreeComponent, comp3).then((component) => {
            assert.equal(
                !!component.treeRoot.refs.cancelNode,
                true,
                'Should show cancel btn for parent tree',
            );
            assert.equal(
                !!component.treeRoot.refs.saveNode,
                true,
                'Should show save btn for parent tree',
            );
            assert.equal(
                !!component.treeRoot.refs.editNode,
                false,
                'Should not show edit btn for parent node in editing tree',
            );

            assert.equal(
                !!component.treeRoot.components[0].treeRoot.refs.cancelNode,
                true,
                'Should show cancel btn for child tree',
            );
            assert.equal(
                !!component.treeRoot.components[0].treeRoot.refs.saveNode,
                true,
                'Should show save btn for child tree',
            );
            assert.equal(
                !!component.treeRoot.components[0].treeRoot.refs.editNode,
                false,
                'Should not show edit btn for child tree in editing tree',
            );

            const clickEvent = new Event('click');
            const childSaveNodeBtn =
                component.treeRoot.components[0].treeRoot.refs.saveNode;
            ...
            setTimeout(() => {
                assert.equal(
                    !!component.treeRoot.refs.cancelNode,
                    false,
                    'Should not show cancel btn for parent tree',
                );
                assert.equal(
                    !!component.treeRoot.refs.saveNode,
                    false,
                    'Should not show save btn for parent tree',
                );
                assert.equal(
                    !!component.treeRoot.refs.editNode,
                    true,
                    'Should show edit btn for parent tree',
                );
                ...
                setTimeout(() => {
                    assert.equal(
                        !!component.treeRoot.components[0].treeRoot.refs
                            .saveNode,
                        false,
                        'Should not show save btn for child tree',
                    );
                    assert.equal(
                        !!component.treeRoot.components[0].treeRoot.refs
                            .editNode,
                        true,
                        'Should show edit btn for child tree',
                    );

                    const childEditNodeBtn =
                        component.treeRoot.components[0].treeRoot.refs.editNode;

                    childEditNodeBtn.dispatchEvent(clickEvent);

                    assert.equal(
                        component.treeRoot.components[0].treeRoot.editing,
                        true,
                        'Should set editing mode for child tree',
                    );
                    assert.equal(
                        !!component.treeRoot.components[0].treeRoot.refs
                            .saveNode,
        ...
        form.setForm(comp4)
            .then(() => {
                ...
                assert.equal(
                    ...
                    true,
                    'Submit button should being disabled',
                );
                const tree = form.getComponent(['tree']);
                const textField = tree
                    .getComponents()[0]
