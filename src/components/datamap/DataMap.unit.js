...
describe('DataMap Component', () => {
  ...
  it('Should render data from submission properly when the Data Map is inside conditionally shown layout component', (done) => {
    Formio.createForm(document.createElement('div'), formWithConditionalPanel, {
      readOnly: true,
    })
      .then((form) => {
        form.submission = {
        ...
        setTimeout(() => {
          ...
          assert.equal(
            dataMap1.visible,
            true,
            'Data Map should become visible',
          );
          assert.equal(
            dataMap.visible,
            true,
            'Data Map inside a panel should become visible',
          );
          assert.deepEqual(
            dataMap1.dataValue,
            {
          ...
          assert.equal(
            dataMap1.rows.length,
            1,
            'Should create rows for Data Grid',
          );
          assert.equal(
            dataMap.rows.length,
            2,
            'Should create rows for Data Grid inside a panel',
          );
