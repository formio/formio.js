...
export default class DataMapComponent extends DataGridComponent {
	...
	get schema() {
		...
		if (this.components && this.components.length > 0) {
			schema.valueComponent =
				this.components[this.components.length - 1].schema;
		}
	...
	componentContext(component) {
		return this.iteratableRows[component.row].find(
			(comp) => comp.components.key === component.key,
		).data;
	}
	...
	getValueAsString(value, options) {
      ...
      `;
          ...
          <tr>
            ...
            <td style="width:100%;padding:5px 10px;">${this.getView(
							value[key],
							options,
						)}</td>
          </tr>
	...
	checkColumns() {
		if (
			this.builderMode ||
			!this.dataValue ||
			!Object.keys(this.dataValue).length
		) {
