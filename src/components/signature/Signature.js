...
export default class SignatureComponent extends Input {
    ...
    static get builderInfo() {
        return {
            ...
            weight: 120,
            documentation:
                '/developers/integrations/esign/esign-integrations#signature-component',
    ...
    init() {
        ...
        if (
            ...
            this.ratio =
                this.component.overlay?.width / this.component.overlay?.height;
            this.component.width = '100%';
        ...
        if (this.signaturePad) {
            if (super.disabled) {
                ...
                if (this.refs.signatureImage && this.dataValue) {
                    this.refs.signatureImage.setAttribute(
                        'src',
                        this.dataValue,
                    );
                }
    ...
    checkSize(force, scale) {
        if (
            ...
            (force ||
                (this.refs.padBody &&
                    this.refs.padBody.offsetWidth !== this.currentWidth))
        ) {
            this.scale = force ? scale : this.scale;
            this.currentWidth = this.refs.padBody.offsetWidth;
            const width = this.currentWidth * this.scale;
            const height = this.ratio
                ? width / this.ratio
                : this.refs.padBody.offsetHeight * this.scale;
            const maxHeight = this.ratio
                ? height
                : this.refs.padBody.offsetHeight * this.scale;
