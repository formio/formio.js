...
describe('Columns Component', function () {
		...
		Harness.testCreate(ColumnsComponent, comp2)
			.then((component) => {
				...
				oddIndexes.forEach((i) => {
					if (i === 0) {
						assert.equal(
							columns[i].currentWidth,
							2,
							`column[${i}] should have width = 2`,
						); // it has at least a component as visible
					} else {
						assert.equal(
							columns[i].currentWidth,
							0,
							`column[${i}] should have width = 0`,
						); // columns which has no visible components should have currentWidth as 0
					}
				});
				...
				assert.equal(
					columns.every((col) => col.currentWidth === 2),
					true,
					'all columns should have width = 2',
				); // ensure we have initial width
			})
			.then(done)
			.catch(done);
	});
	...
	it('Should clear fields in modal window after confirming to clear data in dialog window', function (done) {
		Harness.testCreate(ColumnsComponent, comp3)
			.then((component) => {
				const hiddenModalWindow = component.element.querySelector(
				...
				setTimeout(() => {
					assert.equal(
						!!component.element.querySelector('.component-rendering-hidden'),
						false,
					);

					const inputEvent = new Event('input');
					const columnsInputField = component.element.querySelector(
						'[name="data[textField]"]',
					);
					...
					setTimeout(() => {
						assert.equal(
							component.element.querySelector('[name="data[textField]"]').value,
							'alexy@form.io',
						);
						...
						setTimeout(() => {
							...
							const clickEvent = new Event('click');
							const btnForCleaningValues = document.querySelector(
								'[ref="dialogYesButton"]',
							);
							...
							setTimeout(() => {
								...
								setTimeout(() => {
									assert.equal(
										component.element.querySelector('[name="data[textField]"]')
											.value,
										'',
									);
									done();
