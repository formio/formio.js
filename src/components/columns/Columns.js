...
export default class ColumnsComponent extends NestedComponent {
    ...
    get schema() {
        ...
        schema.columns?.map((column, colIndex) => {
            column.components.map((comp, compIndex) => {
                ...
                schema.columns[colIndex].components[compIndex] =
                    component.schema;
            });
        });
        return schema;
    ...
    init() {
        ...
        _.each(this.component.columns, (column, index) => {
            ...
            column.currentWidth = this.options.condensedMode
                ? this.gridSize
                : column.width || 0;
    ...
    render() {
        return super.render(
            this.renderTemplate('columns', {
                ...
                columnComponents: this.columns.map((column) =>
    ...
    justify() {
        return this.columns.reduce(
            (redraw, items, index) =>
                this.justifyColumn(items, index) || redraw,
            false,
        );
    }
