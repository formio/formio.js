...
describe('Container Component', function () {
    it('Should build a container component', function () {
        return Harness.testCreate(ContainerComponent, comp1).then(
            (component) => {
                const inputs = Harness.testElements(
                    component,
                    'input[type="text"]',
                    2,
                );
                for (let i = 0; i < inputs.length; i++) {
                    assert.equal(
                        inputs[i].name,
                        `data[${comp1.key}][${comp1.components[i].key}]`,
                    );
                }
            },
        );
    ...
    it('Should set the dataValue, but after it sets the value of its nested components', function () {
        return Harness.testCreate(ContainerComponent, comp2).then(
            (component) => {
    ...
    it('Should not set the default value when clearOnHide during the server-side validation', function (done) {
        ...
        Formio.createForm(element, form, { server: true, noDefaults: true })
            .then((form) => {
                form.setValue(
                    { data: { checkbox: false } },
                    {
                ...
                setTimeout(() => {
                    assert.deepEqual(
                        form._data,
                        { checkbox: false },
                        "Should not add Container's key",
                    );
                    done();
                }, 200);
            })
