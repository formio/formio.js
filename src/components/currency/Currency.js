...
export default class CurrencyComponent extends NumberComponent {
    ...
    static get builderInfo() {
        return {
            ...
            icon: 'usd',
            documentation:
                '/userguide/form-building/advanced-components#currency',
            weight: 70,
    ...
    constructor(component, options, data) {
        ...
        if (
            component &&
            !Object.prototype.hasOwnProperty.call(component, 'delimiter')
        ) {
    ...
    addZerosAndFormatValue(value) {
        ...
        const hasPrefix = this.currencyPrefix
            ? value.includes(this.currencyPrefix)
            : false;
        ...
        value = this.stripPrefixSuffix(
            isNegative ? value.replace(negativeValueSymbol, '') : value,
        );
        ...
        const formattedValue = `${isNegative ? negativeValueSymbol : ''}${
            hasPrefix ? this.currencyPrefix : ''
        }${integerPart}${this.decimalSeparator}${decimalPartNumbers.join('')}${
            hasSuffix ? this.currencySuffix : ''
        }`;
    ...
    getValueAsString(value, options) {
        ...
        if (value || value == '0') {
            if (Array.isArray(value)) {
                return value
                    .map((val) =>
                        this.addZerosAndFormatValue(
                            super.getValueAsString(val, options),
                        ),
                    )
                    .join(', ');
            }
