...
describe('Currency Component', function () {
    ...
    it('Should build a currency component', function () {
        return Harness.testCreate(CurrencyComponent, comp1).then(
            (component) => {
    ...
    it('Should place a caret between the period and the underline.', function (done) {
        Harness.testCreate(CurrencyComponent, comp3, {
            language: 'en-US',
        }).then((component) => {
            const inputEvent = new Event('input');
            const currencyElement = component.element.querySelector(
                '[name="data[currency]"]',
            );
    ...
    it('Should format value on blur for USA locale', function (done) {
        Harness.testCreate(CurrencyComponent, comp1, {
            language: 'en-US',
        }).then((component) => {
            component.root = {
                onChange: () => {},
    ...
    it('Should format value on blur for French locale', function (done) {
        Harness.testCreate(CurrencyComponent, comp1, { language: 'fr' }).then(
            (component) => {
                component.root = {
                    onChange: () => {},
                    triggerChange: () => {},
    ...
    it('Should not change entered value on blur if multiple value is set', function (done) {
        Harness.testCreate(CurrencyComponent, comp2).then((component) => {
            ...
            const firstValueElement = component.element.querySelectorAll(
                '[name="data[currency]"]',
            )[0];
            const secondValueElement = component.element.querySelectorAll(
                '[name="data[currency]"]',
            )[1];
    ...
    it('Should format currency submissions for table view for French locale', function () {
        return Harness.testCreate(CurrencyComponent, comp1, {
            language: 'fr',
        }).then((component) => {
