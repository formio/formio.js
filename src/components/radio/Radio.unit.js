                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            assert.equal(
                                _.isEqual(
                                    form.submission.metadata.selectData.radio,
                                    { name: 'Alaska' },
                                ),
                                true,
                            );
                            assert.equal(
                                form.submission.metadata.listData.radio.length,
                                3,
                            );
                            Formio.makeRequest = originalMakeRequest;
                            done();
    ...
    it('Span should have correct text label', function () {
        return Harness.testCreate(RadioComponent, comp1).then((component) => {
            component.element.querySelectorAll('input').forEach((input) => {
                assert(
                    input.getAttribute('class').indexOf('form-check-input') !==
                        -1,
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    values.forEach((value, i) => {
                        assert.equal(
                            _.isEqual(value, radio.loadedOptions[i].value),
                            true,
                        );
                    });
...
describe('Radio Component Part Two', function () {
    it('should have red asterisk left hand side to the options labels if component is required and label is hidden', function () {
        return Harness.testCreate(RadioComponent, comp7).then((component) => {
            const options =
                component.element.querySelectorAll('.form-check-label');
            options.forEach((i) => {
        ...
        Formio.createForm(formElement, JSON)
            .then((form) => {
                const component = form.getComponent(
                    'whichOneIsYourFavoriteFruit',
                );

                const appleRadioInput = component.refs.input[0];
                const appleComponentWrapper =
                    formElement.querySelector('.form-check');
