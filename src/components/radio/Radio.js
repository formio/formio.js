...
export default class RadioComponent extends ListComponent {
			...
			if (this.isRadio) {
				...
				if (this.isSelectURL && _.isObject(this.loadedOptions[index].value)) {
					...
					input.checked =
						...
						(input.value || this.component.dataSrc !== 'url');
				}
	...
	validateValueProperty() {
		...
		return !_.some(
			this.refs.wrapper,
			(wrapper, index) =>
				this.refs.input[index].checked && this.loadedOptions[index].invalid,
		);
	}
	...
	validateValueAvailability(setting, value) {
		...
		if (values) {
			return (
				values.findIndex(
					({ value: optionValue }) =>
						this.normalizeValue(optionValue) === value,
				) !== -1
			);
		}
	...
	loadItemsFromMetadata() {
		this.listData.forEach((item, i) => {
			...
			if (
				_.isEqual(item, this.selectData || _.pick(this.dataValue, _.keys(item)))
			) {
		...
		items?.forEach((item, i) => {
			this.loadedOptions[i] = {
				...
				label: this.component.valueProperty
					? this.itemTemplate(item, item[this.component.valueProperty])
					: this.itemTemplate(item, item, i),
			};
			listData.push(
				this.templateData[
					this.component.valueProperty ? item[this.component.valueProperty] : i
				],
			);

			if (
				(this.component.valueProperty || !this.isRadio) &&
