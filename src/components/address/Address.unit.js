...
describe('Address Component', function () {
    ...
    it('Should set default value and clear it on "clear icon" click (openStreetMap provider)', function (done) {
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                assert.equal(
                    address.refs.searchInput[0].value,
    ...
    it('Test manual mode', function (done) {
        ...
        form.components[0].switchToManualModeLabel =
            ...
            .then((form) => {
                ...
                address.components.forEach((comp) => {
                    assert.equal(
                        comp.visible,
                        false,
                        'Manual mode components should be hidden',
                    );
                });
                ...
                setTimeout(() => {
                    assert.equal(
                        address.refs.modeSwitcher.checked,
                        true,
                        'Manual mode should be turned on',
                    );
                    ...
                    address.components.forEach((comp) => {
                        assert.equal(
                            comp.visible,
                            true,
                            'Manual mode components should be visible',
                        );
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            const modalOverlayButton =
                                address.componentModal.refs.modalOverlay;
                            modalOverlayButton.dispatchEvent(clickEvent);

                            setTimeout(() => {
                                assert.equal(
                                    !!address.componentModal.isValueChanged(),
                                    false,
                                );
                                assert.equal(
                                    !!address.componentModal.dialogElement,
                                    false,
                                );
                                done();
                            }, 200);
