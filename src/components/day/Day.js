...
export default class DayComponent extends Field {
	...
	get monthRequired() {
		return (
			this.showMonth && _.get(this.component, 'fields.month.required', false)
		);
	}
	...
	get yearRequired() {
		return (
			this.showYear && _.get(this.component, 'fields.year.required', false)
		);
	}
	...
	inputDefinition(name) {
		...
		return {
			...
			attr: {
				...
				class: `form-control ${this.transform(
					'class',
					`formio-day-component-${name}`,
				)}`,
				type:
					this.component.fields[name].type === 'select' ? 'select' : 'number',
	...
	renderField(name) {
		if (this.component.fields[name].type === 'select') {
			return this.renderTemplate('select', {
				...
				selectOptions: this[`${name}s`].reduce(
					(html, option) =>
						html +
	...
	attach(element) {
		...
		const updateValueAndSaveFocus = (element, name) => () => {
			try {
				...
				console.warn(
					'An error occurred while trying to save caret position',
					err,
				);
			}
		...
		if (this.shouldDisabled) {
			...
			this.addEventListener(
				this.refs.day,
				'input',
				updateValueAndSaveFocus(this.refs.day, 'day'),
			);
			...
			this.addEventListener(
				this.refs.year,
				'input',
				updateValueAndSaveFocus(this.refs.year, 'year'),
			);
			this.addEventListener(this.refs.input, this.info.changeEvent, () =>
				this.updateValue(null, {
					modified: true,
				}),
			);
	...
	normalizeValue(value) {
		...
		const defaultValue = this.component.defaultValue
			? this.component.defaultValue.split('/')
			: '';
	...
	getDate(value) {
		...
		if (this.component.dayFirst) {
			result = `${day}${
				(this.showDay && this.showMonth) || (this.showDay && this.showYear)
					? '/'
					: ''
			}${month}${this.showMonth && this.showYear ? '/' : ''}${year}`;
		} else {
			result = `${month}${
				(this.showDay && this.showMonth) || (this.showMonth && this.showYear)
					? '/'
					: ''
			}${day}${this.showDay && this.showYear ? '/' : ''}${year}`;
		}
	...
	isPartialDay(value) {
		...
		return (
			values[DAY] === '00' || values[MONTH] === '00' || values[YEAR] === '0000'
		);
	}
