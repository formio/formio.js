...
export default class NumberComponent extends Input {
	...
	constructor(...args) {
		...
		if (this.component.delimiter) {
			if (
				Object.prototype.hasOwnProperty.call(this.options, 'thousandsSeparator')
			) {
				console.warn(
					"Property 'thousandsSeparator' is deprecated. Please use i18n to specify delimiter.",
				);
			}
		...
		this.decimalLimit = getNumberDecimalLimit(
			this.component,
			requireDecimal ? 2 : 20,
		);
	...
	get defaultValue() {
		...
		if (!this.component.multiple && _.isArray(defaultValue)) {
			defaultValue =
				!defaultValue[0] && defaultValue[0] !== 0 ? null : defaultValue[0];
		}
		return defaultValue;
	}
	...
	getMaskedValue(value) {
		...
		return conformToMask(this.formatValue(value), this.numberMask)
			.conformedValue;
	}
	...
	addFocusBlurEvents(element) {
		...
		this.addEventListener(element, 'blur', () => {
			element.value = this.getValueAsString(
				this.formatValue(this.parseValue(element.value)),
			);
		});
	}
}
