...
describe('PhoneNumber Component', function () {
    it('Should build a phone number component', function () {
        return Harness.testCreate(PhoneNumberComponent, comp1).then(
            (component) => {
    ...
    it('Should check mask and value in the phone component in the email template', function (done) {
        ...
        Formio.createForm(element, formJson)
            .then((form) => {
                ...
                setTimeout(() => {
                    assert.equal(
                        phoneNumber.dataValue.value,
                        'mask1',
                        'Should check value',
                    );
                    assert.equal(
                        phoneNumber.dataValue.maskName,
                        'mask2',
                        'Should check maskName',
                    );
                    const toString = phoneNumber.getValueAsString(
                        phoneNumber.dataValue,
                        { email: true },
                    );
                    assert.ok(
                    ...
                    assert.ok(
                        toString.includes(phoneNumber.dataValue.maskName),
                        'Email template should have Phone Number mackName',
                    );
                    assert.ok(
                        toString.includes(phoneNumber.dataValue.value),
                        'Email template should have Phone Number value',
                    );
                    done();
                }, 300);
            })
