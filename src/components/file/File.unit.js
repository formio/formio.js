...
describe('File Component', function () {
    it('Should create a File Component', function () {
        return Harness.testCreate(FileComponent, comp1).then((component) => {
            ...
            Harness.testElements(
                component,
                'ul.list-group-striped li.list-group-header',
                1,
            );
            Harness.testElements(
                component,
                'ul.list-group-striped li.list-group-item',
                1,
            );
            Harness.testElements(component, 'a.browse', 1);
            assert(
                component.checkValidity(component.getValue()),
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        const modalOverlayButton =
                            file.componentModal.refs.modalOverlay;
                        modalOverlayButton.dispatchEvent(clickEvent);

                        setTimeout(() => {
                            assert.equal(
                                !!file.componentModal.dialogElement,
                                true,
                            );
                            const dialogYesButton =
                                file.componentModal.dialogElement.refs
                                    .dialogYesButton;
    ...
    it('Should abort the correct file when user clicks the file remove button', function (done) {
        ...
        Harness.testCreate(FileComponent, cmp, options).then((component) => {
            ...
            setTimeout(function () {
                ...
                assert.equal(
                    component.filesToSync.filesToUpload[1].status,
                    'progress',
                );
                ...
                setTimeout(() => {
                    ...
                    assert(
                        abortedFiles[0] === 'file.1' &&
                            abortedFiles.length === 1,
                    );
                    assert.equal(
                        component.filesToSync.filesToUpload[1].status,
                        'error',
                    ...
                    assert.equal(
                        component.filesToSync.filesToUpload[1].message,
                        'Request was aborted',
                    );
