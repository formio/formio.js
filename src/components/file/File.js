...
export default class FileComponent extends Field {
    ...
    init() {
        ...
        const formDataSupported =
            typeof window !== 'undefined' ? Boolean(window.FormData) : false;
    ...
    get hasTypes() {
        return (
            ...
            (this.component.fileTypes[0].label !== '' ||
                this.component.fileTypes[0].value !== '')
        );
    }
    ...
    get shouldSyncFiles() {
        return Boolean(
            this.filesToSync.filesToDelete.length ||
                this.filesToSync.filesToUpload.length,
        );
    }
    ...
    get browseOptions() {
        ...
        if (!this.isMobile.any) {
            ...
            if (this.imageUpload && (!filePattern || filePattern === '*')) {
                ...
                this.imageUpload &&
                !filePattern.includes(imagesPattern)
            ) {
    ...
    attach(element) {
        ...
        if (this.refs.galleryButton && webViewCamera) {
            this.addEventListener(this.refs.galleryButton, 'click', (event) => {
                ...
                webViewCamera.getPicture(
                    ...
                    {
                        sourceType:
                            webViewCamera.PictureSourceType.PHOTOLIBRARY,
                    },
                );
            });
        }
        ...
        if (fileService) {
            ...
            this.refs.fileImage.forEach((image, index) => {
                loadingImages.push(
                    this.loadImage(this.dataValue[index]).then(
                        (url) => (image.src = url),
                    ),
                );
            });
    ...
    fileSize(a, b, c, d, e) {
        ...
        (d = 1024),
        (e = (c(a) / c(d)) | 0),
        a / b.pow(d, e)).toFixed(2)} ${e ? `${'kMGTPEZY'[--e]}B` : 'Bytes'}`;
    }
        ...
        if (str.length > 2 && str[0] === '/' && str[str.length - 1] === '/') {
            ...
            if (split.length > 1) {
                ...
                if (str.startsWith('!')) {
                    excludes.push(
                        `^((?!${
                            this.globStringToRegex(str.substring(1)).regexp
                        }).)*$`,
                    );
                } else {
                    if (str.startsWith('.')) {
    ...
    translateScalars(str) {
        if (typeof str === 'string') {
            ...
            if (str.search(/mb/i) === str.length - 2) {
                return parseFloat(
                    str.substring(0, str.length - 2) * 1024 * 1024,
                );
            }
            ...
            ? {
                  ...
                  message: this.t(
                      `File with the same name is already ${
                          fileWithSameNameUploading ? 'being ' : ''
                      }uploaded`,
                  ),
              }
            : {};
    ...
    async deleteFile(fileInfo) {
        ...
        if (
            fileInfo &&
            ['url', 'indexeddb', 's3', 'azure', 'googledrive'].includes(
            ...
            if (fileService && typeof fileService.deleteFile === 'function') {
                ...
                const formio =
                    this.options.formio || (this.root && this.root.formio);
    ...
    updateProgress(fileInfo, progressEvent) {
        ...
        if (fileInfo.status !== 'progress') {
            ...
            progress.innerHTML = `<span class="visually-hidden">${
                fileInfo.progress
            }% ${this.t('Complete')}</span>`;
