...
export default class ListComponent extends Field {
    ...
    itemTemplate(data, value) {
        ...
        if (template) {
            ...
            if (
                !label ||
                (hasTranslator && !this.t(label, { _userInput: true }))
            )
                return;
            return hasTranslator
                ? template.replace(label, this.t(label, { _userInput: true }))
                : label;
        } else {
            ...
            case 'resource': {
                ...
                if (
                    ...
                    try {
                        ...
                        console.warn(
                            `Unable to load resources for ${this.key}`,
                        );
                    }
                } else {
                    ...
                    request.onsuccess = (event) => {
                        ...
                        new NativePromise((resolve) => {
                            ...
                            if (!_.isEmpty(this.component.indexeddb.filter)) {
                                items = _.filter(
                                    items,
                                    this.component.indexeddb.filter,
                                );
                            }
                            this.setItems(items);
                        });
