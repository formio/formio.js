...
export default class Multivalue extends Field {
    ...
    useWrapper() {
        return (
            this.component.hasOwnProperty('multiple') && this.component.multiple
        );
    }
        ...
        if (!this.attachMultiMask(index)) {
            const applyMask = () => {
                ...
                if (
                    valueMask &&
                    displayMask &&
                    displayMask !== valueMask &&
                    this.refs.valueMaskInput
                ) {
    ...
    onSelectMaskHandler(event) {
        this.updateMask(
            event.target.maskInput,
            this.getMaskPattern(event.target.value),
        );
    }
    ...
    getMaskPattern(maskName) {
        ...
        const mask = this.component.inputMasks.find(
            (inputMask) => inputMask.label === maskName,
        );
        this.multiMasks[maskName] = mask
            ? mask.mask
            : this.component.inputMasks[0].mask;
        return this.multiMasks[maskName];
    }
