...
export default class NestedComponent extends Field {
    ...
    set parentVisible(value) {
        ...
        this.components.forEach(
            (component) => (component.parentVisible = this.visible),
        );
    }
    ...
    set disabled(disabled) {
        ...
        this.components.forEach(
            (component) => (component.parentDisabled = disabled),
        );
    }
    ...
    get ready() {
        return NativePromise.all(
            this.getComponents().map((component) => component.ready),
        );
    }
    ...
    getComponent(path, fn, originalPath) {
        ...
        this.everyComponent((component, components) => {
            const matchPath =
                component.hasInput && component.path
                    ? pathStr.includes(component.path)
                    : true;
    ...
    calculateComponentPath(component) {
        ...
        if (component.component.key) {
            ...
            const rowIndex = component.row
                ? `[${Number.parseInt(component.row)}]`
                : '';
            path = thisPath.path ? `${thisPath.path}${rowIndex}.` : '';
            path +=
                component._parentPath &&
                component.component.shouldIncludeSubFormPath
                    ? component._parentPath
                    : '';
    ...
    attach(element) {
        ...
        return NativePromise.all([superPromise, childPromise]);
    }
    ...
    checkData(data, flags, row, components) {
        ...
        row = row || this.data;
        components =
            components && _.isArray(components)
                ? components
                : this.getComponents();
    ...
    checkConditions(data, flags, row) {
        ...
        this.getComponents().forEach((comp) =>
            comp.checkConditions(data, flags, comp.data),
        );
        return check;
    }
    ...
    clearOnHide(show) {
        ...
        this.getComponents().forEach((component) =>
            component.clearOnHide(show),
        );
    }

    restoreComponentsContext() {
        this.getComponents().forEach(
            (component) => (component.data = this.dataValue),
        );
    }
    ...
    beforePage(next) {
        return NativePromise.all(
            this.getComponents().map((comp) => comp.beforePage(next)),
        );
    }
    ...
    beforeSubmit() {
        return NativePromise.all(
            this.getComponents().map((comp) => comp.beforeSubmit()),
        );
    }
    ...
    checkChildComponentsValidity(data, dirty, row, silentCheck, isParentValid) {
        return this.getComponents().reduce(
            (check, comp) =>
                comp.checkValidity(data, dirty, row, silentCheck) && check,
            isParentValid,
        );
    }
    ...
    checkAsyncValidity(data, dirty, row, silentCheck) {
        return this.ready.then(() => {
            const promises = [
                super.checkAsyncValidity(data, dirty, row, silentCheck),
            ];
            this.eachComponent((component) =>
                promises.push(
                    component.checkAsyncValidity(data, dirty, row, silentCheck),
                ),
            );
            return NativePromise.all(promises).then((results) =>
                results.reduce((valid, result) => valid && result, true),
            );
        });
    }
    ...
    destroyComponents(all = false) {
        ...
        components.forEach((comp) =>
            this.removeComponent(comp, this.components, all),
        );
        this.components = [];
    }
    ...
    get errors() {
        ...
        return this.getComponents()
            .reduce(
                (errors, comp) => errors.concat(comp.errors || []),
                thisErrors,
            )
    ...
    get dataReady() {
        return NativePromise.all(
            this.getComponents().map((component) => component.dataReady),
        );
    }
    ...
    setNestedValue(component, value, flags = {}) {
        ...
        if (component.type === 'components') {
            ...
            component.shouldAddDefaultValue
        ) {
    ...
    setValue(value, flags = {}) {
        ...
        return this.getComponents().reduce((changed, component) => {
            return (
                this.setNestedValue(component, value, flags, changed) || changed
            );
        }, false);
    }
