...
describe('NestedComponent class', function () {
        ...
        it("components array shouldn't have duplicates", function (done) {
            Harness.testCreate(NestedComponent, comp1)
                .then((nested) => {
                    ...
                    expect(_map(nested.schema.components, 'key')).to.deep.equal(
                        ['firstName', 'lastName'],
                    );
                    done();
                }, done)
        ...
        it('inside data components', function (done) {
            Harness.testCreate(NestedComponent, comp2)
                .then((nested) => {
                    ...
                    setTimeout(() => {
                        assert(
                            dataGrid.components[0].path ===
                                'dataGrid[0].textField',
                        );
                        assert(
                            dataGrid.components[1].path ===
        ...
        it('inside nested forms', function (done) {
            ...
            form.setForm(nestedForm)
                .then(() => {
                    ...
                    assert(
                        dataGrid.components[0].path ===
                            'form.data.dataGrid[0].textField',
                    );
    ...
    describe('getComponent', function () {
        it('the first layer components', function (done) {
            Harness.testCreate(NestedComponent, comp1)
                .then((nested) => {
                    const firstNameTextFieldByStringPath =
                        nested.getComponent('firstName');
        ...
        it('inside data components', function (done) {
            Harness.testCreate(NestedComponent, comp2)
                .then((nested) => {
                    ...
                    dataGrid.setValue([{ textField: '' }, { textField: '' }]);
                    setTimeout(() => {
                        const dataGridFirstRowTextField = nested.getComponent(
                            'dataGrid[0].textField',
                        );
                        const dataGridSecondRowTextField = nested.getComponent(
                            'dataGrid[1].textField',
                        );

                        assert(
                            dataGrid.components[0] ===
                                dataGridFirstRowTextField,
                        );
                        assert(
                            dataGrid.components[1] ===
                                dataGridSecondRowTextField,
                        );
                        done();
                    }, 250);
                })
        ...
        it('inside nested forms', function (done) {
            ...
            form.setForm(nestedForm)
                .then(() => {
                    ...
                    const dataGridTextField = form.getComponent(
                        'form.data.dataGrid[0].textField',
                    );
                    ...
                    assert(
                        dataGridTextField ===
                            childForm.components[1].components[0],
                    );
                    done();
                })
                .catch(done);
