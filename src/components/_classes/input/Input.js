...
export default class Input extends Multivalue {
    constructor(component, options, data) {
        ...
        this.triggerUpdateValueAt = _.debounce(
            this.updateValueAt.bind(this),
            100,
        );
    }
    ...
    get inputInfo() {
        ...
        if (this.component.placeholder) {
            attr.placeholder = this.getFormattedAttribute(
    ...
    get remainingWords() {
        const maxWords = _.parseInt(
            _.get(this.component, 'validate.maxWords'),
            10,
        );
    ...
    get suffix() {
        if (
            ...
            this.component.widget.type === 'calendar'
        ) {
            const calendarIcon = this.renderTemplate('icon', {
                ...
                className: this.iconClass(
                    this.component.enableDate ||
                        this.component.widget.enableDate
                        ? 'calendar'
                        : 'clock-o',
                ),
                styles: '',
    ...
    renderElement(value, index) {
        ...
        info.attr = info.attr || {};
        info.attr.value = this.getValueAsString(
        ...
        const hasDifferentDisplayAndSaveFormats =
            valueMask && displayMask && valueMask !== displayMask;
    ...
    setCounter(type, element, count, max) {
        if (max) {
            ...
            this.setContent(
                element,
                this.t(`{{ remaining }} ${type} remaining.`, {
                    remaining: remaining,
                }),
            );
        } else {
    ...
    updateValueAt(value, flags, index) {
        ...
        if (_.get(this.component, 'showWordCount', false)) {
            if (this.refs.wordcount && this.refs.wordcount[index]) {
                const maxWords = _.parseInt(
                    _.get(this.component, 'validate.maxWords', 0),
                    10,
                );
                this.setCounter(
                    this.t('words'),
        ...
        if (_.get(this.component, 'showCharCount', false)) {
            if (this.refs.charcount && this.refs.charcount[index]) {
                const maxChars = _.parseInt(
                    _.get(this.component, 'validate.maxLength', 0),
                    10,
                );
                this.setCounter(
                    this.t('characters'),
                    this.refs.charcount[index],
                    value.length,
                    maxChars,
                );
            }
    ...
    createWidget(index) {
        ...
        // Create the widget.
        const widget = new Widgets[settings.type](
            settings,
            this.component,
            this,
            index,
