...
export default class ComponentModal {
    ...
    constructor(component, element, isOpened, currentValue) {
        ...
        this.currentValue = fastCloneDeep(
            currentValue ?? this.component.getValue(),
        );
    ...
    setOpenModalElement(template) {
        if (this.component?.visible) {
            ...
            if (this.isOpened) {
                this.refs.modalWrapper.classList.add(
                    'formio-dialog-disabled-animation',
                );
                this.openModal();
            }
        }
    ...
    removeEventListeners() {
        ...
        this.component.removeEventListener(
            ...
            this.refs.modalSave
                ...
                : this.saveModalListener,
        );
        this.component.removeEventListener(
            this.refs.modalClose,
            'click',
            this.showDialogListener,
        );
        this.component.removeEventListener(
    ...
    positionOverElement() {
        ...
        const modalHeight =
            this.refs.modalContents.getBoundingClientRect().height;
    ...
    closeModal() {
        this.refs.modalWrapper.classList.remove(
            'formio-dialog-disabled-animation',
        );
    ...
    closeModalHandler(event) {
        ...
        if (!this.component.disabled) {
            this.component.setValue(_.cloneDeep(this.currentValue), {
                resetValue: true,
            });
        }
        this.closeModal();
    ...
    showDialog() {
        ...
        const dialogContent = `
      <h3 ref="dialogHeader">${this.component.t(
          'Do you want to clear changes?',
      )}</h3>
      <div style="display:flex; justify-content: flex-end;">
        <button ref="dialogCancelButton" class="btn btn-secondary">${this.component.t(
            'Cancel',
        )}</button>
        <button ref="dialogYesButton" class="btn btn-danger">${this.component.t(
            'Yes, delete it',
        )}</button>
      </div>
    `;
    ...
    saveModalValueHandler(event) {
        event.preventDefault();
        this.currentValue = fastCloneDeep(
            this.component.dataValue ?? this.component.getValue(),
        );
        this.closeModal();
    }
}
