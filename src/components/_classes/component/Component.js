...
export default class Component extends Element {
	...
	constructor(component, options, data) {
		 ...
		 * @type {boolean}
		 */
		this._disabled = boolValue(this.component.disabled)
		 ...
		 */
		this._parentVisible = Object.prototype.hasOwnProperty.call(
			this.options,
			'parentVisible',
		)
			...
			: true;
		this._visible =
			this._parentVisible && this.conditionallyVisible(null, data);
	...
	createAddon(addonConfiguration) {
		...
		if (Addon) {
			...
			const supportsThisComponentType =
	...
	get shouldDisabled() {
		return (
			this.options.readOnly ||
			this.component.disabled ||
			(Object.prototype.hasOwnProperty.call(this.options, 'disabled') &&
	...
	get isInputComponent() {
		return (
			!Object.prototype.hasOwnProperty.call(this.component, 'input') ||
    ...
    `;
      ...
      ${
				this.component.hideLabel
					? `max-width: ${100 - this.labelWidth - this.labelMargin}`
					: ''
			};
    `;
	...
	getModifiedSchema(schema, defaultSchema, recursion) {
		...
		_.each(schema, (val, key) => {
			if (
				!_.isArray(val) &&
				_.isObject(val) &&
				Object.prototype.hasOwnProperty.call(defaultSchema, key)
			) {
				const subModified = this.getModifiedSchema(
					val,
					defaultSchema[key],
					true,
				);
	 ...
	 */
	get schema() {
		return fastCloneDeep(
			this.getModifiedSchema(_.omit(this.component, 'id'), this.defaultSchema),
		);
	}
	...
	getFormattedAttribute(attr) {
		return attr
			? this.t(attr, { _userInput: true }).replace(/"/g, '&quot;')
			: '';
	}
	...
	get widget() {
		...
		const widget =
			settings && Widgets[settings.type]
	...
	get submissionTimezone() {
		this.options.submissionTimezone =
			this.options.submissionTimezone ||
			_.get(this.root, 'options.submissionTimezone');
	...
	render(
		children = `Unknown component: ${this.component.type}`,
		topLevel = false,
	) {
	...
	attachTooltips(toolTipsRefs) {
		toolTipsRefs?.forEach((tooltip, index) => {
			if (tooltip) {
				...
				const tooltipText = this.interpolate(
					tooltipDataTitle || tooltipAttribute,
				).replace(/(?:\r\n|\r|\n)/g, '<br />');
	...
	checkRefresh(refreshData, changed, flags) {
		...
		if (refreshData === 'data') {
			...
			changePath &&
			getComponentPath(changed.instance) === refreshData &&
			changed &&
			changed.instance &&
			...
			if (Array.isArray(refreshOn)) {
				refreshOn.forEach((refreshData) =>
					changes.forEach((changed) =>
						this.checkRefresh(refreshData, changed, flags),
					),
				);
			} else {
				changes.forEach((changed) =>
					this.checkRefresh(refreshOn, changed, flags),
				);
			}
		}
	}
	...
	getWidgetValueAsString(value, options) {
		const noInputWidget =
			!this.refs.input || !this.refs.input[0] || !this.refs.input[0].widget;
	 ...
	 */
	get className() {
		let className = this.hasInput
			? `${this.transform('class', 'form-group')} has-feedback `
			: '';
	...
	restoreCaretPosition() {
		if (this.root?.currentSelection) {
			if (this.refs.input?.length) {
				...
				const isInputRangeSelectable = (i) =>
					/text|search|password|tel|url/i.test(i?.type || '');
				if (input) {
	...
	fieldLogic(data, row) {
		...
		let changed = logics.reduce((changed, logic) => {
			...
			return (
				(result
					? this.applyActions(newComponent, logic.actions, result, row, data)
					: false) || changed
			);
		}, false);
		...
		return actions.reduce((changed, action) => {
			switch (action.type) {
				case 'property': {
					...
					if (
						!_.isEqual(
							_.get(this.component, property),
							_.get(newComponent, property),
						)
					) {
						changed = true;
					}
				...
				case 'value': {
					...
					if (
						...
						!(this.component.clearOnHide && !this.visible)
					) {
	...
	addCKE(element, settings, onChange) {
		...
		settings = _.merge(
			...
			_.get(this.options, 'editors.ckeditor.settings', {}),
			settings,
		);

		if (this.component.isUploadEnabled) {
			settings.extraPlugins.push(
				getFormioUploadAdapterPlugin(this.fileService, this),
			);
		}
	...
	addQuill(element, settings, onChange) {
		...
		Formio.requireLibrary(
			...
			[
				{
					type: 'styles',
					src: `${Formio.cdn.quill}/quill.${settings.theme}.css`,
				},
			],
			true,
		);
		...
		return Formio.requireLibrary(
			...
			return Formio.requireLibrary(
				'quill-table',
				'Quill',
				`${Formio.cdn.baseUrl}/quill/quill-table.js`,
				true,
			).then(() => {
				if (!element.parentNode) {
					return Promise.reject();
	...
	addAce(element, settings, onChange) {
		...
		settings = _.merge(
			...
			_.get(this.options, 'editors.ace.settings', {}),
			settings || {},
		);
		return Formio.requireLibrary(
			'ace',
			'ace',
			_.get(this.options, 'editors.ace.src', `${Formio.cdn.ace}/ace.js`),
			true,
		).then((editor) => {
	...
	splice(index, flags = {}) {
		if (this.hasValue()) {
			...
			if (
				_.isArray(dataValue) &&
				Object.prototype.hasOwnProperty.call(dataValue, index)
			) {
		...
		const checkMask = (value) => {
			if (typeof value === 'string') {
				if (this.component.type !== 'textfield') {
					...
					value = conformToMask(value, this.defaultMask, {
						placeholderChar,
					}).conformedValue;
	...
	updateComponentValue(value, flags = {}) {
		...
		let changed =
			newValue !== undefined ? this.hasChanged(newValue, oldValue) : false;
		if (changed) {
			this.dataValue = newValue;
	...
	calculateComponentValue(data, flags, row) {
		...
		if (shouldBeCleared) {
			...
			if (
				Object.prototype.hasOwnProperty.call(this, 'calculatedValue') &&
				allowOverride
			) {
	...
	setComponentValidity(messages, dirty, silentCheck) {
		const hasErrors = !!messages.filter(
			(message) => message.level === 'error' && !message.fromServer,
		).length;
		if (
			messages.length &&
	...
	checkComponentValidity(data, dirty, row, options = {}) {
		...
		return async
			? validations.then((messages) =>
					this.setComponentValidity(messages, dirty, silentCheck),
				)
	...
	checkValidity(data, dirty, row, silentCheck) {
		data = data || this.rootValue;
		row = row || this.data;
		const isValid = this.checkComponentValidity(data, dirty, row, {
			silentCheck,
		});
		this.checkModal();
		return isValid;
	...
	isEmpty(value = this.dataValue) {
		const isEmptyArray =
			_.isArray(value) && value.length === 1
				? _.isEqual(value[0], this.emptyValue)
				: false;
		return (
			value == null ||
			value.length === 0 ||
	...
	setCustomValidity(messages, dirty, external) {
		...
		const hasErrors = !!messages.filter((message) => message.level === 'error')
			.length;
	...
	isValueHidden() {
		...
		if (
			!this.root ||
			!Object.prototype.hasOwnProperty.call(this.root, 'editing')
		) {
			return false;
		}
	...
	getRelativePath(path) {
		...
		const thisPath = this.isInputComponent
			? this.path
	...
	autofocus() {
		const hasAutofocus =
			this.component.autofocus && !this.builderMode && !this.options.preview;
