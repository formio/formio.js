...
export default class Component extends Element {
    ...
    checkTemplate(templates, names, modes) {
        for (const name of names) {
            ...
            if (templatesByName) {
                const templateByMode = this.checkTemplateMode(
                    templatesByName,
                    modes,
                );
                if (templateByMode) {
    ...
    getModalPreviewTemplate() {
        ...
        return this.renderTemplate('modalPreview', {
            previewText:
                this.getValueAsString(dataValue, { modalPreview: true }) ||
    ...
    getWidgetValueAsString(value, options) {
        ...
        if (Array.isArray(value)) {
            ...
            value.forEach((val, index) => {
                const widget =
                    this.refs.input[index] && this.refs.input[index].widget;
                if (widget) {
        ...
        this.logic.forEach((logic) => {
            if (logic.trigger.type === 'event') {
                ...
                this.on(
                    event,
                    (...args) => {
                        const newComponent = fastCloneDeep(
                            this.originalComponent,
                        );
                        if (
                            this.applyActions(newComponent, logic.actions, args)
                        ) {
                            // If component definition changed, replace it.
                            if (!_.isEqual(this.component, newComponent)) {
                                this.component = newComponent;
                                const visible = this.conditionallyVisible(
                                    null,
                                    null,
                                );
                                const disabled = this.shouldDisabled;
...
Component.requireLibrary = function (name, property, src, polling) {
    if (
        ...
        Component.externalLibraries[name].ready = new Promise(
            (resolve, reject) => {
