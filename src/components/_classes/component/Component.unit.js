...
describe('Component', function () {
		...
		const formJson = {
			components: [
				...
				},
				{
					...
					validate: {
						custom:
							"valid = input <= 10 ? true : 'B should be less or equal to 10';",
					},
					key: 'b',
					type: 'number',
					input: true,
		...
		form
			...
			.then(() => {
				setTimeout(() => {
					...
					expect(email.refs.input[0].classList.contains('is-invalid')).to.be
						.false;
					expect(email.refs.input[1].classList.contains('is-invalid')).to.be
						.true;
					expect(email.refs.input[2].classList.contains('is-invalid')).to.be
						.true;
					done();
				}, 300);
			})
	...
	it('Should sanitize HTML even if options.pdf is set', function (done) {
		...
		assert.equal(
			component.sanitize('<a href="javascript:console.log("untrusted")></a>'),
			'<a></a>',
		);
		done();
	});
