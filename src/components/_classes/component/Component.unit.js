...
describe('Component', function () {
    ...
    it('Should build a base component', function () {
        return Harness.testCreate(Component, { type: 'base' }).then(
            (component) => {
                const element =
                    component.element.querySelector('[ref="component"]');
                assert.equal(
                    element.textContent.trim(),
                    'Unknown component: base',
                );
            },
        );
    });
            ...
            .then(() => {
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        expect(
                            b.refs.messageContainer?.innerHTML.indexOf(
                                'B should be less or equal to 10',
                            ) > -1,
                        ).to.be.true;
                        expect(b.refs.input[0].classList.contains('is-invalid'))
                            .to.be.true;
                        done();
                    }, 300);
                }, 300);
    ...
    describe('Component Modal', function () {
        it('Modal window should stay opened after redrawing component if it was opened ont hte moment of calling', function (done) {
            Harness.testCreate(Component, comp3)
                .then((component) => {
                    ...
                    component
                        ...
                        .then(() => {
                            const isVisible =
                                !component.componentModal.refs.modalWrapper.classList.contains(
        ...
        form.setForm(formJson)
            ...
            .then(() => {
                setTimeout(() => {
                    ...
                    expect(email.refs.input[0].classList.contains('is-invalid'))
                        .to.be.false;
                    expect(email.refs.input[1].classList.contains('is-invalid'))
                        .to.be.true;
                    expect(email.refs.input[2].classList.contains('is-invalid'))
                        .to.be.true;
                    done();
                }, 300);
            })
    ...
    it('Should sanitize HTML even if options.pdf is set', function (done) {
        ...
        assert.equal(
            component.sanitize(
                '<a href="javascript:console.log("untrusted")></a>',
            ),
            '<a></a>',
        );
        done();
    ...
    describe('shouldDisplayRedAsterisk', function () {
        it('modalPreview template should have className "field-required" if component is required', function (done) {
            Harness.testCreate(
                ...
                .then((cmp) => {
                    assert.equal(
                        !!cmp.element.querySelector('.field-required'),
                        true,
                    );
                    done();
                }, done)
            ...
            .then(() => {
                setTimeout(() => {
                    assert.equal(
                        window._ee,
                        undefined,
                        'Should not execute code inside Tooltips/Description',
                    );
                    done();
                }, 200);
            })
