...
describe('TextArea Component', function () {
    it('Should build a TextArea component', function () {
        return Harness.testCreate(TextAreaComponent, comp1).then(
            (component) => {
    ...
    it('setValue should be called only once', function () {
        return Harness.testCreate(TextAreaComponent, comp2).then(
            (component) => {
                const valueToSet = [
                    {
                        firstName: 'Bobby',
    ...
    it('Should provide min/max length validation', function (done) {
        ...
        testValidity(
            invalidMin,
            false,
            'Text Area must have at least 5 characters.',
        );
        testValidity(
            invalidMax,
            false,
            'Text Area must have no more than 10 characters.',
    ...
    it('Should provide min/max words validation', function (done) {
        ...
        testValidity(
            invalidMin,
            false,
            'Text Area must have at least 2 words.',
        );
        testValidity(
            invalidMax,
            false,
            'Text Area must have no more than 5 words.',
            invalidMax[invalidMax.length - 1],
        );
    });
        ...
        Formio.createForm(element, form, {
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    ...
                    assert.equal(
                        textArea.refs.input[0].innerHTML,
                        '<b>HTML!</b>',
                    );
                    done();
                }, 300);
        ...
        Formio.createForm(element, form, {
            ...
            .then((form) => {
                ...
                setTimeout(() => {
                    const textArea = form.getComponent('textArea');
                    assert.equal(
                        textArea.refs.input[0].innerText,
                        '<b>Plain text!</b>',
                    );
                    done();
                }, 300);
        ...
        Formio.createForm(element, form)
            .then((form) => {
                ...
                const checkValue = (value, expected) => {
                    ...
                    assert.equal(
                        parseInt(component.refs.wordcount[0].textContent),
                        expected,
                        'Should show correct words number',
                    );
                    assert.equal(
    ...
    describe('Rich text editors', function () {
        describe('CKEditor', function () {
            it('Should allow to insert media fiels and show the in them read-only mode', function (done) {
                ...
                Formio.createForm(element, formWithCKEditor, { readOnly: true })
                    .then((form) => {
                        form.submission = {
                            data: {
                                textArea: `
                ...
                <div data-oembed-url="https://www.youtube.com/watch?v=FmA6U5rXl38&amp;t=111s">
                  <div style="position: relative; padding-bottom: 100%; height: 0; padding-bottom: 56.2493%;">
                    <iframe src="https://www.youtube.com/embed/FmA6U5rXl38" style="position: absolute; width: 100%; height: 100%; top: 0; left: 0;" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen="">
                        ...
                        setTimeout(() => {
                            const mediaA = form.element.querySelector(
                                'iframe[src="https://www.youtube.com/embed/GsLRrmnJXF8"]',
                            );
                            const mediaB = form.element.querySelector(
                                'iframe[src="https://www.youtube.com/embed/FmA6U5rXl38"]',
                            );
        ...
        it('Should clear value in the editor on Reset', function (done) {
            ...
            Formio.createForm(element, formWithRichTextAreas)
                .then((form) => {
                    form.setValue({
                        data: {
                            textArea: 'Test',
                    ...
                    setTimeout(() => {
                        ...
                        const textAreaElement =
                            plainTextArea.element.querySelector('textarea');
        ...
        it('Should not autofocus until the editor is ready', function (done) {
            ...
            Formio.createForm(element, testForm)
                .then((form) => {
                    const textArea = form.getComponent('textArea');
        ...
        it('Should not autofocus if the form is readOnly', function (done) {
            ...
            Formio.createForm(element, testForm, { readOnly: true })
                .then((form) => {
                    const textArea = form.getComponent('textArea');
