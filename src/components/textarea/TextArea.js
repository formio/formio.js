...
export default class TextAreaComponent extends TextFieldComponent {
    ...
    updateEditorValue(index, newValue) {
        ...
        if (
            !_.isEqual(newValue, dataValue) &&
            (!_.isEmpty(newValue) || !_.isEmpty(dataValue))
        ) {
            this.updateValue(
                newValue,
                {
    ...
    attachElement(element, index) {
        if (
            this.autoExpand &&
            (this.isPlain || this.options.readOnly || this.options.htmlView)
        ) {
        ...
        let settings = _.isEmpty(this.component.wysiwyg)
            ? this.wysiwygDefault[this.component.editor] ||
              this.wysiwygDefault.default
            ...
            switch (this.component.editor) {
                case 'ace':
                    ...
                    settings.mode = this.component.as
                        ...
                        : 'ace/mode/javascript';
                    this.addAce(element, settings, (newValue) =>
                        ...
                        .then((ace) => {
                            ...
                            dataValue =
                                ...
                                Array.isArray(dataValue)
                                    ...
                                    : dataValue;
                            ace.setValue(
                                this.setConvertedValue(dataValue, index),
                            );
                            editorReady(ace);
                            return ace;
                        })
                ...
                case 'quill':
                    ...
                    if (
                        settings.hasOwnProperty('toolbarGroups') ||
                        settings.hasOwnProperty('toolbar')
                    ) {
                        console.warn(
                    ...
                    this.addQuill(element, settings, () =>
                        this.updateEditorValue(
                            index,
                            this.editors[index].root.innerHTML,
                        ),
                    )
                        .then((quill) => {
                            ...
                            if (this.component.isUploadEnabled) {
                                const _this = this;
                                quill.getModule('uploader').options.handler =
                                    function (...args) {
                ...
                case 'ckeditor':
                    ...
                    this.addCKE(element, settings, (newValue) =>
                        ...
                        let dataValue = this.dataValue;
                        dataValue =
                            this.component.multiple && Array.isArray(dataValue)
                                ? dataValue[index]
                                : dataValue;
                        ...
                        if (getBrowserInfo().ie) {
                            ...
                            if (
                                ...
                                _.has(
                                    editor,
                                    'ui.view.editable.editableElement',
                                )
                            ) {
    ...
    get htmlView() {
        return (
            ...
            (this.component.editor || this.component.wysiwyg)
        );
    }
            ...
            const setEditorsValue = (flags) => (editor) => {
                if (!flags.skipWysiwyg) {
                    ...
                    switch (this.component.editor) {
                        case 'ace':
                            editor.setValue(
                                this.setConvertedValue(value, index),
                            );
                            break;
                        case 'quill':
                            if (this.component.isUploadEnabled) {
                                this.setAsyncConvertedValue(value).then(
                                    (result) => {
                                        const content =
                                ...
                                const convertedValue = this.setConvertedValue(
                                    value,
                                    index,
                                );
        ...
        if (this.isPlain || this.options.readOnly || this.disabled) {
            value =
                this.component.multiple && Array.isArray(value)
                    ? value.map((val, index) =>
                          this.setConvertedValue(val, index),
                      )
                    : this.setConvertedValue(value);
            return super.setValue(value, flags);
        }
        flags.skipWysiwyg =
            value === '' && flags.resetValue
                ? false
        ...
        if (this.options.readOnly || this.disabled) {
            if (this.refs.input && this.refs.input[index]) {
                if (this.component.inputFormat === 'plain') {
                    this.refs.input[index].innerText = this.interpolate(
                        value,
                        {},
                        { noeval: true },
                    );
    ...
    hasChanged(newValue, oldValue) {
        return super.hasChanged(
            this.trimBlanks(newValue),
            this.trimBlanks(oldValue),
        );
    }
        ...
        switch (this.component.editor) {
            case 'ckeditor': {
                ...
                this.editorsReady[0]
                    ...
                    .catch((err) => {
                        console.warn(
                            'An editor did not initialize properly when trying to focus:',
                            err,
                        );
                    });
                break;
