...
export default class TabsComponent extends NestedComponent {
    ...
    get schema() {
        ...
        schema.components = components.map((tab, index) => {
            tab.components = this.tabs[index].map(
    ...
    attach(element) {
        ...
        ['change', 'error'].forEach((event) =>
            this.on(event, this.handleTabsValidation.bind(this)),
        );
        ...
        this.refs[this.tabKey].forEach((tab, index) => {
            this.attachComponents(
                tab,
                this.tabs[index],
                this.component.components[index].components,
            );
        });
        return superAttach;
    }
    ...
    setTab(index) {
        ...
        _.each(this.refs[this.tabLinkKey], (tabLink, tabIndex) => {
            ...
            if (this.refs[this.tabLikey][tabIndex]) {
                ...
                this.removeClass(
                    this.refs[this.tabLikey][tabIndex],
        ...
        if (this.refs[this.tabLinkKey][index]) {
            ...
            this.addClass(
                this.refs[this.tabLinkKey][index],
    ...
    handleTabsValidation() {
        if (
            ...
            !this.refs[this.tabLinkKey].length ||
            !this.tabs.length
        ) {
            return;
        }
        ...
        const invalidTabsIndexes = this.tabs.reduce(
            (invalidTabs, tab, tabIndex) => {
                const hasComponentWithError = tab.some((comp) => !!comp.error);
                return hasComponentWithError
                    ? [...invalidTabs, tabIndex]
                    : invalidTabs;
            },
            [],
        );
        ...
        const invalidTabs = [...this.refs[this.tabLinkKey]].filter(
            (_, tabIndex) => invalidTabsIndexes.includes(tabIndex),
        );
