...
describe('EditGrid Component', function () {
	it('Should set correct values in dataMap inside editGrid and allow aditing them', function (done) {
		Harness.testCreate(EditGridComponent, comp4).then((component) => {
			...
			setTimeout(() => {
				...
				setTimeout(() => {
					const keyValue =
						component.element.querySelectorAll('[ref="input"]')[0].value;
					const valueValue =
						component.element.querySelectorAll('[ref="input"]')[1].value;
	...
	describe('Display As Modal', function () {
				...
				.then(() => {
					...
					setTimeout(() => {
						...
						setTimeout(() => {
							...
							const areRequiredComponentsHaveErrorWrapper = components.every(
								(comp) => {
									const { className } = comp;
									return (
										(className.includes('required') &&
											className.includes('formio-error-wrapper')) ||
										true
									);
								},
							);
				...
				.then(() => {
					...
					setTimeout(() => {
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								const alertAfterFixingField = dialog.querySelector(
									'.alert.alert-danger',
								);
								assert.equal(form.errors.length, 2);

								const errorsLinksAfterFixingField =
									alertAfterFixingField.querySelectorAll('li');
		...
		it('Should not produce many components in Edit view when minLength validation set', function (done) {
			const formElement = document.createElement('div');
			Formio.createForm(formElement, comp15, {
				attachMode: 'builder',
			})
				.then((form) => {
					const editGrid = form.components[0];
			...
			form
				...
				.then(() => {
					...
					setTimeout(() => {
						...
						setTimeout(() => {
							assert.equal(
								!!document.querySelector('[ref="dialogContents"]'),
								false,
							);
							done();
						}, 100);
										...
										setTimeout(() => {
											editGrid.editRow(0).then(() => {
												...
												setTimeout(() => {
													...
													const hasError =
														textField.className.includes('has-error');
													assert(
														!hasError,
						...
						setTimeout(() => {
							...
							setTimeout(() => {
								...
								setTimeout(() => {
									...
									assert(
										textField.element.className.includes('has-error'),
									...
									setTimeout(() => {
										...
										assert.equal(textField.dataValue, 'new value');
										assert(
											!textFieldEl.className.includes('has-error'),
											'Should remove an error class from component when it was fixed',
										);
										const editRow = editGrid.editRows[0];
										const textField2 =
											editRow.components[0].getComponent('textField2');
											...
											setTimeout(() => {
												...
												setTimeout(() => {
													...
													setTimeout(() => {
														...
														setTimeout(() => {
															const dialog =
																editGrid.editRows[0].confirmationDialog;
	...
	it('Should use custom text for save/cancel/add btns', function (done) {
		...
		Formio.createForm(element, form)
			.then((form) => {
				const editGrid = form.getComponent('editGrid');
				assert.equal(
					editGrid.refs['editgrid-editGrid-addRow'][0].textContent.trim(),
					'add custom',
				);
				const clickEvent = new Event('click');
	...
	it('Should render headers when openWhenEmpry is enabled', function (done) {
		...
		Formio.createForm(element, form)
			.then((form) => {
				...
				const headerEls =
					editGrid.element.querySelector('.list-group-header').firstElementChild
						.children;
			...
			.then((form) => {
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						...
						editGrid.refs['editgrid-editGrid-saveRow'][0].dispatchEvent(
							clickEvent,
						);
