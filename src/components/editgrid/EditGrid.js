...
export default class EditGridComponent extends NestedArrayComponent {
  ...
  `;
	...
	attach(element) {
		...
		this.rowElements.forEach((row, rowIndex) => {
			...
			if (this.isOpen(editRow)) {
				...
				this.addEventListener(this.saveRowElements[openRowCount], 'click', () =>
							...
							this.editRow(rowIndex).then(() => {
								if (this.component.rowDrafts) {
									...
									const shouldShowRowErrorsAlert =
										this.component.modal && hasErrors && this.root?.submitted;
	...
	displayComponentValue(component, header) {
		return !!((!Object.prototype.hasOwnProperty.call(component, 'tableView') ||
			component.tableView) &&
		header
		...
		if (this.isOpen(row)) {
			...
			return this.renderString(rowTemplate, {
				...
				isVisibleInRow: (component) =>
					this.isComponentVisibleInRow(component, flattenedComponents),
