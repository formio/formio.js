...
export default class EditGridComponent extends NestedArrayComponent {
    ...
    <div style="display:flex; justify-content: flex-end;">
      <button ref="dialogCancelButton" class="btn btn-secondary" aria-label="${this.t(
          'Cancel',
      )}">${this.t('Cancel')}</button>
      <button ref="dialogYesButton" class="btn btn-danger" aria-label="${this.t(
          'Yes, delete it',
      )}">${this.t('Yes, delete it')}</button>
    </div>
  `;
    }
    ...
    get defaultValue() {
        ...
        _.times(this.minLength - defaultValue.length, () =>
            defaultValue.push({}),
        );
    ...
    init() {
        ...
        if (openWhenEmpty) {
            ...
            this.editRows = dataValue.map((row, rowIndex) => ({
                components: this.lazyLoad
                    ? []
                    : this.createRowComponents(row, rowIndex),
                data: row,
    ...
    checkRowVariableTypeComponents(editRow, rowIndex) {
        ...
        if (
            _.some(this.variableTypeComponentsIndexes, (compIndex) => {
                const variableTypeComp = rowComponents[compIndex];
                return (
                    variableTypeComp.type !== variableTypeComp.component.type
                );
            })
        ) {
            editRow.components = this.createRowComponents(
                editRow.data,
                rowIndex,
                true,
            );
    ...
    isComponentVisibleInSomeRow(component) {
        ...
        const savedRows = rows.filter((row) =>
            _.includes(savedStates, row.state),
        );
        ...
        const isOpenRowWhenEmpty =
            _.get(this.component, 'openWhenEmpty') &&
            rows.length === 1 &&
                ...
                this.renderTemplate(templateName, {
                    ...
                    header: this.renderString(headerTemplate, {
                        displayValue: (component) =>
                            this.displayComponentValue(component, true),
                        components: this.component.components,
                        value: dataValue,
                        t,
                    }),
                    footer: this.renderString(
                        _.get(this.component, 'templates.footer'),
                        {
                            components: this.component.components,
                            value: dataValue,
                            t,
    ...
    renderComponents(components) {
        ...
        const templateName =
            this.displayAsTable && this.prevHasAddButton
                ? 'tableComponents'
                : 'components';
    ...
    attach(element) {
        ...
        this.rowElements.forEach((row, rowIndex) => {
            ...
            if (this.isOpen(editRow)) {
                ...
                this.addEventListener(
                    this.saveRowElements[openRowCount],
                    'click',
                    () => this.saveRow(rowIndex, true),
                );
                this.addEventListener(
                    this.cancelRowElements[openRowCount],
                    'click',
                ...
                [
                        ...
                        action: () => {
                            this.editRow(rowIndex).then(() => {
                                if (this.component.rowDrafts) {
                                    ...
                                    const shouldShowRowErrorsAlert =
                    ...
                    },
                    {
                        ...
                        action: () => {
                            ...
                            if (
                                Array.from(row.classList).includes('selected')
                            ) {
        ...
        if (openRowCount) {
            this.addClass(
                this.refs.component,
                `formio-component-${this.component.type}-row-open`,
            );
        } else {
            this.removeClass(
                this.refs.component,
    ...
    flattenRowDataValue(dataValue) {
        ...
        Object.keys(dataValue).forEach((key) => {
            if (_.isObject(dataValue[key]) && !_.isNil(dataValue[key])) {
                Object.assign(
                    flattened,
                    this.flattenRowDataValue(dataValue[key]),
                );
            } else {
    ...
    renderRow(row, rowIndex) {
        ...
        if (this.isOpen(row)) {
            ...
            return this.renderString(rowTemplate, {
                ...
                flattenedComponents,
                displayValue: (component) =>
                    this.displayComponentValue(component),
                isVisibleInRow: (component) =>
                    this.isComponentVisibleInRow(
                        component,
                        flattenedComponents,
                    ),
                getView: (component, data) => {
                    const instance = flattenedComponents[component.key];
                    const view = instance
                        ? instance.getView(data || instance.dataValue)
                        : '';
                    ...
                    return typeof view === 'string' &&
                        ...
                        ? `<input type="text" value="${view.replace(
                              /"/g,
                              '&quot;',
                          )}" readonly/>`
                        : view;
    ...
    getComponents(rowIndex) {
        ...
        return this.builderMode
            ...
            : _.isNumber(rowIndex)
              ...
              : this.editRows.reduce(
                    (result, row) => result.concat(row.components || []),
                    [],
                );
    }
    ...
    addRowModal(rowIndex) {
        ...
        dialog.refs.dialogContents.appendChild(
            this.ce(
                'button',
                {
                    ...
                    onClick: () => {
                        ...
                        if (
                            this.validateRow(editRow, true) ||
                            this.component.rowDrafts
                        ) {
    ...
    showDialog(rowIndex) {
        ...
        const dialogContent =
            this.component.dialogTemplate || this.defaultDialogTemplate;
        ...
        this.addEventListener(
            wrapper.refs.dialogCancelButton,
            'click',
            (event) => {
                close(event);
                dialogResult.reject();
    ...
    editRow(rowIndex) {
        ...
        editRow.state = this.options.readOnly
            ? EditRowState.Viewing
            : EditRowState.Editing;
    ...
    saveRow(rowIndex, modified) {
            ...
            switch (editRow.state) {
                case EditRowState.New: {
                    ...
                    editRow.components.forEach(
                        (component) => (component.rowIndex = newIndex),
                    );
        ...
        editRow.state =
            this.component.rowDrafts && !isRowValid
                ? EditRowState.Draft
                : EditRowState.Saved;
        editRow.backup = null;
        ...
        if (this.component.rowDrafts) {
            editRow.components.forEach((comp) =>
                comp.setPristine(this.pristine),
            );
        }
        this.checkValidity(null, true);
        this.redraw();
    ...
    createRowComponents(row, rowIndex, recreatePartially) {
        ...
        const currentRowComponents = _.get(
            this.editRows,
            `[${rowIndex}].components`,
            null,
        );
        return this.component.components.map((col, colIndex) => {
            if (
                ...
                this.variableTypeComponentsIndexes.length
            ) {
                const currentComp = currentRowComponents[colIndex];
                const shouldRecreate =
                    _.includes(this.variableTypeComponentsIndexes, colIndex) &&
            ...
            options.onChange = (flags = {}, changed, modified) => {
                if (
                    ...
                    this.root?.id !== changed.instance.root.id
                ) {
                    changed.instance.root.triggerChange(
                        flags,
                        changed,
                        modified,
                    );
            ...
            const comp = this.createComponent(
                ...
                row,
                null,
                recreatePartially && currentRowComponents
                    ? currentRowComponents[colIndex]
                    : null,
            );
    ...
    validateRow(editRow, dirty, forceSilentCheck) {
        ...
        editRow.errors = !valid
            ? this.errors.filter((err) => !errorsSnapshot.includes(err))
            : null;
    ...
    checkComponentValidity(data, dirty, row, options = {}) {
        ...
        const superValid = super.checkComponentValidity(
            data,
            dirty,
            row,
            options,
        ...
        this.editRows.forEach((editRow, index) => {
            ...
            if (this.rowRefs) {
                ...
                if (rowContainer) {
                    const errorContainer = rowContainer.querySelector(
                        '.editgrid-row-error',
                    );
                    ...
                    if (
                        ...
                        errorContainer.textContent = this.t(
                            this.errorMessage('invalidRowError'),
                        );
        ...
        if (!rowsValid) {
            if (
                ...
                this.setCustomValidity(
                    this.t(this.errorMessage('invalidRowsError')),
                    dirty,
                );
            ...
            rowsEditing &&
            this.saveEditMode &&
            !this.component.openWhenEmpty
        ) {
            this.setCustomValidity(
                this.t(this.errorMessage('unsavedRowsError')),
                dirty,
            );
            return false;
        }
        ...
        if (
            this.errors?.length !== errorsLength &&
            this.root?.submitted &&
            !message
        ) {
    ...
    changeState(changed, flags) {
        if (
            this.visible &&
            (changed || (flags.resetValue && this.component.modalEdit))
        ) {
            this.rebuild();
        } else {
            this.redraw();
    ...
    setValue(value, flags = {}) {
        ...
        if (
            this.editRows[dataLength] &&
            this.editRows[dataLength].state === EditRowState.New
        ) {
            dataLength = dataLength + 1;
        }
        this.editRows
            .slice(dataLength)
            .forEach((editRow, index) =>
                this.baseRemoveRow(dataLength + index),
            );
    ...
    openWhenEmpty() {
        const shouldBeOpened =
            !this.dataValue.length && this.component.openWhenEmpty;
    ...
    emptyRows() {
        this.editRows.forEach((editRow, index) =>
            this.destroyComponents(false, index),
        );
        this.editRows = [];
    }
