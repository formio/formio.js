        ...
        return Harness.testCreate(SurveyComponent, comp1).then((component) => {
            ...
            inputs.forEach((input, index) => {
                if (index < 5) {
                    ...
                    assert.equal(
                        input.id,
                        `${component.key}-service-${comp1.values[index].value}`,
                    );
                } else {
                    // How do you like our service?
                    assert.equal(
                        input.name,
                        'data[surveyQuestions][howWouldYouRateTheTechnology]',
                    );
                    assert.equal(
                        input.id,
                        `${component.key}-howWouldYouRateTheTechnology-${
                            comp1.values[index - 5].value
                        }`,
                    );
                }
            });
        ...
        return Harness.testCreate(SurveyComponent, comp1).then((component) => {
            ...
            for (const input of inputs) {
                if (
                    ...
                    input.id ===
                        `${component.key}-howWouldYouRateTheTechnology-good`
                ) {
