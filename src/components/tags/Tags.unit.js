...
import { comp1, comp2, comp3, comp4, comp5, comp6 } from './fixtures';
...
describe('Tags Component', function () {
	...
	it('Should set placeholder', function (done) {
		Harness.testCreate(TagsComponent, comp4)
			.then((component) => {
				assert.equal(component.choices.config.placeholder, true);
				assert.equal(
					component.choices.config.placeholderValue,
					component.component.placeholder,
				);
				assert.equal(
					component.choices.input.element.attributes.placeholder.value,
					component.component.placeholder,
				);
				done();
			})
	...
	it('Check getValueAsString', function (done) {
		...
		Formio.createForm(element, modalTagsComponent)
			.then((form) => {
				...
				setTimeout(() => {
					const modalPreview =
						component.element.querySelector('[ref="openModal"]');
					assert.equal(
						modalPreview.textContent.trim(),
						'test, test1, test2',
						'All tags should be rendered inside Modal Preview',
					);
					form.destroy();
					done();
				}, 250);
		...
		Formio.createForm(element, form)
			.then((form) => {
				...
				setTimeout(() => {
					assert.deepEqual(
						tags.getValue(),
						value.join(tags.component.delimeter),
					);
					assert.deepEqual(form.submission.data.tags, value);
					assert.equal(tags.dataValue, value);
					assert.equal(
						tags.getValueAsString(value),
						value.join(`${tags.component.delimeter} `),
					);
			...
			.then((form) => {
				...
				setTimeout(() => {
					...
					setTimeout(() => {
						assert(
							tags.error,
							'Should set error after Tags component was blurred',
						);
						done();
					}, 500);
				}, 300);
