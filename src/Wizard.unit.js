			...
			.then(() => {
				...
				setTimeout(() => {
					const clickWizardBtn = (pathPart, clickError) => {
						const btn = _.get(
							wizard.refs,
							clickError ? pathPart : `${wizard.wizardKey}-${pathPart}`,
						);
						const clickEvent = new Event('click');
		...
		customizedWizard
			...
			.then(() => {
				...
				const navigatePage = (btnKey) => {
					const customBtn =
						customizedWizard.components[customizedWizard.page].getComponent(
							btnKey,
						).refs.button;
					const clickEvent = new Event('click');
	...
	it('Should correctly set values in HTML render mode', function (done) {
		...
		formHTMLMode
			...
			.then(() => {
				...
				setTimeout(() => {
					const numberValue =
						formHTMLMode.element.querySelector('[ref="value"]').textContent;
					assert.equal(
						+numberValue,
	...
	it('Should not clear wizard data when navigating between wizard pages with hidden panel', function (done) {
		...
		formWithHiddenPage
			...
			.then(() => {
				...
				setTimeout(() => {
					assert.equal(
						formWithHiddenPage.element.querySelector('[name="data[number]"]')
							.value,
						555,
					);

					const nextPageBtn =
						formWithHiddenPage.refs[`${formWithHiddenPage.wizardKey}-next`];
					nextPageBtn.dispatchEvent(clickEvent);
		...
		formWithSignatureHTMLMode
			...
			.then(() => {
				...
				setTimeout(() => {
					...
					assert.equal(
						signatureImage.src === formWithSignature.submission.data.signature,
						true,
					);
	...
	it('Should display editGrid submission data in readOnly mode', function (done) {
		...
		wizardForm
			...
			.then(() => {
				...
				setTimeout(() => {
					assert.equal(
						wizardForm.element.querySelector('[name="data[number]"]').value,
						'222',
					);
