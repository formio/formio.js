...
export default class Element {
    constructor(options) {
        ...
        this.options.i18next = this.i18next =
            this.options.i18next || I18n.init(this.options.i18n);
         ...
         */
        this.events =
            options && options.events ? options.events : new EventEmitter();
            ...
            _.each(this.events._events, (events, type) => {
                _.each(events, (listener) => {
                    if (
                        listener &&
                        this.id === listener.id &&
                        (includeExternal || listener.internal)
                    ) {
                        this.events.off(type, listener);
                    }
                });
    ...
    maskPlaceholder(mask) {
        return mask
            .map((char) =>
                char instanceof RegExp ? this.placeholderChar : char,
            )
            .join('');
    }
    ...
    setInputMask(input, inputMask, usePlaceholder) {
        if (input && inputMask) {
            const mask = FormioUtils.getInputMask(
                inputMask,
    ...
    interpolate(string, data, options = {}) {
        if (
            ...
            string = FormioUtils.translateHTMLTemplate(
                String(string),
                (value) => this.t(value),
            );
        }
    ...
    hook() {
        ...
        if (this.options && this.options.hooks && this.options.hooks[name]) {
            return this.options.hooks[name].apply(
                this,
