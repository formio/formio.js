...
export default class PDF extends Webform {
    ...
    attach(element) {
        return super.attach(element).then(() => {
            ...
            const submitButton = this.components.find(
                (c) => c.element === this.refs.button,
            );
            if (submitButton) {
                this.refs.button.classList.toggle(
                    'hidden',
    ...
    getSrc() {
        ...
        if (
            this.options.showCheckboxBackground ||
            this._form.settings.showCheckboxBackground
        ) {
    ...
    showErrors(error, triggerEvent) {
        ...
        const isSubmitErrorShown =
            this.refs.buttonMessage?.textContent.trim() === submitError;
...
if (typeof window !== 'undefined') {
    window.addEventListener('message', (event) => {
        ...
        if (
            ...
            Formio.forms[eventData.formId].emit(
                `iframe-${eventData.name}`,
                eventData.data,
            );
        }
    });
