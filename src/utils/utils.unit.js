...
describe('Util Tests', function () {
	...
	describe('checkTrigger', function () {
		it('should default to false', function () {
			expect(utils.checkCondition({}, { type: 'none' }, null, {})).to.be.equal(
				true,
			);
		});
		...
		it('should calculate simple triggers', function () {
			...
			const data1 = { test: 3 };
			const data2 = { test: 5 };
			expect(utils.checkTrigger(component, trigger, null, data1)).to.be.equal(
				true,
			);
			expect(utils.checkTrigger(component, trigger, null, data2)).to.be.equal(
				false,
			);
		});
