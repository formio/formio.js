...
export function evaluate(func, args, ret, tokenize) {
    ...
    if (typeof func === 'string') {
        ...
        try {
            ...
            console.warn(
                `An error occured within the custom function for ${componentKey}`,
                err,
            );
            returnVal = null;
            func = false;
    ...
    if (typeof func === 'function') {
        try {
            ...
            returnVal = null;
            console.warn(
                `An error occured within custom function for ${componentKey}`,
                err,
            );
        }
    ...
    if (condition.when) {
        ...
        const conditionsResult = _.map(conditions, (cond) => {
            const {
                value: comparedValue,
                operator,
                component: conditionComponentPath,
            } = cond;
            if (!conditionComponentPath) {
                return true;
            }
            const value = getComponentActualValue(
...
export function checkJsonConditional(
    ...
    try {
        ...
        console.warn(
            `An error occurred in jsonLogic advanced condition for ${component.key}`,
            err,
        );
        return onError;
    }
    ...
    const isTriggerCondtionComponentPath =
        condition.when || !condition.conditions
            ...
            : _.some(condition.conditions, (cond) =>
                  cond.component.startsWith(parentPath),
              );
...
export function setActionProperty(
    component,
    action,
    result,
    row,
...
export function getDateSetting(date) {
    ...
    let dateSetting =
        typeof date !== 'string' || date.indexOf('moment(') === -1
            ? moment(date)
            : null;
...
export function offsetDate(date, timezone) {
    ...
    return {
        date: new Date(
            date.getTime() +
                (dateMoment.utcOffset() + date.getTimezoneOffset()) * 60000,
        ),
...
export function formatDate(
    ...
    if (timezone === currentTimezone()) {
        ...
        if (format.match(/\s(z$|z\s)/)) {
            ...
            if (moment.zonesLoaded) {
                return momentDate
                    .tz(timezone)
                    .format(convertFormatToMoment(format));
            } else {
                return momentDate.format(
                    convertFormatToMoment(format.replace(/\s(z$|z\s)/, '')),
                );
            }
        }
    ...
    if (moment.zonesLoaded && timezone) {
        return momentDate
            .tz(timezone)
            .format(`${convertFormatToMoment(format)} z`);
    } else {
        return momentDate.format(convertFormatToMoment(format));
    }
}
...
export function matchInputMask(value, inputMask) {
    ...
    for (let i = 0; i < inputMask.length; i++) {
        ...
        if (
            !(
                (_.isRegExp(charPart) && charPart.test(char)) ||
                charPart === char
            )
        ) {
            return false;
...
export function uniqueKey(map, base) {
    let newKey = base;
    while (Object.prototype.hasOwnProperty.call(map, newKey)) {
...
export function bootstrapVersion(options) {
    ...
    if (
        ...
        typeof jQuery().collapse === 'function'
    ) {
        return parseInt(
            jQuery.fn.collapse.Constructor.VERSION.split('.')[0],
            10,
        );
    }
...
export function getContextComponents(
    ...
    context.utils.eachComponent(
        ...
        (component, path) => {
            const addToContextComponents = excludeNested
                ? !component.tree
                : true;
            if (
...
function translateElemValue(elem, translate) {
    ...
    if (elemValue !== translatedValue) {
        ...
        if (links && links.length) {
            if (
                links.length === 1 &&
                links[0].length === elem.innerHTML.length
            ) {
...
export function sanitize(string, options) {
    ...
    if (options.sanitizeConfig && options.sanitizeConfig.useProfiles) {
        Object.keys(options.sanitizeConfig.useProfiles).forEach((key) => {
            sanitizeOptions.USE_PROFILES[key] =
                options.sanitizeConfig.useProfiles[key];
        });
    }
    // Add attrs
    if (
    ...
    if (
        ...
        sanitizeOptions.ADD_URI_SAFE_ATTR =
            options.sanitizeConfig.addUriSafeAttr;
    }
...
export function getComponentPath(component, path = '') {
    if (
        !component ||
        !component.key ||
        component?._form?.display === 'wizard'
    ) {
        // unlike the Webform, the Wizard has the key and it is a duplicate of the panel key
        return path;
    }
    path =
        component.isInputComponent || component.input === true
            ? `${component.key}${path ? '.' : ''}${path}`
            : path;
...
export function isSelectResourceWithObjectValue(comp = {}) {
    ...
    return (
        reference ||
        (dataSrc === 'resource' && (!valueProperty || valueProperty === 'data'))
    );
}
