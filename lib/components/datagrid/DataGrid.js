  ...
  _createClass(DataGridComponent, [{
    ...
    value: function createRows(init, rebuild) {
      ...
      rowValues.forEach(function (row, index) {
        if (!rebuild && _this6.rows[index]) {
          _this6.setRowComponentsData(index, row);
        } else {
          _this6.rows[index] = _this6.createRowComponents(row, index);
          added = true;
        }
      ...
      if (removed) {
        removedRows.forEach(function (row) {
          return _this6.removeRowComponents(row);
        });
      }
    ...
    value: function createRowComponents(row, rowIndex) {
      ...
      this.component.components.map(function (col, colIndex) {
        ...
        var component = _this7.createComponent(columnComponent, options, row);

        component.parentDisabled = !!_this7.disabled;
        component.rowIndex = rowIndex;
        component.inDataGrid = true;

        if (columnComponent.tabindex && parseInt(columnComponent.tabindex) > _this7.tabIndex) {
          _this7.tabIndex = parseInt(columnComponent.tabindex);
        }
