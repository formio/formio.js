  ...
  _createClass(SelectComponent, [{
    ...
    value: function getNormalizedValues() {
      ...
      return this.component.data.values.map(function (value) {
        return {
          label: value.label,
          value: String(_this9.normalizeSingleValue(value.value))
        };
      });
    }
  }, {
    ...
    value: function asString(value) {
      ...
      var convertToString = function convertToString(data, valueProperty) {
        ...
        if (_this10.isBooleanOrNumber(data)) {
          data = data.toString();
        }

        if (Array.isArray(data) && data.some(function (item) {
          return _this10.isBooleanOrNumber(item);
        })) {
          data = data.map(function (item) {
            if (_this10.isBooleanOrNumber(item)) {
              item = item.toString();
            }
          });
        }
      ...
      if (Array.isArray(value)) {
        ...
        value.forEach(function (item) {
          return _items.push(_this10.itemTemplate(item));
        });
      ...
      if (this.component.data && this.component.data.headers) {
        try {
          _lodash.default.each(this.component.data.headers, function (header) {
            if (header.key) {
              headers.set(header.key, _this11.interpolate(header.value));
            }
          });
        } catch (err) {
          console.warn(err.message);
