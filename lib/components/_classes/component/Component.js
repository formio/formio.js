...
export default class Component extends Base {
    ...
    setErrorClasses(elements, dirty, { hasErrors, hasWarnings, hasInfos, hasMessages, }) {
    ...
    addQuill(element, settings, onChange) {
        ...
        settings = Object.assign(Object.assign({}, settings), { modules: {
                table: true
            } });
    ...
    calculateComponentValue(data, flags, row) {
        ...
        const { hidden, clearOnHide, calculateValue, calculateValueVariable, } = this.component;
    ...
    updateLogicContext(context) {
        return Object.assign(Object.assign({}, context), { targetComponentInstance: this });
    }
    calculateCondition(name, context = this.logicContext) {
        ...
        const { conjunction = 'and', parts = [], } = condition;
        const Conjunction = Conjunctions.getConjunction(conjunction);
        if (!Conjunction) {
            return false;
        }
        const conjunctionInstance = new Conjunction(updatedContext);
        const result = conjunctionInstance.execute(parts.map(((part) => {
            const evaluationContext = {
                part,
                context: updatedContext,
            };
            const evaluator = ({ part = evaluationContext.part, context = evaluationContext.context, } = evaluationContext) => this.calculateConditionPart(part, context);
        ...
        const { cachable = true, } = ((_a = updatedContext.engineOptions) !== null && _a !== void 0 ? _a : this.engineOptions);
        if (cachable) {
    ...
    calculateOperator(operator, context = this.logicContext) {
        const updatedContext = this.updateLogicContext(context);
        const { name, [`${name}Arguments`]: args = {}, [`${name}Options`]: options = {}, } = operator;
        const Operator = Operators.getOperator(name);
        if (!Operator) {
            return false;
        }
        const { presetArguments, } = Operator;
        const operatorInstance = new Operator(updatedContext);
        return operatorInstance.execute(_.mapValues(Object.assign(Object.assign({}, presetArguments), args), ({ valueSource, [`${valueSource}Input`]: input, }) => {
            const evaluationContext = {
                valueSource,
                input,
                context: updatedContext,
            };
            const evaluator = ({ valueSource = evaluationContext.valueSource, input = evaluationContext.input, context = evaluationContext.context, } = evaluationContext) => this.calculateValueDefinition(valueSource, input, context);
            evaluator.evaluationContext = evaluationContext;
    ...
    calculateVariable(name, context = this.logicContext) {
        ...
        const { valueSource, [`${valueSource}Input`]: input, transform = {}, } = variable;
        const evaluationContext = {
            valueSource,
            input,
            context: updatedContext,
        };
        const valueEvaluator = ({ valueSource = evaluationContext.valueSource, input = evaluationContext.input, context = evaluationContext.context, } = evaluationContext) => this.calculateValueDefinition(valueSource, input, context);
    ...
    applyTransform(valueEvaluator, transform, context = this.logicContext) {
        const updatedContext = this.updateLogicContext(context);
        const { name = 'identity', [`${name}Arguments`]: args = {}, [`${name}Options`]: options = {}, } = transform;
        const Transformer = Transformers.getTransformer(name);
        if (!Transformer) {
            return valueEvaluator();
        }
        const { presetArguments, } = Transformer;
        const transformerInstance = new Transformer(updatedContext);
        return transformerInstance.transform((Transformer.lazyValueEvaluation ? valueEvaluator : valueEvaluator()), _.mapValues(Object.assign(Object.assign({}, presetArguments), args), ({ valueSource, [`${valueSource}Input`]: input, }) => {
            const evaluationContext = {
                valueSource,
                input,
                context: updatedContext,
