...
function (_Element) {
  ...
  _createClass(Component, [{
      ...
      if (refreshOn) {
        if (Array.isArray(refreshOn)) {
          refreshOn.forEach(function (refreshData) {
            _this5.checkRefresh(refreshData, changed);
          });
        } else {
    ...
    value: function getWidgetValueAsString(value) {
      ...
      if (Array.isArray(value)) {
        ...
        value.forEach(function (val, index) {
          if (_this6.refs.input[index] && _this6.refs.input[index].widget) {
            values.push(_this6.refs.input[index].widget.getValueAsString(val));
          }
        });
        return values;
      }
    ...
    value: function checkComponentConditions(data, flags, row) {
      data = data || this.rootValue;
      flags = flags || {};
      row = row || this.data; // Check advanced conditions
    ...
    value: function applyActions(newComponent, actions, result, row, data) {
      ...
      return actions.reduce(function (changed, action) {
        switch (action.type) {
          case 'property':
            {
              FormioUtils.setActionProperty(newComponent, action, result, row, data, _this9);
              var property = action.property.value;

              if (!_lodash.default.isEqual(_lodash.default.get(_this9.component, property), _lodash.default.get(newComponent, property))) {
                changed = true;
              }
          ...
          case 'mergeComponentSchema':
            {
              var schema = _this9.evaluate(action.schemaDefinition, {
                value: _lodash.default.clone(_this9.getValue()),
                data: data,
                row: row,
                component: newComponent,
                result: result
              ...
              if (!_lodash.default.isEqual(_this9.component, newComponent)) {
                changed = true;
              }
    ...
    value: function removeInputError(elements) {
      ...
      if (elements) {
        elements.forEach(function (element) {
          return _this12.removeClass(_this12.performInputMapping(element), 'is-invalid');
        });
        elements.forEach(function (element) {
          return _this12.removeClass(_this12.performInputMapping(element), 'is-warning');
        });
      }
    }
  }, {
    ...
    value: function addCKE(element, settings, onChange) {
      ...
      settings = _lodash.default.merge(_lodash.default.get(this.options, 'editors.ckeditor.settings', {}), settings);
    ...
    value: function addQuill(element, settings, onChange) {
      ...
      return _Formio.default.requireLibrary('quill', 'Quill', _lodash.default.get(this.options, 'editors.quill.src', "".concat(QUILL_URL, "/quill.min.js")), true).then(function () {
        ...
        if (qlSource) {
          _this13.addEventListener(qlSource, 'click', function (event) {
            event.preventDefault();

            if (txtArea.style.display === 'inherit') {
              _this13.quill.setContents(_this13.quill.clipboard.convert(txtArea.value));
            }
        ...
        _this13.addEventListener(element, 'click', function () {
          return _this13.quill.focus();
    ...
    value: function addAce(element, settings, onChange) {
      settings = _lodash.default.merge(_lodash.default.get(this.options, 'editors.ace.settings', {}), settings || {});
    ...
    value: function restoreValue() {
      if (this.hasSetValue) {
        ...
        var defaultValue = this.component.multiple ? this.dataValue.length ? [this.defaultValue] : [] : this.defaultValue;
    ...
    value: function calculateComponentValue(data, flags, row) {
      ...
      var allowOverride = this.component.allowCalculateOverride; // Skip this operation if this component allows modification and it is no longer pristine.
    ...
    value: function checkComponentValidity(data, dirty, row) {
      ...
      var errors = _Validator.default.checkComponent(this, data);

      if (errors.length && (dirty || !this.pristine)) {
        var message = this.invalidMessage(data, dirty, true, row);
    ...
    value: function setCustomValidity(message, dirty, external) {
      ...
      this.refs.input.forEach(function (input) {
        input = _this14.performInputMapping(input);
    ...
    value: function shouldSkipValidation(data, dirty, row) {
      ...
      function () {
        return !_this15.visible;
      }, // Force valid if component is conditionally hidden.
      function () {
        return !_this15.checkCondition(row, data);
      }];
    ...
    value: function detachLogic() {
      ...
      this.logic.forEach(function (logic) {
        if (logic.trigger.type === 'event') {
          var event = _this17.interpolate(logic.trigger.event);

          _this17.off(event); // only applies to callbacks on this component
      ...
      this.logic.forEach(function (logic) {
        if (logic.trigger.type === 'event') {
          ...
          _this18.on(event, function () {
            ...
            if (_this18.applyActions(newComponent, logic.actions, args)) {
              // If component definition changed, replace it.
              if (!_lodash.default.isEqual(_this18.component, newComponent)) {
                _this18.component = newComponent;
              }
