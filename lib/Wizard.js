...
function (_Webform) {
  ...
  _createClass(Wizard, [{
    ...
    value: function attachHeader() {
      ...
      if (this.isBreadcrumbClickable()) {
        this.refs["".concat(this.wizardKey, "-link")].forEach(function (link, index) {
          _this4.addEventListener(link, 'click', function (event) {
            _this4.emit('wizardNavigationClicked', _this4.pages[index]);

            event.preventDefault();

            _this4.setPage(index);
          });
        });
      }
    }
    ...
    value: function detachNav() {
      ...
      _lodash.default.each(this.buttons, function (button) {
        _this5.removeEventListener(_this5.refs["".concat(_this5.wizardKey, "-").concat(button.name)], 'click');
      });
    }
  }, {
    key: "detachHeader",
    value: function detachHeader() {
      ...
      this.refs["".concat(this.wizardKey, "-link")].forEach(function (link) {
        _this6.removeEventListener(link, 'click');
      });
    }
  }, {
    key: "establishPages",
      ...
      if (this.originalComponents) {
        this.originalComponents.forEach(function (item) {
          if (item.type === 'panel') {
            ...
            var isVisible = (0, _utils.checkCondition)(item, _this7.data, _this7.data, _this7.component, _this7);

            if (isVisible) {
              visible.push(item);

              if (page) {
                _this7.pages.push(page);
              }
            }

            if (!page && isVisible) {
              page = _this7.createComponent(item, pageOptions);

              _this7.pages.push(page);

              page.eachComponent(function (component) {
                component.page = _this7.pages.length - 1;
              });
            } else if (page && !isVisible) {
              _this7.removeComponent(page);
            }
          } else if (item.type !== 'button') {
            if (!_this7.pages.length) {
              _this7.prefixComps.push(_this7.createComponent(item, pageOptions));
            } else {
              _this7.suffixComps.push(_this7.createComponent(item, pageOptions));
            }
          }
        });
      }
    ...
    value: function beforePage(next) {
      ...
      return new _nativePromiseOnly.default(function (resolve, reject) {
        _this8.hook(next ? 'beforeNext' : 'beforePrev', _this8.currentPage, _this8.submission, function (err) {
          if (err) {
            _this8.showErrors(err, true);

            reject(err);
          }

          var form = _this8.currentPage;
    ...
    value: function nextPage() {
      ...
      if (this.options.readOnly) {
        return this.setPage(this.getNextPage()).then(function () {
          _this9.emit('nextPage', {
            page: _this9.page,
            submission: _this9.submission
          });
        });
      ...
      if (this.checkValidity(this.submission.data, true, this.submission.data, true)) {
        ...
        return this.beforePage(true).then(function () {
          return _this9.setPage(_this9.getNextPage()).then(function () {
            _this9.emit('nextPage', {
              page: _this9.page,
              submission: _this9.submission
            });
    ...
    value: function prevPage() {
      ...
      return this.beforePage().then(function () {
        return _this10.setPage(_this10.getPreviousPage()).then(function () {
          _this10.emit('prevPage', {
            page: _this10.page,
            submission: _this10.submission
          });
        });
      });
    }
    ...
    value: function checkData(data, flags) {
      var dirty = this.currentPage.components.some(function (component) {
        return !component.isEmpty();
      });
      return _get(_getPrototypeOf(Wizard.prototype), "checkData", this).call(this, data, flags) && this.checkValidity(data, dirty, true);
    }
  }, {
    key: "cancel",
    ...
    value: function setComponentSchema() {
      ...
      this.component.components.map(function (item) {
        ...
        _this11.originalComponents.push(_lodash.default.clone(item));
      });
    ...
    get: function get() {
      ...
      [{
        ...
        if (_this12.hasButton(button.name)) {
