  ...
  if (typeof func === 'function') {
    try {
      if ((typeof window === "undefined" ? "undefined" : _typeof(window)) === 'object') {
        ...
        var sandbox = vm.createContext(_objectSpread({}, originalArgs, {
          result: null
        })); // Build the arg string

        var argStr = _lodash.default.keys(originalArgs).join();

        if (!Array.isArray(args)) {
          argStr = "{".concat(argStr, "}");
        } // Execute the script


        var script = new vm.Script("result = ".concat(func.toString(), "(").concat(argStr, ");"));
