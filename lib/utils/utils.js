...
import * as moment_1 from 'moment-timezone/moment-timezone';
export { moment_1 as moment };
export { jsonLogic };
/**
 * Evaluate a method.
 *
...
export function getDateSetting(date) {
    ...
    var dateSetting = ((typeof date !== 'string') || (date.indexOf('moment(') === -1)) ? moment(date) : null;
    if (dateSetting && dateSetting.isValid()) {
        return dateSetting.toDate();
    }
    dateSetting = null;
    try {
        var value = Evaluator.evaluator("return " + date + ";", 'moment')(moment);
...
export function loadZones(timezone) {
    ...
    // eslint-disable-next-line no-import-assign
    return moment.zonesPromise = fetch('https://cdn.form.io/moment-timezone/data/packed/latest.json').then(function (resp) { return resp.json().then(function (zones) {
