...
export function matchComponent(component, query) {
    if (isString(query)) {
        ...
        forOwn(query, function (value, key) {
            matches_1 = (get(component, key) === value);
            if (!matches_1) {
                return false;
            }
        });
    ...
    changes.forEach(function (change) {
        ...
        switch (change.op) {
            ...
            case 'remove':
                findComponent(form.components, change.key, null, function (component, path) {
                    found = true;
                    var oldComponent = get(form.components, path);
            ...
            case 'edit':
                findComponent(form.components, change.key, null, function (component, path) {
                    ...
                    try {
                        var oldComponent = get(form.components, path);
                        var newComponent_1 = applyPatch(component, change.patches).newDocument;
                        if (oldComponent.key !== newComponent_1.key) {
                            path.pop();
                        }
                        set(form.components, path, newComponent_1);
                    }
                    catch (err) {
                        failed.push(change);
                    }
