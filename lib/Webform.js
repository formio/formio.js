  ...
  _createClass(Webform, [{
    ...
    set: function set(lang) {
      ...
      return new _nativePromiseOnly.default(function (resolve, reject) {
        _this3.options.language = lang;

        if (_this3.i18next.language === lang) {
          return resolve();
        }

        try {
          _this3.i18next.changeLanguage(lang, function (err) {
            if (err) {
              return reject(err);
            }
    ...
    value: function showErrors(error, triggerEvent, onChange) {
      ...
      errors.forEach(function (err) {
        ...
        components.forEach(function (path) {
          var component = _this17.getComponent(path, _lodash.default.identity);
      ...
      errors.forEach(function (err) {
        if (err) {
          var createListItem = function createListItem(message, index) {
            ...
            var li = _this17.ce('li', params);

            var span = _this17.ce('span');

            li.style.cursor = 'pointer';

            _this17.setContent(span, (0, _utils.unescapeHTML)(message));

            _this17.appendTo(span, li);
          ...
          if (err.messages && err.messages.length) {
            ...
            err.messages.forEach(function (_ref, index) {
              ...
              var text = _this17.t('alertMessage', {
    ...
    value: function submit(before, options) {
      ...
      if (!before) {
        return this.beforeSubmit(options).then(function () {
          return _this21.executeSubmit(options);
        });
      } else {
    ...
    value: function submitUrl(URL, headers) {
      ...
      if (headers && headers.length > 0) {
        headers.map(function (e) {
          if (e.header !== '' && e.value !== '') {
            settings.headers[e.header] = _this22.interpolate(e.value, submission);
          }
        });
      }
      ...
      if (API_URL && settings) {
        _Formio.default.makeStaticRequest(API_URL, settings.method, submission, {
          ...
          _this22.emit('requestDone');

          _this22.setAlert('success', '<p> Success </p>');
        }).catch(function (e) {
          _this22.showErrors("".concat(e.statusText ? e.statusText : '', " ").concat(e.status ? e.status : e));

          _this22.emit('error', "".concat(e.statusText ? e.statusText : '', " ").concat(e.status ? e.status : e));

          console.error("".concat(e.statusText ? e.statusText : '', " ").concat(e.status ? e.status : e));

          _this22.setAlert('danger', "<p> ".concat(e.statusText ? e.statusText : '', " ").concat(e.status ? e.status : e, " </p>"));
        });
      } else {
